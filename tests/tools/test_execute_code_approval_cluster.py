...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad GPUCLOUD_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: GPUCLOUD_GATEWAY_SESSION set, a bound session
    key, and isolated gateway queues/callbacks. Yields the session_key."""
    monkeypatch.setenv("GPUCLOUD_GATEWAY_SESSION", "1")
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("GPUCLOUD_CRON_SESSION", "1")
    monkeypatch.delenv("GPUCLOUD_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_gpucloud_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "GPUCLOUD_HOME": "/h", "GPUCLOUD_PROFILE": "p",
        "GPUCLOUD_CONFIG": "/c.yaml", "GPUCLOUD_ENV": "/e",
        # other GPUCLOUD_* → dropped (broad prefix removed)
        "GPUCLOUD_BASE_URL": "https://x", "GPUCLOUD_INTERACTIVE": "1",
        "GPUCLOUD_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("GPUCLOUD_HOME", "GPUCLOUD_PROFILE", "GPUCLOUD_CONFIG", "GPUCLOUD_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "GPUCLOUD_BASE_URL", "GPUCLOUD_INTERACTIVE", "GPUCLOUD_KANBAN_DB",
...
def test_env_scrub_logs_dropped_gpucloud_vars(caplog):
    """Dropping a non-allowlisted, non-secret GPUCLOUD_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset GPUCLOUD_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "GPUCLOUD_HOME": "/h",          # allowlisted → kept, not logged
        "GPUCLOUD_BASE_URL": "https://x",   # dropped → logged
        "GPUCLOUD_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "GPUCLOUD_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "GPUCLOUD_HOME" in out and "PATH" in out
    assert "GPUCLOUD_BASE_URL" not in out and "GPUCLOUD_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "GPUCLOUD_BASE_URL" in msgs and "GPUCLOUD_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "GPUCLOUD_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped GPUCLOUD_* vars."""
    import logging
