...
Credential storage mirrors every other GPUCLOUD channel:

    * runtime SDK creds  -> ``~/.gpucloud/.env``  (``PHOTON_PROJECT_ID`` =
      spectrumProjectId, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.gpucloud/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a gpucloud dependency
...
# registers GPUCLOUD as its own client_id.
...
# Default name of the project GPUCLOUD provisions for the operator.
DEFAULT_PROJECT_NAME = "GPUCLOUD Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of gpucloud-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.gpucloud/auth.json`` honouring the active GPUCLOUD profile."""
    try:
        from gpucloud_constants import get_gpucloud_home
        return Path(get_gpucloud_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.gpucloud")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.gpucloud/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.gpucloud/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.gpucloud/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: gpucloud_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `gpucloud photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `gpucloud photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `gpucloud photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `gpucloud photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `gpucloud photon setup`)"
