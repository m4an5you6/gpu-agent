...
import { formatRefValue, gpucloudDirectiveFormatter } from './directive-text'
...
describe('gpucloudDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = gpucloudDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = gpucloudDirectiveFormatter.parse('@file:src/main.tsx the entry point')
