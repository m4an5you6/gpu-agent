"""
Backup and import commands for gpucloud CLI.

`gpucloud backup` creates a zip archive of the entire ~/.gpucloud/ directory
(excluding the gpucloud-agent repo and transient files).

`gpucloud import` restores from a backup zip, overlaying onto the current
GPUCLOUD_HOME root.
"""
...
from gpucloud_constants import get_default_gpucloud_root, get_gpucloud_home, display_gpucloud_home
...
_EXCLUDED_DIRS = {
    "gpucloud-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to gpucloud root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside GPUCLOUD_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the GPUCLOUD home directory."""
    gpucloud_root = get_default_gpucloud_root()

    if not gpucloud_root.is_dir():
        print(f"Error: GPUCLOUD home directory not found at {gpucloud_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"gpucloud-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"gpucloud-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_gpucloud_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(gpucloud_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(gpucloud_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(gpucloud_root)
    ...
    print(f"\nRestore with: gpucloud import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a GPUCLOUD backup.
    ...
    # Look for telltale files that a gpucloud home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a GPUCLOUD backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.gpucloud/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a gpucloud dir name
        if prefix in {".gpucloud", "gpucloud"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_gpucloud_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (gpucloud_root / "config.yaml").exists()
        has_env = (gpucloud_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has GPUCLOUD configuration.")
        ...
        for member in members:
            ...
            target = gpucloud_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(gpucloud_root.resolve())
            except ValueError:
        ...
        profiles_dir = gpucloud_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # gpucloud_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: gpucloud profile list  (after installing gpucloud)")

        # Guidance
        print()
        if not (gpucloud_root / "gpucloud-agent").is_dir():
            print("Note: The gpucloud-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: gpucloud update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  gpucloud -p {pname} gateway install")

        print("Done. Your GPUCLOUD configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and gpucloud backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to GPUCLOUD_HOME).
...
# — `gpucloud update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(gpucloud_home: Optional[Path] = None) -> Path:
    home = gpucloud_home or get_gpucloud_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    gpucloud_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(gpucloud_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside GPUCLOUD_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``gpucloud update``.
    ...
    Args:
        ...
        gpucloud_home: Override for the GPUCLOUD home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(gpucloud_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for gpucloud backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_gpucloud_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, gpucloud_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``gpucloud_root`` to ``out_path``.
...
def _pre_update_backup_dir(gpucloud_home: Optional[Path] = None) -> Path:
    home = gpucloud_home or get_gpucloud_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of GPUCLOUD_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<GPUCLOUD_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``gpucloud update``) should continue even if the backup fails.
    """
    gpucloud_root = gpucloud_home or get_default_gpucloud_root()
    if not gpucloud_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(gpucloud_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, gpucloud_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `gpucloud claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of GPUCLOUD_HOME under ``backups/`` before a
    ``gpucloud claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``gpucloud import <archive>``.  Writes to
    ``<GPUCLOUD_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `gpucloud import` and the
