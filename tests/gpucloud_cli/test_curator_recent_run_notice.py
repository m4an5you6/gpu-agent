...
The notice prints the most recent curator run summary on `gpucloud update`,
...
session start) so users normally never see the rename map. `gpucloud update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    importlib.reload(gpucloud_main)
