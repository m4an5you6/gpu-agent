#!/usr/bin/env python3
"""Live test harness for GPUCLOUD Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any gpucloud imports.
ORIGINAL_HOME = os.environ.get("GPUCLOUD_HOME")
ORIGINAL_AUTH = Path.home() / ".gpucloud" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.gpucloud/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.gpucloud/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="gpucloud_ts_live_"))
    gpucloud_home = home_dir / ".gpucloud"
    gpucloud_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, gpucloud_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".gpucloud" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, gpucloud_home / ".env")
        ...
        load_gpucloud_dotenv(gpucloud_home=str(Path.home() / ".gpucloud"))
    ...
    (gpucloud_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
...
def reset_module_state():
    """Drop cached modules so the new GPUCLOUD_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "gpucloud_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["GPUCLOUD_HOME"] = str(home)
...
def main():
    ...
    # Restore original GPUCLOUD_HOME
    if ORIGINAL_HOME is not None:
        os.environ["GPUCLOUD_HOME"] = ORIGINAL_HOME
    else:
