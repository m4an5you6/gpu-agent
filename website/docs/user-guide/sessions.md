...
GPUCLOUD Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.gpucloud/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
GPUCLOUD stores session history so it can resume conversations, but it does not
...
GPUCLOUD explicitly injects for that turn.
...
For example, if a user sends an image and asks GPUCLOUD to make a meme from it,
GPUCLOUD may inspect that image once with vision and run an image-processing
...
`gpucloud sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`gpucloud` or `gpucloud chat`) |
...
gpucloud -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
gpucloud -c "my project"   # → resumes "my project #3"
```
...
gpucloud --resume 20250305_091523_a1b2c3d4
gpucloud -r 20250305_091523_a1b2c3d4

# Resume by title
gpucloud --resume "refactoring auth"

# Or with the chat subcommand
gpucloud chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `gpucloud sessions list`.

### Conversation Recap on Resume

When you resume a session, GPUCLOUD displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a GPUCLOUD session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.gpucloud/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `gpucloud -r "<title>"` from the shell) and pick up where the platform left off.
...
GPUCLOUD automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `gpucloud sessions list` or `gpucloud sessions browse`.
...
```bash
gpucloud sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), GPUCLOUD creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`gpucloud -c "my project"`), it automatically picks the most recent session in the lineage.
...
GPUCLOUD provides a full set of session management commands via `gpucloud sessions`:
...
# Filter by platform
gpucloud sessions list --source telegram

# Show more sessions
gpucloud sessions list --limit 50
```
...
gpucloud sessions export backup.jsonl

# Export sessions from a specific platform
gpucloud sessions export telegram-history.jsonl --source telegram

# Export a single session
gpucloud sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
gpucloud sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
gpucloud sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
gpucloud sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
gpucloud sessions prune

# Custom age threshold
gpucloud sessions prune --older-than 30

# Only prune sessions from a specific platform
gpucloud sessions prune --source telegram --older-than 60

# Skip confirmation
gpucloud sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`gpucloud insights`](/reference/cli-commands#gpucloud-insights).
...
When GPUCLOUD cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, GPUCLOUD uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to GPUCLOUD in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.gpucloud/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.gpucloud/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.gpucloud/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |
...
`*.jsonl` files in `~/.gpucloud/sessions/`. They are no longer written or
read by GPUCLOUD. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every GPUCLOUD process in the same `GPUCLOUD_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.gpucloud/config.yaml`:
...
gpucloud sessions delete <session_id>
