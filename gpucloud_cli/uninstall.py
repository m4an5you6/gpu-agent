"""
GPUCLOUD Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.gpucloud/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove GPUCLOUD PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing gpucloud-agent or gpucloud PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# GPUCLOUD Agent" comment and following line
                if '# GPUCLOUD Agent' in line or '# gpucloud-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('gpucloud' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing gpucloud
                if 'gpucloud' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the gpucloud wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "gpucloud",
        Path("/usr/local/bin/gpucloud"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains gpucloud_cli reference)
                content = wrapper.read_text()
                if 'gpucloud_cli' in content or 'gpucloud-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(gpucloud_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``gpucloud`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this GPUCLOUD home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (gpucloud_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``gpucloud gateway run`` processes
    ...
    try:
        from gpucloud_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from gpucloud_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``GPUCLOUD_HOME`` and ``GPUCLOUD_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\gpucloud\git\cmd``, ``%LOCALAPPDATA%\gpucloud\git\bin``,
#      ``%LOCALAPPDATA%\gpucloud\git\usr\bin``, ``%LOCALAPPDATA%\gpucloud\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\gpucloud\git\`` and Node to
#      ``%LOCALAPPDATA%\gpucloud\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``gpucloud dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\gpucloud\gateway-service\`` and sometimes
...
def _gpucloud_path_markers(gpucloud_home: Path) -> list[str]:
    """Path-entry substrings that identify GPUCLOUD-owned User-PATH entries."""
    root = str(gpucloud_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare gpucloud-agent install dir.
    markers = [root + "\\gpucloud-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if GPUCLOUD_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "gpucloud".  We don't want to catch
    # unrelated entries like "cgpucloud-foo" or "ephermeral", so we look for
    # backslash-gpucloud as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(gpucloud_home: Path) -> list[str]:
    """Strip GPUCLOUD-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _gpucloud_path_markers(gpucloud_home)
            kept: list[str] = []
...
def remove_gpucloud_env_vars_windows() -> list[str]:
    """Delete GPUCLOUD_HOME and GPUCLOUD_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("GPUCLOUD_HOME", "GPUCLOUD_GIT_BASH_PATH"):
                try:
...
def _is_default_gpucloud_home(gpucloud_home: Path) -> bool:
    """Return True when ``gpucloud_home`` points at the default (non-profile) root."""
    try:
        from gpucloud_constants import get_default_gpucloud_root
        return gpucloud_home.resolve() == get_default_gpucloud_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its GPUCLOUD_HOME directory.

    We shell out to ``gpucloud -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current GPUCLOUD_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m gpucloud_cli.main` so we don't depend on a `gpucloud`
    #    wrapper that may be half-removed mid-uninstall.
    gpucloud_invocation = [_sys.executable, "-m", "gpucloud_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                gpucloud_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's GPUCLOUD_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``gpucloud uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$GPUCLOUD_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from gpucloud_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(gpucloud_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ GPUCLOUD Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No GPUCLOUD Chat GUI installation was found.")
        print(f"  Checked: {gpucloud_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The GPUCLOUD agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(gpucloud_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The GPUCLOUD agent at {gpucloud_home / 'gpucloud-agent'}")
        print(f"  • Your config, sessions, and secrets under {gpucloud_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The GPUCLOUD agent is still installed. Run 'gpucloud' to use the CLI,")
    print("or 'gpucloud uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.gpucloud/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.gpucloud/ for future reinstall
    """
    project_root = get_project_root()
    gpucloud_home = get_gpucloud_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie GPUCLOUD_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_gpucloud_home(gpucloud_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.gpucloud data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ GPUCLOUD Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {gpucloud_home / 'config.yaml'}")
    print(f"  Secrets: {gpucloud_home / '.env'}")
    print(f"  Data:    {gpucloud_home / 'cron/'}, {gpucloud_home / 'sessions/'}, {gpucloud_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their GPUCLOUD_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL GPUCLOUD data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the GPUCLOUD code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``gpucloud`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$GPUCLOUD_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in gpucloud_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\gpucloud\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(gpucloud_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No GPUCLOUD-owned PATH entries in User environment")

        log_info("Removing GPUCLOUD_HOME / GPUCLOUD_GIT_BASH_PATH User env vars...")
        removed_env = remove_gpucloud_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No GPUCLOUD-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing gpucloud command...")
    ...
    #     (only when they still point into this GPUCLOUD home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing GPUCLOUD-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(gpucloud_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No GPUCLOUD-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(gpucloud_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE GPUCLOUD_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from gpucloud_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(gpucloud_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.gpucloud/, just remove the gpucloud-agent subdir
            if gpucloud_home in project_root.parents or project_root.parent == gpucloud_home:
    ...
    #     under GPUCLOUD_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(gpucloud_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(gpucloud_home)
    ...
    # 5. Optionally remove ~/.gpucloud/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile GPUCLOUD_HOME dirs live under
        ...
        try:
            if gpucloud_home.exists():
                shutil.rmtree(gpucloud_home)
                log_success(f"Removed {gpucloud_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {gpucloud_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {gpucloud_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {gpucloud_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://gpucloud-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://gpucloud-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using GPUCLOUD Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m gpucloud_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import gpucloud_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``gpucloud_constants`` + ``gpucloud_cli.colors``
    (and lazily ``gpucloud_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m gpucloud_cli.uninstall")
    parser.add_argument(
        "--mode",
