...
it is automatically downloaded from GitHub releases to $GPUCLOUD_HOME/bin/tirith.
...
def _get_gpucloud_home() -> str:
    """Return the GPUCLOUD home directory, respecting GPUCLOUD_HOME env var."""
    return str(get_gpucloud_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_gpucloud_home(), ".tirith-install-failed")
...
def _gpucloud_bin_dir() -> str:
    """Return $GPUCLOUD_HOME/bin, creating it if needed."""
    d = os.path.join(_get_gpucloud_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $GPUCLOUD_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_gpucloud_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $GPUCLOUD_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $GPUCLOUD_HOME/bin/tirith
    ...
    gpucloud_bin = os.path.join(_gpucloud_bin_dir(), "tirith")
    if os.path.isfile(gpucloud_bin) and os.access(gpucloud_bin, os.X_OK):
        _resolved_path = gpucloud_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return gpucloud_bin
