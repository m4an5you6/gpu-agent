...
import { api, GPUCLOUD_BASE_PATH } from "@/lib/api";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${GPUCLOUD_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${GPUCLOUD_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?gpucloud_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-gpucloud-plugin", manifest.name);
