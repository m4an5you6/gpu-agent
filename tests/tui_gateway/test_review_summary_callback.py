...
def server():
    with patch.dict(
        "sys.modules",
        {
            "gpucloud_constants": MagicMock(
                get_gpucloud_home=MagicMock(return_value="/tmp/gpucloud_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'gpucloud-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'gpucloud-release' patched"
    }
