...
- ``gpucloud_cli.providers``        -- canonical provider identity + overlays
- ``gpucloud_cli.model_normalize``  -- per-provider name formatting
...
_GPUCLOUD_MODEL_WARNING = (
    "Nous Research GPUCLOUD 3 & 4 models are NOT agentic and are not designed "
    "for use with GPUCLOUD Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research GPUCLOUD 3 / GPUCLOUD 4 chat families.
# The previous substring check (`"gpucloud" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``gpucloud-brain:qwen3-14b-ctx16k`` that just
# happen to carry "gpucloud" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/GPUCLOUD-3-Llama-3.1-70B, gpucloud-4-405b, openrouter/gpucloud3:70b
# Negative examples it must NOT match:
#   gpucloud-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_GPUCLOUD_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])gpucloud[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_gpucloud_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous GPUCLOUD 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_GPUCLOUD_NON_AGENTIC_RE.search(model_name))


def _check_gpucloud_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous GPUCLOUD 3/4 chat model."""
    if is_nous_gpucloud_non_agentic(model_name):
        return _GPUCLOUD_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``gpucloud config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from gpucloud_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'gpucloud model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'gpucloud doctor' — config issues detected:"
        ...
        from gpucloud_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from gpucloud_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from gpucloud_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in gpucloud_cli.runtime_provider.resolve_runtime_provider;
    ...
    gpucloud_warn = _check_gpucloud_model_warning(new_model)
    if gpucloud_warn:
        warnings.append(gpucloud_warn)
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from gpucloud_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from gpucloud_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from gpucloud_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by gpucloud provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://gpucloud-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a GPUCLOUD release. Falls back to the in-repo
    ...
    # --- 1. Check GPUCLOUD-mapped providers ---
    from gpucloud_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from gpucloud_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for gpucloud_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(gpucloud_id)
        if (
            _alias_target
            and _alias_target != gpucloud_id
        ...
        pconfig = PROVIDER_REGISTRY.get(gpucloud_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (GPUCLOUD_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from gpucloud_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(gpucloud_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `gpucloud model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(gpucloud_id)
        if not model_ids:
            model_ids = curated.get(gpucloud_id, [])
            if gpucloud_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(gpucloud_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check GPUCLOUD-only providers (nous, openai-codex, copilot, opencode-go) ---
    from gpucloud_cli.providers import GPUCLOUD_OVERLAYS
    from gpucloud_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → GPUCLOUD provider ID.
    # GPUCLOUD_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use GPUCLOUD IDs ("copilot").
    _mdev_to_gpucloud = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in GPUCLOUD_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve GPUCLOUD slug — e.g. "github-copilot" → "copilot"
        gpucloud_slug = _mdev_to_gpucloud.get(pid, pid)
        if gpucloud_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(gpucloud_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, gpucloud_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or gpucloud_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(gpucloud_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", gpucloud_slug, exc)
        ...
        # configured.
        if not has_creds and gpucloud_slug == "anthropic":
            try:
                ...
                if (gpucloud_creds and gpucloud_creds.get("accessToken")) or \
        ...
        if gpucloud_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(gpucloud_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(gpucloud_slug)
                model_ids = _ids if _ids else (curated.get(gpucloud_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(gpucloud_slug, []) or curated.get(pid, [])
        elif gpucloud_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `gpucloud model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from gpucloud_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(gpucloud_slug, []) or curated.get(pid, [])
                if gpucloud_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(gpucloud_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": gpucloud_slug,
            "name": get_label(gpucloud_slug),
            "is_current": gpucloud_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "gpucloud",
        ...
        seen_slugs.add(gpucloud_slug.lower())
        _record_builtin_endpoint(gpucloud_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or GPUCLOUD_OVERLAYS (keeps /model in sync
    # with `gpucloud model`).
    try:
        from gpucloud_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is GPUCLOUD's canonical write key (matches
            ...
            # GPUCLOUD writes ``models:`` as a dict keyed by model id
            # (see gpucloud_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # GPUCLOUD's own writer uses; a hyphen variant is accepted
            ...
            # active model. GPUCLOUD's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, gpucloud_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`gpucloud_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from gpucloud_cli.models import fetch_openrouter_models
