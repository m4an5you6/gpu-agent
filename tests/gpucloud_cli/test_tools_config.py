...
def test_save_platform_tools_does_not_preserve_gpucloud_telegram():
...
def test_fresh_install_browser_default_is_free_local_not_paid_nous():
    ...
    """
    from gpucloud_cli.tools_config import _detect_active_provider_index
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'gpucloud-acp' noise).
    """
...
def test_reconfigure_provider_syncs_use_gateway(monkeypatch, provider, config_key, expected):
    ...
    monkeypatch.setattr(
        "gpucloud_cli.nous_subscription.ensure_nous_portal_access",
        lambda **kwargs: True,
    )
    config = {}
...
def test_configure_non_managed_provider_skips_portal_gate(monkeypatch):
    ...
    monkeypatch.setattr(
        "gpucloud_cli.nous_subscription.ensure_nous_portal_access", _boom
    )
...
def test_apply_provider_selection_web_sets_backend():
    ...
    from gpucloud_cli.tools_config import apply_provider_selection
...
def test_apply_provider_selection_does_not_prompt_or_post_setup(monkeypatch):
    ...
    from gpucloud_cli import tools_config
...
#    reported as added/removed by `gpucloud tools` ──────────────────────────
...
def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `gpucloud tools` printed ``- kanban``
