...
Maps each role archetype to the GPUCLOUD skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public gpucloud-agent
...
## GPUCLOUD skills relevant to video production

### Visual / rendering skills (`gpucloud-agent/skills/creative/`)
...
### Design / pre-production skills (`gpucloud-agent/skills/creative/`)
...
### Audio / media skills (`gpucloud-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`gpucloud-agent/skills/devops/`)
...
These are **not** GPUCLOUD skills but external CLIs / APIs that profiles invoke.
...
## Built-in GPUCLOUD tools for media review

These are native GPUCLOUD tools — not invoked via terminal but through their own
...
key is present in `~/.gpucloud/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every GPUCLOUD profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.gpucloud/.env` → environment variable.
...
When a new GPUCLOUD-public video skill ships:
