"""
Dump command for gpucloud CLI.

Outputs a compact, plain-text summary of the user's GPUCLOUD setup
...
from gpucloud_cli.config import get_gpucloud_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.gpucloud_build_sha`` by the Dockerfile's
    ``GPUCLOUD_GIT_SHA`` build-arg (see ``gpucloud_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``gpucloud dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from gpucloud_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(gpucloud_home: Path) -> int:
...
def _cron_summary(gpucloud_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = gpucloud_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"gpucloud_home:      {display_gpucloud_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `gpucloud auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `gpucloud auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["gpucloud-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(gpucloud_home)}")
    lines.append(f"  skills:             {_count_skills(gpucloud_home)}")
