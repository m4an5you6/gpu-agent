"""Tests for gpucloud_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_gpucloud_home):
    """Drop the example-dashboard fixture into the per-test GPUCLOUD_HOME
    ...
    isolated ``GPUCLOUD_HOME``.

    The user-plugin source is preferred over a transient
    ``GPUCLOUD_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, gpucloud-achievements, model providers
    ...
    from gpucloud_cli import web_server
    ...
    #      and start serving requests against a torn-down GPUCLOUD_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known GPUCLOUD vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-GPUCLOUD env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    """The desktop shell mints GPUCLOUD_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import importlib
        import gpucloud_cli.web_server as ws

        monkeypatch.setenv("GPUCLOUD_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("GPUCLOUD_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_gpucloud_home):
        """Create a TestClient and isolate the state DB under the test GPUCLOUD_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")

        import gpucloud_state
        from gpucloud_constants import get_gpucloud_home
        from gpucloud_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(gpucloud_state, "DEFAULT_DB_PATH", get_gpucloud_home() / "state.db")
    ...
    def test_get_media_404_for_missing_file(self):
        ...
        missing = get_gpucloud_home() / "images" / "nope.png"
        resp = self.client.get("/api/media", params={"path": str(missing)})
        assert resp.status_code == 404

    def test_get_media_requires_auth(self):
        from gpucloud_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_get_sessions_forwards_min_messages(self, monkeypatch):
        ...
        monkeypatch.setattr("gpucloud_state.SessionDB", _FakeDB)
    ...
    def test_desktop_audio_routes_registered(self):
        ...
        without failing CI.
        """
        from gpucloud_cli.web_server import app
    ...
    def test_elevenlabs_voices_unavailable_without_key(self, monkeypatch):
        import gpucloud_cli.web_server as web_server
    ...
    def test_update_gpucloud_returns_docker_guidance_without_spawning(self, monkeypatch):
        import gpucloud_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn gpucloud update")

        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_gpucloud_action", fail_spawn)
        web_server._ACTION_PROCS.pop("gpucloud-update", None)
        web_server._ACTION_RESULTS.pop("gpucloud-update", None)

        resp = self.client.post("/api/gpucloud/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "gpucloud-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/gpucloud-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/gpucloud-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull nousresearch/gpucloud-agent:latest" in line for line in status_data["lines"])

    def test_update_gpucloud_spawns_on_non_docker_install(self, monkeypatch):
        import gpucloud_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "_spawn_gpucloud_action", fake_spawn)
        web_server._ACTION_PROCS.pop("gpucloud-update", None)
        web_server._ACTION_RESULTS.pop("gpucloud-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "gpucloud-update"}
        assert calls == [(["update"], "gpucloud-update")]
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("gpucloud-update", None)
        web_server._ACTION_RESULTS.pop("gpucloud-update", None)
        web_server._ACTION_PROCS["gpucloud-update"] = proc

        resp = self.client.get("/api/actions/gpucloud-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "gpucloud-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["gpucloud-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from gpucloud_cli.web_server import _channel_managed_env_keys
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from gpucloud_cli.web_server import (
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from gpucloud_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from gpucloud_cli.config import save_env_value
        from gpucloud_cli.web_server import _SESSION_TOKEN
    ...
    def test_update_messaging_platform_saves_env_and_enablement(self):
        from gpucloud_cli.config import load_config, load_env
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "GPUCLOUD Agent"}
        assert kwargs["headers"]["Accept"] == "application/json"
        assert kwargs["headers"]["Authorization"] == "Bearer poll-secret"
        assert kwargs["headers"]["Content-Type"] == "application/json"
        assert kwargs["headers"]["User-Agent"].startswith("GPUCLOUDDashboard/")
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "gpucloud_pair123_bot",
                "deep_link": "https://t.me/newbot/GPUCLOUDSetupBot/gpucloud_pair123_bot",
                "qr_payload": "https://t.me/newbot/GPUCLOUDSetupBot/gpucloud_pair123_bot",
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "gpucloud_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/GPUCLOUDSetupBot/gpucloud_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/GPUCLOUDSetupBot/gpucloud_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "gpucloud_pair_ready_bot",
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "gpucloud_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/GPUCLOUDSetupBot/gpucloud_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/GPUCLOUDSetupBot/gpucloud_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "gpucloud_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/GPUCLOUDSetupBot/gpucloud_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/GPUCLOUDSetupBot/gpucloud_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/gpucloud_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        import gpucloud_cli.nous_subscription as ns
        ...
        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "nous", "model": "gpucloud-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from gpucloud_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from gpucloud_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_reports_stale_auxiliary_pins(self):
        ...
        cfg = load_config()
        cfg["model"] = {"provider": "nous", "default": "gpucloud-4"}
        cfg["auxiliary"] = {
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        model (mirroring `gpucloud model`), not the first curated paid entry."""
        import gpucloud_cli.models as models_mod
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from gpucloud_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from gpucloud_cli.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_gpucloud_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_list_includes_default(self):
        from gpucloud_constants import get_gpucloud_home
        get_gpucloud_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from gpucloud_constants import get_gpucloud_home
        import gpucloud_cli.profiles as profiles_mod

        gpucloud_home = get_gpucloud_home()
        gpucloud_home.mkdir(parents=True, exist_ok=True)
        (gpucloud_home / "config.yaml").write_text(
            "model:\n  provider: openrouter\n  name: anthropic/claude-sonnet-4.6\n",
            encoding="utf-8",
        )
        named = gpucloud_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_gpucloud_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profile_setup_command_uses_gpucloud_for_default_profile(self):
        from gpucloud_constants import get_gpucloud_home
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "gpucloud setup"
    ...
    def test_profiles_create_with_clone_from_default_copies_default_skills(self, monkeypatch):
        ...
        default_skill = get_gpucloud_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = get_gpucloud_home() / "profiles" / "cloned" / "skills" / "custom" / "new-skill" / "SKILL.md"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_gpucloud_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_gpucloud_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_gpucloud_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_gpucloud_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import gpucloud_cli.skills_config as skills_config
        import gpucloud_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import gpucloud_cli.tools_config as tools_config
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from gpucloud_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from gpucloud_cli.web_server import _denormalize_config_from_web
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from gpucloud_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.gpucloud/dashboard-themes/."""
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``gpucloud_cli/web_server.py``).
    ...
    def _setup_test_client(self, monkeypatch, _isolate_gpucloud_home):
        ...
        monkeypatch.setattr(
            gpucloud_state, "DEFAULT_DB_PATH", get_gpucloud_home() / "state.db"
        )
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in gpucloud_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_gpucloud_home):
        ...
        # Pin the SessionDB to the isolated GPUCLOUD_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_gpucloud_home, _install_example_plugin):
        ...
        installs it into the per-test ``GPUCLOUD_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into GPUCLOUD_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, gpucloud-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (gpucloud-achievements is loaded by default).
        resp = self.client.get("/api/plugins/gpucloud-achievements/overview")
        assert resp.status_code == 401
...
# or ``sh -c 'printf …'``) instead of the real ``gpucloud --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        import gpucloud_cli.main as main_mod
        ...
        assert env["GPUCLOUD_TUI_INLINE"] == "1"
        assert env["GPUCLOUD_TUI_DISABLE_MOUSE"] == "1"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        monkeypatch.setattr(
            ...
            lambda resume=None, sidecar_url=None: (
                ["/bin/sh", "-c", "printf gpucloud-ws-ok"],
                None,
                None,
            ),
        )
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"gpucloud-ws-ok" in buf:
                    break
            assert b"gpucloud-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from gpucloud_cli.pty_bridge import PtyUnavailableError
        ...
        import gpucloud_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        GPUCLOUD_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from gpucloud_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("GPUCLOUD_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_gpucloud_home, _install_example_plugin):
        ...
        the per-test ``GPUCLOUD_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_gpucloud_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.setenv("GPUCLOUD_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under GPUCLOUD_DESKTOP=1"

    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_gpucloud_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.delenv("GPUCLOUD_DESKTOP", raising=False)
