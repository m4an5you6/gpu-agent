"""SimpleX Chat platform adapter (GPUCLOUD plugin).
...
This adapter ships as a GPUCLOUD platform plugin under
``plugins/platforms/simplex/``. The GPUCLOUD plugin loader scans the
...
Optional environment variables:
    ...
    GPUCLOUD_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``gpucloud setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside GPUCLOUD — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("GPUCLOUD_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``gpucloud setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.gpucloud/.env`` via
    ``gpucloud_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "gpucloud_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.gpucloud/.env"
        )
        return
