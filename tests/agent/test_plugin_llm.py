...
""",
        ...
        from gpucloud_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        gpucloud_home = tmp_path / ".gpucloud"
        gpucloud_home.mkdir()
        (gpucloud_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("GPUCLOUD_HOME", str(gpucloud_home))
        from gpucloud_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from gpucloud_cli.plugins import PluginContext, PluginManifest, PluginManager
