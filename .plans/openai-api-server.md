# OpenAI-Compatible API Server for GPUCLOUD Agent
...
SSE streaming. By exposing this endpoint, gpucloud-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  gpucloud-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.gpucloud/.env`
2. Run `gpucloud gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with gpucloud-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns gpucloud-agent as a model) |
...
Authorization: Bearer gpucloud-api-key-here
...
But gpucloud-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "gpucloud-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured gpucloud-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of gpucloud-agent's core prompt
...
Once implemented, gpucloud-agent works as a drop-in backend for:
