...
``_check_spotify_available()`` — when the user has not run ``gpucloud auth
spotify``, the tools remain registered (so they appear in ``gpucloud tools``) but the runtime check prevents dispatch.
...
The Spotify auth flow (``gpucloud auth spotify``), CLI plumbing, and docs
