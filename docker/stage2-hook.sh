...
#
# Wired into the image as /etc/cont-init.d/01-gpucloud-setup by the
...
INSTALL_DIR="/opt/gpucloud"

# Drop to gpucloud via s6-setuidgid, but skip it when already non-root.
as_gpucloud() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid gpucloud "$@"; }
...
# the container starts non-root. The baked image trees (/opt/data, /opt/gpucloud/
# .venv, ui-tui, node_modules) stay owned by the gpucloud build UID (10000), so an
...
# into $GPUCLOUD_HOME). See #34837 for the supervision-tree side of this.
#
# The supported way to match host-side ownership is to start as root (the image
# default) and pass GPUCLOUD_UID/GPUCLOUD_GID — or the PUID/PGID aliases — which the
...
# `id -u gpucloud` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# gpucloud UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u gpucloud)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-gpucloud UID).
...
and the baked image directories are owned by the gpucloud user (UID $(id -u gpucloud)),
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e GPUCLOUD_UID=\$(id -u) -e GPUCLOUD_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the gpucloud user to that UID/GID at boot and chowns the data
...
# root can create (e.g. `GPUCLOUD_HOME=/home/gpucloud/.gpucloud` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for GPUCLOUD_UID/GPUCLOUD_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  GPUCLOUD_UID/
# GPUCLOUD_GID still win when both are set.  See #15290, salvages #25872.
GPUCLOUD_UID="${GPUCLOUD_UID:-${PUID:-}}"
GPUCLOUD_GID="${GPUCLOUD_GID:-${PGID:-}}"

if [ -n "${GPUCLOUD_UID:-}" ] && validate_uid_gid "$GPUCLOUD_UID" && [ "$GPUCLOUD_UID" != "$(id -u gpucloud)" ]; then
    echo "[stage2] Changing gpucloud UID to $GPUCLOUD_UID"
    usermod -u "$GPUCLOUD_UID" gpucloud
fi
if [ -n "${GPUCLOUD_GID:-}" ] && validate_uid_gid "$GPUCLOUD_GID" && [ "$GPUCLOUD_GID" != "$(id -g gpucloud)" ]; then
...
# host's `docker` group (or root). The supervised gpucloud user (UID 10000)
...
# gpucloud process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes gpucloud. Idempotent across container
...
#   - gpucloud is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G gpucloud 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] gpucloud already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" gpucloud 2>/dev/null; then
        echo "[stage2] Added gpucloud to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group gpucloud failed; docker backend may fail with EACCES"
    fi
    break
done
...
actual_gpucloud_uid=$(id -u gpucloud)
needs_chown=false
if [ "$(stat -c %u "$GPUCLOUD_HOME" 2>/dev/null)" != "$actual_gpucloud_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $GPUCLOUD_HOME (targeted) to gpucloud ($actual_gpucloud_uid)"
    ...
    chown gpucloud:gpucloud "$GPUCLOUD_HOME" 2>/dev/null || \
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing; do
        if [ -e "$GPUCLOUD_HOME/$sub" ]; then
            chown -R gpucloud:gpucloud "$GPUCLOUD_HOME/$sub" 2>/dev/null || \
...
# GPUCLOUD-owned trees under $INSTALL_DIR must be re-chowned whenever the
# runtime gpucloud UID no longer owns them — otherwise:
...
# The set mirrors the build-time `chown -R gpucloud:gpucloud` line in the
...
#
# This MUST be gated independently of the $GPUCLOUD_HOME ownership check
# above. `usermod -u <new> gpucloud` re-chowns the gpucloud home dir
# ($GPUCLOUD_HOME == /opt/data) to the new UID as a side effect, so after a
# GPUCLOUD_UID/PUID remap `stat $GPUCLOUD_HOME` always already matches the new
# UID and `needs_chown` is false — but the build trees under /opt/gpucloud
# are NOT touched by usermod and remain owned by the build-time UID
# (10000). Gating them on $GPUCLOUD_HOME ownership (as #35027 did) silently
...
# when the venv is not already owned by the runtime gpucloud UID. Idempotent
...
if [ -n "$venv_owner" ] && [ "$venv_owner" != "$actual_gpucloud_uid" ]; then
    echo "[stage2] Fixing ownership of build trees under $INSTALL_DIR to gpucloud ($actual_gpucloud_uid)"
    chown -R gpucloud:gpucloud \
        "$INSTALL_DIR/.venv" \
        "$INSTALL_DIR/ui-tui" \
        "$INSTALL_DIR/gateway" \
...
if [ -d "$GPUCLOUD_HOME/profiles" ]; then
    chown -R gpucloud:gpucloud "$GPUCLOUD_HOME/profiles" 2>/dev/null || true
fi

# Reset ownership of gpucloud-owned top-level state files on every boot.
# The targeted data-volume chown above only covers gpucloud-owned
# *subdirectories*; loose state files living directly under $GPUCLOUD_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> gpucloud …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged gpucloud runtime then hits
...
# sweep so host-owned files in a bind-mounted $GPUCLOUD_HOME are never
...
# entries of gpucloud_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$GPUCLOUD_HOME/$f" ]; then
        chown gpucloud:gpucloud "$GPUCLOUD_HOME/$f" 2>/dev/null || true
    fi
done
...
if [ -f "$GPUCLOUD_HOME/config.yaml" ]; then
    chown gpucloud:gpucloud "$GPUCLOUD_HOME/config.yaml" 2>/dev/null || true
...
as_gpucloud mkdir -p \
    ...
    "$GPUCLOUD_HOME/home" \
    "$GPUCLOUD_HOME/pairing" \
    "$GPUCLOUD_HOME/platforms/pairing"
...
printf 'docker\n' | as_gpucloud tee "$GPUCLOUD_HOME/.install_method" >/dev/null \
    || true
...
seed_one() {
    ...
    if [ ! -f "$GPUCLOUD_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        as_gpucloud cp "$INSTALL_DIR/$src" "$GPUCLOUD_HOME/$dest"
    fi
}
...
if [ -f "$GPUCLOUD_HOME/.env" ]; then
    chown gpucloud:gpucloud "$GPUCLOUD_HOME/.env" 2>/dev/null || true
    chmod 600 "$GPUCLOUD_HOME/.env" 2>/dev/null || true
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $GPUCLOUD_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `gpucloud update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set GPUCLOUD_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$GPUCLOUD_HOME/config.yaml" ]; then
    s6-setuidgid gpucloud "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$GPUCLOUD_HOME/auth.json" ] && [ -n "${GPUCLOUD_AUTH_JSON_BOOTSTRAP:-}" ]; then
    ...
    chown gpucloud:gpucloud "$GPUCLOUD_HOME/auth.json" 2>/dev/null || true
...
# can set GPUCLOUD_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# GPUCLOUD_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$GPUCLOUD_HOME/gateway_state.json" ] && \
        [ "${GPUCLOUD_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    printf '{"gateway_state":"running"}\n' > "$GPUCLOUD_HOME/gateway_state.json"
    chown gpucloud:gpucloud "$GPUCLOUD_HOME/gateway_state.json" 2>/dev/null || true
    chmod 644 "$GPUCLOUD_HOME/gateway_state.json"
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_gpucloud "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/gpucloud/.playwright) with
