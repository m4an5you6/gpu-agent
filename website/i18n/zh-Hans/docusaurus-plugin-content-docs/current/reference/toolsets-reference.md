...
| `gpucloud-telegram` | 与 `hermes-cli` 相同。 |
| `gpucloud-discord` | 在 `hermes-cli` 基础上添加了 `discord` 和 `discord_admin`。 |
| `gpucloud-slack` | 与 `hermes-cli` 相同。 |
| `gpucloud-whatsapp` | 与 `hermes-cli` 相同。 |
...
| `gpucloud-gateway` | 内部 gateway 编排器工具集——所有 `hermes-<platform>` 工具集的并集；当 gateway 需要接受任意消息来源时使用。 |
