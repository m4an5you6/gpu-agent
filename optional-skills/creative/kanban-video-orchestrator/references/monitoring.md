...
gpucloud kanban watch --tenant <project-slug>

# Snapshot of the board
gpucloud kanban list --tenant <project-slug>
gpucloud kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
gpucloud kanban stats --tenant <project-slug>

# Visual dashboard (browser)
gpucloud dashboard

# Inspect a specific task (includes comments + events)
gpucloud kanban show <task-id>

# Follow a single task's event stream
gpucloud kanban tail <task-id>
```

Verify available subcommands with `gpucloud kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `gpucloud kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `gpucloud kanban block <id>` then `gpucloud kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `gpucloud kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `gpucloud kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `gpucloud profile list` (profile exists?), `gpucloud status` (gateway/dispatcher up?), and verify tenant. |
...
gpucloud kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
gpucloud kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(gpucloud kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
gpucloud kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
gpucloud kanban block <task-id>
gpucloud kanban archive <task-id>

# Diagnose what happened
gpucloud kanban show <task-id>      # task body, comments, recent events
gpucloud kanban tail <task-id>      # follow the live event stream
gpucloud kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `gpucloud kanban create` for the director
...
while true; do
    clear
    gpucloud kanban list --tenant <slug>
    echo "---"
    gpucloud kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `gpucloud kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `gpucloud profile list` and start
