...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_gpucloud_home() / "sessions")
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.gpucloud/config.yaml (primary user-facing config)
    3. ~/.gpucloud/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_gpucloud_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``gpucloud config set gateway.streaming.*``
