"""Tests for the `gpucloud memory reset` CLI command.
...
- Profile-scoped reset (uses GPUCLOUD_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake GPUCLOUD_HOME with memory files."""
    gpucloud_home = tmp_path / ".gpucloud"
    memories = gpucloud_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("GPUCLOUD_HOME", str(gpucloud_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nGPUCLOUD repo is at ~/.gpucloud/gpucloud-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `gpucloud memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        gpucloud_home = tmp_path / ".gpucloud"
        (gpucloud_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("GPUCLOUD_HOME", str(gpucloud_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's GPUCLOUD_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_gpucloud_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
