...
def test_gpucloud_provider_subclass_exists():
    """GPUCLOUDMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _GPUCLOUD_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _GPUCLOUD_PROVIDER_CLS is not None
    assert issubclass(_GPUCLOUD_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_gpucloud_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns GPUCLOUDMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _GPUCLOUD_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("GPUCLOUD_HOME", str(tmp_path))
    ...
    assert _GPUCLOUD_PROVIDER_CLS is not None
    assert isinstance(provider, _GPUCLOUD_PROVIDER_CLS)
    assert provider._gpucloud_server_name == "srv"
