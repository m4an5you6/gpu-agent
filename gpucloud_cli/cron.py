"""
Cron subcommand for gpucloud CLI.
...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(gpucloud\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*gpucloud)"
    r"|(systemctl\s+(restart|stop|start)\s+.*gpucloud)"
    r"|(p?kill\s+.*gpucloud.*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'gpucloud cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: gpucloud gateway install", Colors.DIM))
        print(color("                    sudo gpucloud gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    gpucloud gateway install    # Install as a user service")
        print("    sudo gpucloud gateway install --system  # Linux servers: boot-time system service")
        print("    gpucloud gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `gpucloud gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: gpucloud cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
