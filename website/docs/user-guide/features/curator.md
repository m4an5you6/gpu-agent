...
gpucloud curator list-archived    # list skills currently in ~/.gpucloud/skills/.archive/
gpucloud curator archive <skill>  # manually archive a single skill now
gpucloud curator prune [--days N] # bulk-archive agent-created skills idle >= N days (default 90)
```
...
`~/.gpucloud/skills/.usage.json`. A skill qualifies when ALL of the following
are true:

1. Its name is **not** in `~/.gpucloud/skills/.bundled_manifest` (bundled skills shipped with the repo).
2. Its name is **not** in `~/.gpucloud/skills/.hub/lock.json` (hub-installed skills).
...
If you manually created a `SKILL.md` or pointed GPUCLOUD at an external skill
...
**To see which skills the curator actually manages**, run `gpucloud curator status`.
...
- Archives are recoverable via `gpucloud curator restore <name>`

If you want to protect a specific skill from ever being touched — for example a
hand-authored skill you rely on — use `gpucloud curator pin <name>`. See the next
section.
