"""``gpucloud portal`` — small CLI surface for Nous Portal users.
...
already in ``gpucloud auth`` or ``gpucloud tools``. It's a discovery + status
...
def _cmd_status(args) -> int:
    ...
    if logged_in:
        ...
        print(f"  Login:   gpucloud auth add nous --type oauth")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `gpucloud auth add nous --type oauth`.", Colors.YELLOW))
        print()
