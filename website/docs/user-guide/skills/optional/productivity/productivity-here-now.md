...
| | |
|---|---|
| Source | Optional — install with `gpucloud skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${GPUCLOUD_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${GPUCLOUD_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${GPUCLOUD_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client gpucloud
```
...
```bash
PUBLISH="${GPUCLOUD_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client gpucloud
```
...
```bash
DRIVE="${GPUCLOUD_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `gpucloud`)    |
