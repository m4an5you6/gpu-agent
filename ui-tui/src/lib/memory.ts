...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because GPUCLOUD_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.GPUCLOUD_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.gpucloud', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `gpucloud-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.GPUCLOUD_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.GPUCLOUD_HEAPDUMP_MAX_BYTES?.trim()
