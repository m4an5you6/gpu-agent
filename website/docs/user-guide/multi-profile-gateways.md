...
If you only run one GPUCLOUD agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more GPUCLOUD agents that should all
...
(`ai.gpucloud.gateway-<name>.plist`) or systemd user service
(`gpucloud-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
gpucloud profile create coder
gpucloud profile create personal-bot
gpucloud profile create research
...
`~/.local/bin/gpucloud-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: gpucloud-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    gpucloud gateway "$action"
  else
    gpucloud -p "$profile" gateway "$action"
  fi
}
...
```bash
gpucloud-gateways start      # start every configured profile
gpucloud-gateways stop       # stop every configured profile
gpucloud-gateways restart    # restart all
gpucloud-gateways status     # status across all
gpucloud-gateways list       # delegates to `gpucloud gateway list`
```

:::tip
The `default` profile is targeted with `gpucloud gateway <action>` (no `-p`),
not `gpucloud -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `gpucloud -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.gpucloud.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/gpucloud-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.gpucloud.gateway.plist` /
`gpucloud-gateway.service`.
...
```bash
# Default profile
tail -f ~/.gpucloud/logs/gateway.log
tail -f ~/.gpucloud/logs/gateway.error.log

# Named profile
tail -f ~/.gpucloud/profiles/<name>/logs/gateway.log
tail -f ~/.gpucloud/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.gpucloud/logs/gateway.log ~/.gpucloud/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
gpucloud logs -f                  # follow default profile
gpucloud -p coder logs -f         # follow one profile
gpucloud logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
gpucloud profile list             # profiles + model + gateway state
gpucloud-gateways status          # full status across every profile
launchctl list | grep gpucloud    # macOS — PIDs and labels
systemctl --user list-units 'gpucloud-gateway-*'   # Linux — units
```
...
```
~/.gpucloud/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.gpucloud/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
gpucloud config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.gpucloud/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=gpucloud --why="gateways running" \
  sleep infinity &
...
`gpucloud-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.gpucloud/.env ~/.gpucloud/profiles/*/.env
```

## Updating the code

`gpucloud update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `gpucloud gateway start` after a previous `gpucloud gateway stop`. The
...
```bash
ps -ef | grep "gpucloud_cli.*-p <profile>"
cat ~/.gpucloud/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.gpucloud.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.gpucloud.gateway-<profile>.plist

# Linux
systemctl --user restart gpucloud-gateway-<profile>.service
```

### Health check

```bash
gpucloud doctor                  # default profile
gpucloud -p <profile> doctor     # one profile
```
