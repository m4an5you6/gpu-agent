"""
LINE Messaging API platform adapter for GPUCLOUD Agent.
...
to GPUCLOUD Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``GPUCLOUD_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            gpucloud_home = Path.home().joinpath(".gpucloud").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``gpucloud status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``gpucloud status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``gpucloud setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.gpucloud/.env`` via ``gpucloud_cli.config``.
    """
    print()
    ...
    try:
        from gpucloud_cli.config import get_env_var, set_env_var
    except ImportError:
        print("gpucloud_cli.config not available; set LINE_* vars manually in ~/.gpucloud/.env")
        return
