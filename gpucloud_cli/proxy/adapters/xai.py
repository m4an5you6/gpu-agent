...
# xAI's public API is OpenAI-compatible for the endpoints GPUCLOUD commonly
# uses. The Responses endpoint is included because GPUCLOUD' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via GPUCLOUD-managed OAuth credentials."""

    auth_hint = "gpucloud auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`gpucloud auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`gpucloud auth reset xai-oauth` or re-authenticate with "
                    "`gpucloud auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `gpucloud auth add xai-oauth --type oauth`."
            )
