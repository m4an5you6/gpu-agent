"""
Tests for timezone support (gpucloud_time module + integration points).
...
import gpucloud_time


def _reset_gpucloud_time_cache():
    """Reset the gpucloud_time module cache (replacement for removed reset_cache)."""
    gpucloud_time._cached_tz = None
    gpucloud_time._cached_tz_name = None
    gpucloud_time._cache_resolved = False


# =========================================================================
# gpucloud_time.now() — core helper
# =========================================================================

class TestGPUCLOUDTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_gpucloud_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["GPUCLOUD_TIMEZONE"] = "Asia/Kolkata"
        result = gpucloud_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["GPUCLOUD_TIMEZONE"] = "UTC"
        result = gpucloud_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["GPUCLOUD_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="gpucloud_time"):
            result = gpucloud_time.now()
    ...
    def test_cache_invalidation(self):
        ...
        os.environ["GPUCLOUD_TIMEZONE"] = "UTC"
        _reset_gpucloud_time_cache()
        r1 = gpucloud_time.now()
        assert r1.utcoffset() == timedelta(0)

        os.environ["GPUCLOUD_TIMEZONE"] = "Asia/Kolkata"
        _reset_gpucloud_time_cache()
        r2 = gpucloud_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["GPUCLOUD_TIMEZONE"] = "Europe/London"
        tz = gpucloud_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["GPUCLOUD_TIMEZONE"] = "Not/A/Timezone"
        tz = gpucloud_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When GPUCLOUD_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) GPUCLOUD_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("GPUCLOUD_TIMEZONE=" + os.environ.get("GPUCLOUD_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "GPUCLOUD_TIMEZONE=NOT_SET" in result["output"], (
            "GPUCLOUD_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When GPUCLOUD_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("GPUCLOUD_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=gpucloud_tz) which shifted
        absolute time when system-local tz != GPUCLOUD tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_gpucloud_tz(self):
        """Already-aware datetimes should be normalized to GPUCLOUD tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in GPUCLOUD tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of GPUCLOUD tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), GPUCLOUD is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when GPUCLOUD tz
        ...
        # Use a GPUCLOUD timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _gpucloud_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["GPUCLOUD_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_gpucloud_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of GPUCLOUD timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["GPUCLOUD_TIMEZONE"] = "US/Eastern"
        _reset_gpucloud_time_cache()
