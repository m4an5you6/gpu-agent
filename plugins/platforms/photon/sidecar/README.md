...
Small Node helper that bridges GPUCLOUD Agent to Photon's Spectrum SDK
(`spectrum-ts`).  GPUCLOUD is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-GPUCLOUD-Sidecar-Token`)
...
The GPUCLOUD plugin's `gpucloud photon setup` command runs `npm install`
here automatically.
