# ============================================================================
# GPUCLOUD Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$GPUCLOUDHome = $(if ($env:GPUCLOUD_HOME) { $env:GPUCLOUD_HOME } else { "$env:LOCALAPPDATA\gpucloud" }),
    [string]$InstallDir = $(if ($env:GPUCLOUD_HOME) { "$env:GPUCLOUD_HOME\gpucloud-agent" } else { "$env:LOCALAPPDATA\gpucloud\gpucloud-agent" }),
    ...
    # builds apps/desktop into a launchable GPUCLOUD.exe.
    #
    # Why opt-in:
    #   * GPUCLOUD-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched GPUCLOUD.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live GPUCLOUD.exe
    ...
    #     `gpucloud desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:NousResearch/gpucloud-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/gpucloud-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * GPUCLOUD Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $GPUCLOUDHome)) {
        New-Item -ItemType Directory -Force -Path $GPUCLOUDHome | Out-Null
    }
    $envFile = Join-Path $GPUCLOUDHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $GPUCLOUDHome "node"
...
function Install-Uv {
    # GPUCLOUD owns its own uv at $GPUCLOUDHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (gpucloud_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `gpucloud update` stay in sync.
    $managedUv = Join-Path $GPUCLOUDHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $GPUCLOUDHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $GPUCLOUDHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $GPUCLOUDHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $GPUCLOUDHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and GPUCLOUD_HOME wasn't propagated).
...
function Install-Git {
    ...
    which GPUCLOUD uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\gpucloud\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    GPUCLOUD needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the GPUCLOUD copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\gpucloud\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``GPUCLOUD_GIT_BASH_PATH`` (User scope) so GPUCLOUD can find it in a fresh
    ...
    # Download PortableGit into $GPUCLOUDHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $GPUCLOUDHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent GPUCLOUD features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  GPUCLOUD needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``GPUCLOUD_GIT_BASH_PATH`` (User env scope) so GPUCLOUD can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $GPUCLOUDHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $GPUCLOUDHome\git\usr\bin\bash.exe
    $candidates += "$GPUCLOUDHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$GPUCLOUDHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("GPUCLOUD_GIT_BASH_PATH", $candidate, "User")
            $env:GPUCLOUD_GIT_BASH_PATH = $candidate
            Write-Info "Set GPUCLOUD_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- GPUCLOUD may not find Git Bash."
    Write-Info "If needed, set GPUCLOUD_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$GPUCLOUDHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$GPUCLOUDHome\node;$env:Path"
        Write-Success "Node.js $version found (GPUCLOUD-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing GPUCLOUD-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $GPUCLOUDHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $GPUCLOUDHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\gpucloud-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$GPUCLOUDHome\node") { Remove-Item -Recurse -Force "$GPUCLOUDHome\node" }
                Move-Item $extractedDir.FullName "$GPUCLOUDHome\node"
                ...
                $version = & "$GPUCLOUDHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $GPUCLOUDHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\gpucloud-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    $stashName = "gpucloud-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if GPUCLOUD behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running gpucloud processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/gpucloud-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/gpucloud-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/gpucloud-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\gpucloud-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\gpucloud-agent-extract"
    ...
    # next `gpucloud update` checkout aborts on a "dirty" tree the user never
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `gpucloud.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install gpucloud-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and gpucloud.exe broken with
    ...
    # here, not 30 seconds later when the user runs `gpucloud`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `gpucloud dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `gpucloud dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `gpucloud dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up gpucloud command..."
    
    if ($NoVenv) {
        $gpucloudBin = "$InstallDir"
    } else {
        $gpucloudBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so gpucloud is globally available
    # On Windows, the gpucloud.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$gpucloudBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$gpucloudBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $gpucloudBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set GPUCLOUD_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\gpucloud instead
    # of the Unix default ~/.gpucloud
    $currentGPUCLOUDHome = [Environment]::GetEnvironmentVariable("GPUCLOUD_HOME", "User")
    if (-not $currentGPUCLOUDHome -or $currentGPUCLOUDHome -ne $GPUCLOUDHome) {
        [Environment]::SetEnvironmentVariable("GPUCLOUD_HOME", $GPUCLOUDHome, "User")
        Write-Success "Set GPUCLOUD_HOME=$GPUCLOUDHome"
    }
    $env:GPUCLOUD_HOME = $GPUCLOUDHome
    
    # Update current session
    $env:Path = "$gpucloudBin;$env:Path"
    
    Write-Success "gpucloud command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.gpucloud-bootstrap-complete which tells the GPUCLOUD
    ...
    # GPUCLOUD-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `gpucloud update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".gpucloud-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create ~/.gpucloud directory structure
    New-Item -ItemType Directory -Force -Path "$GPUCLOUDHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GPUCLOUDHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GPUCLOUDHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GPUCLOUDHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GPUCLOUDHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GPUCLOUDHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GPUCLOUDHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GPUCLOUDHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GPUCLOUDHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$GPUCLOUDHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.gpucloud/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.gpucloud/.env"
        }
    } else {
        Write-Info "~/.gpucloud/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$GPUCLOUDHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.gpucloud/config.yaml from template"
        }
    } else {
        Write-Info "~/.gpucloud/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and GPUCLOUD's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.gpucloud/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.gpucloud/"
    
    # Seed bundled skills into ~/.gpucloud/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.gpucloud/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.gpucloud/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$GPUCLOUDHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.gpucloud/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (GPUCLOUD-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'gpucloud setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\gpucloud-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\gpucloud-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\gpucloud-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `GPUCLOUD` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable GPUCLOUD.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_gpucloud_desktop command
    # resolves apps/desktop/release/win-unpacked/GPUCLOUD.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the GPUCLOUD-managed
    ...
    # The --dir mode produces an unpacked GPUCLOUD.exe in
    ...
    # are NOT trying to work around). The GPUCLOUD icon + product name are
    # stamped onto GPUCLOUD.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\gpucloud-desktop-build-$(Get-Random).log"
    ...
    try {
        ...
        if ($code -ne 0) {
            ...
            # ENOENT on the final `electron` -> `GPUCLOUD` rename: app-builder's
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\GPUCLOUD.exe",
        "$desktopDir\release\win-arm64-unpacked\GPUCLOUD.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no GPUCLOUD.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The GPUCLOUD icon + identity are stamped onto GPUCLOUD.exe by the
    ...
    #    GPUCLOUD.exe. We deliberately do NOT point them at `gpucloud desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'GPUCLOUD.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'GPUCLOUD.lnk')
        )
        ...
        # with the GPUCLOUD icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.gpucloud/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'gpucloud setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m gpucloud_cli.main setup
    } else {
        python -m gpucloud_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $gpucloudCmd = "$InstallDir\venv\Scripts\gpucloud.exe"
    if (-not (Test-Path $gpucloudCmd)) {
        $gpucloudCmd = "gpucloud"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$GPUCLOUDHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'gpucloud whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: gpucloud gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$GPUCLOUDHome\logs\gateway.log"
            Start-Process -FilePath $gpucloudCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$GPUCLOUDHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: gpucloud gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: gpucloud gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$GPUCLOUDHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$GPUCLOUDHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$GPUCLOUDHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$GPUCLOUDHome\gpucloud-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   gpucloud              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   gpucloud setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   gpucloud config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   gpucloud config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   gpucloud gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   gpucloud update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning GPUCLOUD repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (GPUCLOUD-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding GPUCLOUD to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://gpucloud-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
