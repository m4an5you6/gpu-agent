#!/usr/bin/env python3
"""Build the GPUCLOUD Model Catalog — a centralized JSON manifest of curated models.
...
GPUCLOUD CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a GPUCLOUD release.
...
# Ensure GPUCLOUD_HOME is set for imports that touch it at module level.
os.environ.setdefault("GPUCLOUD_HOME", os.path.join(os.path.expanduser("~"), ".gpucloud"))

from gpucloud_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS  # noqa: E402
...
def build_catalog() -> dict:
    return {
        ...
        "metadata": {
            "source": "gpucloud-agent repo",
            "docs": "https://gpucloud-agent.nousresearch.com/docs/reference/model-catalog",
        },
        "providers": {
            "openrouter": {
                "metadata": {
