 ...
 *
 * Drives apps/desktop's first-launch install of GPUCLOUD Agent by spawning
...
function bootstrapCacheDir(gpucloudHome) {
  return path.join(gpucloudHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.gpucloud/gpucloud-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(gpucloudHome) {
  if (!gpucloudHome) return null
  const candidate = path.join(gpucloudHome, 'gpucloud-agent', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function cachedScriptPath(gpucloudHome, commit) {
  return path.join(bootstrapCacheDir(gpucloudHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NousResearch/gpucloud-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({ installStamp, sourceRepoRoot, gpucloudHome, emit, _download = downloadInstallScript }) {
  ...
  const cached = cachedScriptPath(gpucloudHome, installStamp.commit)
  try {
  ...
  try {
    ...
    const installed = installedAgentInstallScript(gpucloudHome)
    if (installed) {
      emit({
        type: 'log',
        line:
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, gpucloudHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(ps, fullArgs, {
      ...
      env: {
        ...process.env,
        // Pass GPUCLOUD_HOME through so install.ps1 respects the caller's
        // choice rather than re-computing the default.
        GPUCLOUD_HOME: gpucloudHome || process.env.GPUCLOUD_HOME || ''
      }
    })
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, gpucloudHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, gpucloudHome }) {
  const args = ['--dir', activeRoot, '--gpucloud-home', gpucloudHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, gpucloudHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, gpucloudHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, gpucloudHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, gpucloudHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(gpucloudHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, gpucloudHome, emit })
