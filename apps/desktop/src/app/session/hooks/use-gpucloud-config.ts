...
import { getGPUCLOUDConfig, getGPUCLOUDConfigDefaults } from '@/gpucloud'
...
interface GPUCLOUDConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useGPUCLOUDConfig({ activeSessionIdRef, refreshProjectBranch }: GPUCLOUDConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshGPUCLOUDConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getGPUCLOUDConfig(), getGPUCLOUDConfigDefaults().catch(() => ({}))])
  ...
  return { refreshGPUCLOUDConfig, sttEnabled, voiceMaxRecordingSeconds }
}
