...
GPUCLOUD middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing GPUCLOUD'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before GPUCLOUD calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving GPUCLOUD retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving GPUCLOUD guardrails,
...
- `telemetry_schema_version`: currently `gpucloud.observer.v1`
- `middleware_schema_version`: currently `gpucloud.middleware.v1`
...
GPUCLOUD stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, GPUCLOUD runs
them as a nested chain in registration order. Middleware failures are fail-open:
GPUCLOUD logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, GPUCLOUD applies middleware in this order:
...
For each tool call, GPUCLOUD applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal GPUCLOUD pre-execution path against the effective arguments:
...
```bash
gpucloud plugins enable <plugin-name>
```

For isolated local testing, use one `GPUCLOUD_HOME` for plugin enablement and the
agent run:

```bash
export GPUCLOUD_HOME=/tmp/gpucloud-middleware-test
mkdir -p "$GPUCLOUD_HOME"
gpucloud plugins enable <plugin-name>
gpucloud chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run gpucloud plugins enable <plugin-name>
uv run gpucloud chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["gpucloud_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/gpucloud-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape GPUCLOUD expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, GPUCLOUD treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, GPUCLOUD preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. GPUCLOUD does not convert downstream
