...
1. agent identity — `SOUL.md` from `GPUCLOUD_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_hermes_md(cwd_path)       # 1. .gpucloud.md / HERMES.md (walks to git root)
...
|----------|-------|-------------|-------|
| 1 | `.gpucloud.md`, `HERMES.md` | CWD up to git root | GPUCLOUD-native project config |
...
1. `.gpucloud.md` / `HERMES.md` (walks to git root)
...
- Project context files such as `.gpucloud.md`, `HERMES.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
