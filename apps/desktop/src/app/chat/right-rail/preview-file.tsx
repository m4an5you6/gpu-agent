...
import { GPUCLOUD_PATHS_MIME } from '@/app/chat/hooks/use-composer-actions'
...
async function readTextPreview(filePath: string) {
  if (window.gpucloudDesktop.readFileText) {
    try {
      return await window.gpucloudDesktop.readFileText(filePath)
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)

      if (!message.includes("No handler registered for 'gpucloud:readFileText'")) {
        throw error
      }
    }
  }
  ...
  const dataUrl = await window.gpucloudDesktop.readFileDataUrl(filePath)
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(GPUCLOUD_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
