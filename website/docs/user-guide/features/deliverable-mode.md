...
When GPUCLOUD Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.gpucloud/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.gpucloud/config.yaml` (switchable per session
via `/personality`).
...
If you use GPUCLOUD' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.gpucloud/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. GPUCLOUD Agent's deliverable
