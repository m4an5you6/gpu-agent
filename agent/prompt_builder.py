...
from gpucloud_constants import get_gpucloud_home, get_skills_dir, is_wsl
...
_GPUCLOUD_MD_NAMES = (".gpucloud.md", "GPUCLOUD.md")


def _find_gpucloud_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.gpucloud.md`` or ``GPUCLOUD.md``.
    ...
    for directory in [current, *current.parents]:
        for name in _GPUCLOUD_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are GPUCLOUD Agent, an intelligent AI assistant created by Nous Research. "
...
GPUCLOUD_AGENT_HELP_GUIDANCE = (
    "You run on GPUCLOUD Agent (by Nous Research). When the user needs help with "
    "GPUCLOUD itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://gpucloud-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `gpucloud-agent` skill with skill_view(name='gpucloud-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.gpucloud/kanban.db`. Your task id is in "
    "`$GPUCLOUD_KANBAN_TASK`; your workspace is `$GPUCLOUD_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $GPUCLOUD_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `gpucloud kanban <verb>` for board operations. Use "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that GPUCLOUD "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the GPUCLOUD WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where GPUCLOUD itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host GPUCLOUD runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where GPUCLOUD itself is running. The host OS, home, and cwd "
                f"of the GPUCLOUD process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where GPUCLOUD "
    ...
    # Embedder-supplied environment description. Lets a host that wraps GPUCLOUD
    ...
    extra = (os.getenv("GPUCLOUD_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_gpucloud_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.gpucloud/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("GPUCLOUD_PLATFORM")
        or get_session_env("GPUCLOUD_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot GPUCLOUD Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `gpucloud-agent` skill "
            "first. It has the actual commands (e.g. `gpucloud config set …`, `gpucloud tools`, "
            "`gpucloud setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: gpucloud setup, gpucloud setup tools, gpucloud setup terminal, gpucloud status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from GPUCLOUD_HOME and return its content, or None.
    ...
    """
    try:
        from gpucloud_cli.config import ensure_gpucloud_home
        ensure_gpucloud_home()
    except Exception as e:
        logger.debug("Could not ensure GPUCLOUD_HOME before loading SOUL.md: %s", e)

    soul_path = get_gpucloud_home() / "SOUL.md"
...
def _load_gpucloud_md(cwd_path: Path) -> str:
    """.gpucloud.md / GPUCLOUD.md — walk to git root."""
    gpucloud_md_path = _find_gpucloud_md(cwd_path)
    if not gpucloud_md_path:
        return ""
    try:
        content = gpucloud_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = gpucloud_md_path.name
        try:
            rel = str(gpucloud_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".gpucloud.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", gpucloud_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .gpucloud.md / GPUCLOUD.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from GPUCLOUD_HOME is independent and always included when present.
    ...
    # SOUL.md from GPUCLOUD_HOME only — skip when already loaded as identity
    if not skip_soul:
