...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/gpucloud>
...
```bash
gpucloud memory setup honcho   # configure Honcho directly (works on a fresh install)
gpucloud memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
gpucloud config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.gpucloud/.env
```

> `gpucloud honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `gpucloud memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$GPUCLOUD_HOME/honcho.json` | Profile-local (isolated GPUCLOUD instances) |
| 2 | `~/.gpucloud/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active GPUCLOUD profile: `gpucloud` (default) or `gpucloud_<profile>`.
...
**Deployment shapes** (`gpucloud memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect GPUCLOUD to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | GPUCLOUD session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`gpucloud-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`gpucloud`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-gpucloud-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening gpucloud in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — GPUCLOUD session ID (timestamp + hex). Every `gpucloud` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple GPUCLOUD profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`gpucloud`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active GPUCLOUD profile: `gpucloud` (default) or `gpucloud_<profile>` (e.g. `gpucloud -p coder` -> host key `gpucloud_coder`). Older `gpucloud.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `GPUCLOUD_HONCHO_HOST` | Host key override |
...
|---------|-------------|
| `gpucloud memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `gpucloud honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `gpucloud memory setup`) |
| `gpucloud honcho status` | Show resolved config for active profile |
| `gpucloud honcho enable` / `disable` | Toggle Honcho for active profile |
| `gpucloud honcho mode <mode>` | Change recall or observation mode |
| `gpucloud honcho peer --user <name>` | Update user peer name |
| `gpucloud honcho peer --ai <name>` | Update AI peer name |
| `gpucloud honcho tokens --context <N>` | Set context token budget |
| `gpucloud honcho tokens --dialectic <N>` | Set dialectic max chars |
| `gpucloud honcho map <name>` | Map current directory to a session name |
| `gpucloud honcho sync` | Create host blocks for all GPUCLOUD profiles |
