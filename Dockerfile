...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/gpucloud/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when gpucloud
...
RUN set -eu; \
    ...
    # 'GPUCLOUD WebUI' catalog). The image has moved to s6-overlay /init\
...
RUN useradd -u 10000 -m -d /opt/data gpucloud
...
# for the same reason: it lazy-installs into /opt/gpucloud/.venv at first
...
# gpucloud_cli/main.py succeeds (see #18800). /opt/gpucloud/web is build-time
# only (GPUCLOUD_WEB_DIST points at gpucloud_cli/web_dist) and is intentionally
# not chowned here.
# /opt/gpucloud/gateway is runtime-writable: Python may create __pycache__ and
# gateway state artifacts beneath the package after services drop privileges,
# especially when the gpucloud UID is remapped at boot (#27221).
# The .venv MUST remain gpucloud-writable so lazy_deps.py can install
# remaining optional platform packages and future pin bumps at first use.
# Without this, `uv pip install` fails with EACCES and adapters silently
# fail to load.  See tools/lazy_deps.py.
USER root
RUN chmod -R a+rX /opt/gpucloud && \
    chown -R gpucloud:gpucloud /opt/gpucloud/.venv /opt/gpucloud/ui-tui /opt/gpucloud/gateway /opt/gpucloud/node_modules
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the gpucloud user via
# `s6-setuidgid gpucloud` in its run script. If GPUCLOUD_UID is unset, services
...
# /opt/gpucloud/.gpucloud_build_sha at build time, and have
...
RUN if [ -n "${GPUCLOUD_GIT_SHA}" ]; then \
        printf '%s\n' "${GPUCLOUD_GIT_SHA}" > /opt/gpucloud/.gpucloud_build_sha && \
        chown gpucloud:gpucloud /opt/gpucloud/.gpucloud_build_sha; \
    fi
...
# `exec gpucloud`. Wired in as cont-init.d/01- so it
...
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/gpucloud/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-gpucloud-setup && \
    chmod +x /etc/cont-init.d/01-gpucloud-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV GPUCLOUD_WEB_DIST=/opt/gpucloud/gpucloud_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/gpucloud/ui-tui/dist/entry.js`) and skip the
...
ENV GPUCLOUD_TUI_DIR=/opt/gpucloud/ui-tui
...
# The shim lives at /opt/gpucloud/bin/gpucloud, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid gpucloud`
# when invoked as root. Non-root callers (supervised processes,
# `--user gpucloud`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/gpucloud/.venv/bin/gpucloud). See the shim source for
# the opt-out env var (GPUCLOUD_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/gpucloud-exec-shim.sh /opt/gpucloud/bin/gpucloud
...
# subprocess that doesn't activate the venv first still find gpucloud.
#
# /opt/gpucloud/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/gpucloud/bin:/opt/gpucloud/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. gpucloud
# subcommand vs. no-args), drops to the gpucloud user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/gpucloud/docker/main-wrapper.sh" ]
CMD [ ]
