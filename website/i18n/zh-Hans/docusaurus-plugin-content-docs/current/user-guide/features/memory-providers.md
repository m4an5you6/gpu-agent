...
**配置：** `$GPUCLOUD_HOME/honcho.json`（profile 本地）或 `~/.honcho/config.json`（全局）。解析顺序：`$GPUCLOUD_HOME/honcho.json` > `~/.gpucloud/honcho.json` > `~/.honcho/config.json`。参见[配置参考](https://github.com/gpucloud-ai/gpucloud-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/hermes)。
...
| **AI peer**（`aiPeer`） | 每个 GPUCLOUD profile 一个。host key `gpucloud` → 默认；其他 profile 使用 `hermes.<profile>`。 |
...
参见[配置参考](https://github.com/gpucloud-ai/gpucloud-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/hermes)。
...
- **Profile 范围容器**——在 `container_tag` 中使用 `{identity}`（例如 `hermes-{identity}` → `hermes-coder`），按 GPUCLOUD profile 隔离记忆
