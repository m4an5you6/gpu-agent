...
| | |
|---|---|
| Source | Optional — install with `gpucloud skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into GPUCLOUD or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to GPUCLOUD. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.gpucloud/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.gpucloud/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is GPUCLOUD integration, either:

- configure the server in `~/.gpucloud/config.yaml` using the `native-mcp` skill, or
...
### GPUCLOUD cannot see the deployed server

The server-building part may be correct while the GPUCLOUD config is not. Load the `native-mcp` skill and configure the server in `~/.gpucloud/config.yaml`, then restart GPUCLOUD.
