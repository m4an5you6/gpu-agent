...
```bash
gpucloud honcho setup    # full interactive wizard (cloud or local)
gpucloud memory setup    # generic picker, also works
```
...
Host key is derived from the active GPUCLOUD profile: `gpucloud` (default) or `hermes.<profile>`.
...
**Deployment shapes** (`gpucloud honcho setup` asks one prompt to set these):
...
Host key is derived from the active GPUCLOUD profile: `gpucloud` (default) or `hermes.<profile>` (e.g. `gpucloud -p coder` → host key `hermes.coder`).
...
| Command | Description |
|---------|-------------|
| `gpucloud honcho setup` | Full interactive setup wizard |
