#!/bin/sh
# shellcheck shell=sh
# /opt/hermes/bin/gpucloud — `docker exec` privilege-drop shim.
#
# Background
# ----------
...
# Fix
# ---
# This shim sits at /opt/hermes/bin/gpucloud and is placed earliest on PATH.
...
# (/opt/hermes/.venv/bin/gpucloud), so the second hop cannot re-enter this
...
REAL=/opt/hermes/.venv/bin/gpucloud
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "hermes-shim: re-run with --user gpucloud or set GPUCLOUD_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi
