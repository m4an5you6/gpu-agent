...
Override the image with ``GPUCLOUD_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("GPUCLOUD_TEST_IMAGE", "gpucloud-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``GPUCLOUD_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("GPUCLOUD_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged gpucloud user
# ---------------------------------------------------------------------------
#
# Background: every GPUCLOUD runtime path inside the container drops to UID
# 10000 (the ``gpucloud`` user) via ``s6-setuidgid gpucloud``. ``docker exec``
...
# fails (PermissionError swallowed) for gpucloud, so a test that ran as root
...
# helpers below run every probe under ``-u gpucloud`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: gpucloud).
