...
from gpucloud_constants import get_gpucloud_dir, get_gpucloud_home
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The GPUCLOUD home itself contains credentials (auth.json, .env) — only the
    # cache subdirectories under it are explicitly allowlisted above.
    denied.append(_GPUCLOUD_HOME / ".env")
    denied.append(_GPUCLOUD_HOME / "auth.json")
    denied.append(_GPUCLOUD_HOME / "credentials")
    return denied
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("GPUCLOUD_GATEWAY_BUSY_TEXT_MODE", "queue").strip().lower()
            or "queue"
        )
