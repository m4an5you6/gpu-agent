...
- Legacy session key (fallback): mint an opaque 24h key when JWT auth is
  unavailable, or when GPUCLOUD_AGENT_USE_LEGACY_SESSION_KEYS is set for
...
NOUS_LEGACY_SESSION_KEYS_ENV = "GPUCLOUD_AGENT_USE_LEGACY_SESSION_KEYS"
...
def _sync_codex_pool_entries(
    ...
    * ``manual:device_code`` — entries created by ``gpucloud auth add openai-codex``
      ...
      ``gpucloud auth add`` workaround for #33000 would silently leave that
