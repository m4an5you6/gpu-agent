...
Connect GPUCLOUD to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
GPUCLOUD supports two WeCom integration modes:
...
> Run `gpucloud gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. GPUCLOUD decrypts the message, queues it for the agent
...
(Use `gpucloud gateway start` only after `gpucloud gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in GPUCLOUD and the token the
...
in `~/.gpucloud/.env` values around `=` will also break signature checks. After
fixing, restart `gpucloud gateway run`.
...
Check `gpucloud gateway run` logs for the bound host/port. If the adapter bound to
