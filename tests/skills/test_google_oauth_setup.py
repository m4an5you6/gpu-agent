...
class TestGPUCLOUDConstantsFallback:
    """Tests for _gpucloud_home.py fallback when gpucloud_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_gpucloud_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _gpucloud_home.py with gpucloud_constants blocked."""
        monkeypatch.setitem(sys.modules, "gpucloud_constants", None)
        spec = importlib.util.spec_from_file_location("_gpucloud_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_gpucloud_home_env_var(self, monkeypatch, tmp_path):
        """When gpucloud_constants is missing, GPUCLOUD_HOME comes from env var."""
        monkeypatch.setenv("GPUCLOUD_HOME", str(tmp_path / "custom-gpucloud"))
        module = self._load_helper(monkeypatch)
        assert module.get_gpucloud_home() == tmp_path / "custom-gpucloud"

    def test_fallback_defaults_to_dot_gpucloud(self, monkeypatch):
        """When gpucloud_constants is missing and GPUCLOUD_HOME unset, default to ~/.gpucloud."""
        monkeypatch.delenv("GPUCLOUD_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_gpucloud_home() == Path.home() / ".gpucloud"

    def test_fallback_ignores_empty_gpucloud_home(self, monkeypatch):
        """Empty/whitespace GPUCLOUD_HOME is treated as unset."""
        monkeypatch.setenv("GPUCLOUD_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_gpucloud_home() == Path.home() / ".gpucloud"

    def test_fallback_display_gpucloud_home_shortens_path(self, monkeypatch):
        """Fallback display_gpucloud_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("GPUCLOUD_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_gpucloud_home() == "~/.gpucloud"

    def test_fallback_display_gpucloud_home_profile_path(self, monkeypatch):
        """Fallback display_gpucloud_home() handles profile paths under ~/."""
        monkeypatch.setenv("GPUCLOUD_HOME", str(Path.home() / ".gpucloud/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_gpucloud_home() == "~/.gpucloud/profiles/coder"

    def test_fallback_display_gpucloud_home_custom_path(self, monkeypatch):
        """Fallback display_gpucloud_home() returns full path for non-home locations."""
        monkeypatch.setenv("GPUCLOUD_HOME", "/opt/gpucloud-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_gpucloud_home() == "/opt/gpucloud-custom"

    def test_delegates_to_gpucloud_constants_when_available(self):
        """When gpucloud_constants IS importable, _gpucloud_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_gpucloud_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import gpucloud_constants
        assert module.get_gpucloud_home is gpucloud_constants.get_gpucloud_home
        assert module.display_gpucloud_home is gpucloud_constants.display_gpucloud_home
...
class TestInstallDeps:
    ...
    Regression coverage for the GPUCLOUD Docker image, whose venv is built with
    ...
    def test_returns_false_when_no_pip_and_no_uv(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "gpucloud-agent[google]" in out
