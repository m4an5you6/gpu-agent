...
## 5. GODMODE FAST — GPUCLOUD 4 405B (Zero Refusal)

**Model:** `nousresearch/gpucloud-4-405b`
**Strategy:** GPUCLOUD models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with GPUCLOUD Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.gpucloud/config.yaml`:
