    ...
    try:
        ...
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                ...
                logger.warning(
                    "credits header version %d unsupported, ignoring — update GPUCLOUD",
                    version_val,
                )
            return None
...
# ── Dev test fixtures (GPUCLOUD_DEV_CREDITS_FIXTURE) ───────────────────────────
# Throwaway dev scaffolding: trigger any notice state on demand for testing,
# without real spend or Redis seeding. Set GPUCLOUD_DEV_CREDITS_FIXTURE to either a
...
# GPUCLOUD_DEV_CREDITS scaffolding.
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Return a fixture CreditsState for GPUCLOUD_DEV_CREDITS_FIXTURE, or None.
    ...
    Hard prod-leak guard: a fixture applies ONLY when the dev flag GPUCLOUD_DEV_CREDITS
    is also on, so a stray GPUCLOUD_DEV_CREDITS_FIXTURE (leaked into a shell profile, a
    container env, a launch plist, …) can never surface fabricated balances/notices
    on a real account.
    """
    if not is_truthy_value(os.environ.get("GPUCLOUD_DEV_CREDITS")):
        return None
    raw = os.environ.get("GPUCLOUD_DEV_CREDITS_FIXTURE", "").strip()
    if not raw:
        return None
    name = raw
