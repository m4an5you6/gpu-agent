...
def test_root_session_no_compression(db):
    ...
    assert prov["currentGPUCLOUDSessionId"] == "root1"
    assert prov["rootGPUCLOUDSessionId"] == "root1"
    assert prov["parentGPUCLOUDSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_gpucloud_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentGPUCLOUDSessionId"] == "old"
    assert prov["rootGPUCLOUDSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousGPUCLOUDSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootGPUCLOUDSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootGPUCLOUDSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_gpucloud_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousGPUCLOUDSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"gpucloud"}
    assert "sessionProvenance" in meta["gpucloud"]
    assert meta["gpucloud"]["sessionProvenance"]["currentGPUCLOUDSessionId"] == "root1"
