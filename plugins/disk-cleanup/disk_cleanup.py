"""disk_cleanup — ephemeral file cleanup for GPUCLOUD Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under GPUCLOUD_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly GPUCLOUD_HOME and /tmp/gpucloud-*
Never touches: ~/.gpucloud/logs/ or any system directory.
"""
...
try:
    ...
    def get_gpucloud_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("GPUCLOUD_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".gpucloud").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$GPUCLOUD_HOME/logs/``."""
    return get_gpucloud_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$GPUCLOUD_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under GPUCLOUD_HOME or ``/tmp/gpucloud-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    gpucloud_home = get_gpucloud_home()
    try:
        path.resolve().relative_to(gpucloud_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/gpucloud-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("gpucloud-"):
        return True
    return False
...
# Paths under $GPUCLOUD_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so GPUCLOUD_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        gpucloud_home = get_gpucloud_home()
        for parent in ("cron", "cronjobs"):
            base = gpucloud_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside GPUCLOUD_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under GPUCLOUD_HOME (but leave GPUCLOUD_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "gpucloud-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(gpucloud_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == gpucloud_home:
                continue
            try:
                rel_parts = dirpath.relative_to(gpucloud_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    gpucloud_home = get_gpucloud_home()
    try:
        rel = path.resolve().relative_to(gpucloud_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "gpucloud-agent",
        }:
            return None
        ...
        # Path isn't under GPUCLOUD_HOME (e.g. /tmp/gpucloud-*) — fall through.
        pass
