...
const { canImportGPUCLOUDCli, verifyGPUCLOUDCli } = require('./backend-probes.cjs')
...
const USER_DATA_OVERRIDE = process.env.GPUCLOUD_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.GPUCLOUD_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with GPUCLOUD_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[gpucloud] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[gpucloud] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[gpucloud] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[gpucloud] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// GPUCLOUD_HOME — the user-facing root for everything GPUCLOUD-related. Mirrors
// scripts/install.ps1's $GPUCLOUDHome and scripts/install.sh's $GPUCLOUD_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\gpucloud (matches install.ps1)
//   macOS / Linux: ~/.gpucloud (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.gpucloud directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\gpucloud yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// GPUCLOUD_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// GPUCLOUD_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.gpucloud / %LOCALAPPDATA%\gpucloud.
function resolveGPUCLOUDHome() {
  if (process.env.GPUCLOUD_HOME) return path.resolve(process.env.GPUCLOUD_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'gpucloud-home')
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'gpucloud')
    const legacy = path.join(app.getPath('home'), '.gpucloud')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.gpucloud setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.gpucloud')
}

const GPUCLOUD_HOME = resolveGPUCLOUDHome()
// ACTIVE_GPUCLOUD_ROOT — the canonical mutable GPUCLOUD install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_GPUCLOUD_ROOT = path.join(GPUCLOUD_HOME, 'gpucloud-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_GPUCLOUD_ROOT, 'venv')
...
// boots, going straight to `resolveGPUCLOUDBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_GPUCLOUD_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_GPUCLOUD_ROOT, '.gpucloud-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
// active-profile.json records which GPUCLOUD profile the desktop launches its
// local backend as. When set, startGPUCLOUD() passes `gpucloud --profile <name>
// dashboard …`, which deterministically pins GPUCLOUD_HOME (see
// _apply_profile_override in gpucloud_cli/main.py) and bypasses the sticky
// ~/.gpucloud/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors gpucloud_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// gpucloudDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under GPUCLOUD_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by gpucloud_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(GPUCLOUD_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (gpucloud_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.GPUCLOUD_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.GPUCLOUD_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
  if (!Number.isFinite(raw) || raw <= 0) return 650
  return Math.max(120, raw)
})()
const APP_NAME = 'GPUCLOUD'
...
// Seed the native About panel with the live GPUCLOUD version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `gpucloud update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveGPUCLOUDVersion(),
...
const MEDIA_PROTOCOL = 'gpucloud-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(gpucloud-media://…)`.
...
let mainWindow = null
let gpucloudProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by gpucloudProcess +
// connectionPromise + startGPUCLOUD(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.GPUCLOUD_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.GPUCLOUD_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startGPUCLOUD() calls (e.g. the renderer's
...
let bootProgressState = {
  ...
  message: 'Waiting to start GPUCLOUD backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (!text) return
  const lines = text.split(/\r?\n/).map(line => `[gpucloud] ${line}`)
  gpucloudLog.push(...lines)
  if (gpucloudLog.length > 300) {
    gpucloudLog.splice(0, gpucloudLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-gpucloud-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('gpucloud:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the gpucloud:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('gpucloud:bootstrap:event', ev)
}
...
function isGPUCLOUDSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'gpucloud_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.GPUCLOUD_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several GPUCLOUD deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. GPUCLOUD' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\gpucloud\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'gpucloud', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'gpucloud', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// install only has PortableGit under %LOCALAPPDATA%\gpucloud\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'gpucloud', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'gpucloud', 'git', 'bin', 'git.exe'))
  }
...
function recentGPUCLOUDLog() {
  return gpucloudLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's gpucloud root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_GPUCLOUD_ROOT.
// GPUCLOUD_DESKTOP_GPUCLOUD_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.GPUCLOUD_DESKTOP_GPUCLOUD_ROOT && path.resolve(process.env.GPUCLOUD_DESKTOP_GPUCLOUD_ROOT),
    !IS_PACKAGED && isGPUCLOUDSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isGPUCLOUDSourceRoot(ACTIVE_GPUCLOUD_ROOT) ? ACTIVE_GPUCLOUD_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_GPUCLOUD_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('gpucloud:updates:progress', merged)
  }
}
...
// GPUCLOUD_HOME/gpucloud-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_gpucloud_home). That binary owns
// ALL repo mutation — running `gpucloud update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'gpucloud-setup.exe' : 'gpucloud-setup'
  const candidate = path.join(GPUCLOUD_HOME, name)
...
// Path to the venv shim whose lock decides whether `gpucloud update` can write
// fresh entry points. On Windows this is the file the running backend
// `gpucloud.exe` holds open; on POSIX it's never mandatory-locked.
function venvGPUCLOUDShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'gpucloud.exe')
    : path.join(updateRoot, 'venv', 'bin', 'gpucloud')
}
...
// only signals the direct child, so on Windows a backend `gpucloud.exe` that
// spawned its own grandchildren (a `gpucloud` REPL, a pty terminal session, the
...
// `gpucloudProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `gpucloud.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a gpucloud
// REPL, a pty terminal, the gateway) keep `gpucloud.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (gpucloudProcess && Number.isInteger(gpucloudProcess.pid)) pids.push(gpucloudProcess.pid)
  ...
  if (gpucloudProcess && !gpucloudProcess.killed) {
    try {
      gpucloudProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvGPUCLOUDShimPath(updateRoot)
...
// `gpucloud update`). Instead we spawn the staged GPUCLOUD-Setup binary with
// --update and quit, so it can run `gpucloud update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri gpucloud-setup. Unlike Windows
      ...
      // whole update itself: `gpucloud update` (backend) + `gpucloud desktop
    ...
    if (!updater) {
      ...
      // `gpucloud desktop`, never the Tauri installer that self-copies
      // gpucloud-setup.exe into GPUCLOUD_HOME). They DO have a working `gpucloud`
      ...
      // checkout they're on — bare `gpucloud update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'gpucloud update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `gpucloud update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `gpucloud update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, gpucloudRoot: updateRoot }
    }

    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the GPUCLOUD updater…', percent: 100 })
    ...
    // gpucloud.exe (held by the backend child / its grandchildren) and the update
    // bricks. See releaseBackendLockForUpdate for the full failure analysis.
    await releaseBackendLockForUpdate(updateRoot)

    // Detached so the updater outlives this process — it needs us GONE before
    // `gpucloud update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: GPUCLOUD_HOME,
      env: {
        ...process.env,
        GPUCLOUD_HOME,
        PATH: [path.join(GPUCLOUD_HOME, 'node', 'bin'), venvBin, process.env.PATH].filter(Boolean).join(path.delimiter)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
// Resolve the gpucloud CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `gpucloud` on PATH.
function resolveGPUCLOUDCliBinary(updateRoot) {
  const venvGPUCLOUD = path.join(updateRoot, 'venv', 'bin', 'gpucloud')
  if (fileExists(venvGPUCLOUD)) return venvGPUCLOUD
  return findOnPath('gpucloud') || null
}
...
// macOS/Linux in-app update: backend (`gpucloud update`) + OS-aware GUI rebuild
// (`gpucloud desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const gpucloud = resolveGPUCLOUDCliBinary(updateRoot)
  if (!gpucloud) {
    emitUpdateProgress({ stage: 'manual', message: 'gpucloud update', percent: null })
    return { ok: true, manual: true, command: 'gpucloud update', gpucloudRoot: updateRoot }
  }

  // Put the GPUCLOUD-managed Node and the venv on PATH so `gpucloud desktop`'s
  // npm build can find them on a machine with no system Node.
  const extraPath = [path.join(GPUCLOUD_HOME, 'node', 'bin'), path.join(updateRoot, 'venv', 'bin')]
    .filter(Boolean)
    .join(path.delimiter)
  const env = {
    GPUCLOUD_HOME,
  ...
  // `gpucloud update` reaps stale `gpucloud dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads GPUCLOUD_DESKTOP_CHILD_PID and excludes
  ...
  if (gpucloudProcess && Number.isInteger(gpucloudProcess.pid)) {
    desktopChildPids.push(gpucloudProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.GPUCLOUD_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating GPUCLOUD (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(gpucloud, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'gpucloud update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'gpucloud update failed' }
  }

  emitUpdateProgress({ stage: 'rebuild', message: 'Rebuilding the desktop app…', percent: 60 })
  const rebuilt = await runStreamedUpdate(gpucloud, ['desktop', '--build-only'], {
    cwd: updateRoot,
    env,
    stage: 'rebuild'
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart GPUCLOUD to retry.',
  ...
  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'GPUCLOUD.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'GPUCLOUD.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart GPUCLOUD to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.gpucloud-update-new"; then
    rm -rf "$DST.gpucloud-update-old" 2>/dev/null || true
    mv "$DST" "$DST.gpucloud-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.gpucloud-update-new" "$DST"
    rm -rf "$DST.gpucloud-update-old" 2>/dev/null || true
  fi
fi
...
/usr/bin/open "$DST"
`
  const scriptPath = path.join(app.getPath('temp'), `gpucloud-desktop-update-${Date.now()}.sh`)
  try {
    fs.writeFileSync(scriptPath, swapScript, { mode: 0o755 })
  } catch (err) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart GPUCLOUD to load the new version.',
      percent: 100
    })
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `gpucloud
  ...
  return isGPUCLOUDSourceRoot(ACTIVE_GPUCLOUD_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function resolveWebDist() {
  const override = process.env.GPUCLOUD_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set GPUCLOUD_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: gpucloud desktop --force-build`
  )
  return candidates[0]
}

function resolveGPUCLOUDCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/GPUCLOUD.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.GPUCLOUD_DESKTOP_CWD,
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'gpucloud_cli.main', ...dashboardArgs],
    env: {
...
// createActiveBackend — build a backend pointing at ACTIVE_GPUCLOUD_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    kind: 'python',
    label: `GPUCLOUD at ${ACTIVE_GPUCLOUD_ROOT}`,
    command: fileExists(venvPython) ? venvPython : findSystemPython(),
    args: ['-m', 'gpucloud_cli.main', ...dashboardArgs],
    env: {
      PYTHONPATH: [ACTIVE_GPUCLOUD_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
    },
    root: ACTIVE_GPUCLOUD_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveGPUCLOUDBackend(dashboardArgs) {
  // 1. Explicit override -- GPUCLOUD_DESKTOP_GPUCLOUD_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.GPUCLOUD_DESKTOP_GPUCLOUD_ROOT && path.resolve(process.env.GPUCLOUD_DESKTOP_GPUCLOUD_ROOT)
  if (overrideRoot && isGPUCLOUDSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `GPUCLOUD source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `gpucloud` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isGPUCLOUDSourceRoot.)
  if (!IS_PACKAGED && isGPUCLOUDSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `GPUCLOUD source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_GPUCLOUD_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\gpucloud\gpucloud-agent (Windows) or ~/.gpucloud/gpucloud-agent.
  ...
  //    to spawning gpucloud. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `gpucloud update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 4. Existing `gpucloud` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    GPUCLOUD_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.GPUCLOUD_DESKTOP_IGNORE_EXISTING !== '1') {
    let gpucloudCommand = null
    const gpucloudOverride = process.env.GPUCLOUD_DESKTOP_GPUCLOUD

    if (gpucloudOverride) {
      const resolvedOverride = findOnPath(gpucloudOverride)
      if (resolvedOverride) {
        gpucloudCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(gpucloudOverride, { isWsl: IS_WSL })) {
        gpucloudCommand = gpucloudOverride
      } else {
        rememberLog(`Ignoring Windows GPUCLOUD override under WSL: ${gpucloudOverride}`)
      }
    } else {
      gpucloudCommand = findOnPath('gpucloud')
    }

    if (gpucloudCommand) {
      if (looksLikeDesktopAppBinary(gpucloudCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving GPUCLOUD CLI: ${gpucloudCommand}`)
        gpucloudCommand = null
      }
    }

    if (gpucloudCommand) {
      // Smoke-test the candidate before trusting it. A `gpucloud` shim
      ...
      const shellForProbe = isCommandScript(gpucloudCommand)
      if (verifyGPUCLOUDCli(gpucloudCommand, { shell: shellForProbe })) {
        return {
          label: `existing GPUCLOUD CLI at ${gpucloudCommand}`,
          command: gpucloudCommand,
          args: dashboardArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing GPUCLOUD CLI at ${gpucloudCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed gpucloud_cli module via system Python.
  ...
  if (python) {
    ...
    // having gpucloud_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\gpucloud\gpucloud-agent\venv.
    if (canImportGPUCLOUDCli(python)) {
      return {
        kind: 'python',
        label: `installed gpucloud_cli module via ${python}`,
        command: python,
        args: ['-m', 'gpucloud_cli.main', ...dashboardArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: gpucloud_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveGPUCLOUDBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'GPUCLOUD Agent not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_GPUCLOUD_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveGPUCLOUDBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no GPUCLOUD install found; starting first-launch bootstrap')
    ...
    const bootstrapResult = await runBootstrap({
      ...
      gpucloudHome: GPUCLOUD_HOME,
      logRoot: path.join(GPUCLOUD_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('GPUCLOUD install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `GPUCLOUD bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(GPUCLOUD_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startGPUCLOUD() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // gpucloud:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveGPUCLOUDBackend(backend.args))
  }
  ...
  if (!isGPUCLOUDSourceRoot(ACTIVE_GPUCLOUD_ROOT)) {
    throw new Error(
      `GPUCLOUD install at ${ACTIVE_GPUCLOUD_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. GPUCLOUD' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\gpucloud\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `gpucloud` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for GPUCLOUD on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch GPUCLOUD.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isGPUCLOUDSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `GPUCLOUD venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = venvPython
  backend.label = `GPUCLOUD at ${ACTIVE_GPUCLOUD_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'GPUCLOUD runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported GPUCLOUD backend URL protocol: ${parsed.protocol}`))
      return
    }
    ...
    const req = client.request(
      parsed,
      {
        ...
        headers: {
          ...
          'X-GPUCLOUD-Session-Token': token,
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the GPUCLOUD backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to GPUCLOUD backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-GPUCLOUD-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('gpucloud:link-titles', { cache: false })
...
function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveGPUCLOUDCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('gpucloud:preview-file-changed', payload)
}
...
async function waitForGPUCLOUD(baseUrl, token) {
  ...
  throw new Error(`GPUCLOUD backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('gpucloud:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('gpucloud:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting GPUCLOUD...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('gpucloud:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('gpucloud:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('gpucloud:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'gpucloud:desktop:zoomLevel'
...
//
// Hosted GPUCLOUD gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``gpucloud_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / gpucloud #37247).
//     Both are set as HttpOnly cookies (``gpucloud_session_at`` ~15 min,
//     ``gpucloud_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:gpucloud-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to GPUCLOUD gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to GPUCLOUD backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root GPUCLOUD_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.GPUCLOUD_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.GPUCLOUD_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by GPUCLOUD_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (gpucloud #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote GPUCLOUD gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote GPUCLOUD gateway is selected, but no session token is saved. ' +
...
//   2. env override (GPUCLOUD_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.GPUCLOUD_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.GPUCLOUD_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'GPUCLOUD_DESKTOP_REMOTE_URL is set but GPUCLOUD_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote GPUCLOUD backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.GPUCLOUD_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every GPUCLOUD
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startGPUCLOUD())
  ...
  // connect to GPUCLOUD gateway". Mirror the renderer's connect here so the test
...
function resetGPUCLOUDConnection() {
...
// startGPUCLOUD() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetGPUCLOUDConnection() nulls gpucloudProcess.
  const dying = gpucloudProcess && !gpucloudProcess.killed ? gpucloudProcess : null
  resetGPUCLOUDConnection()
...
// profile to startGPUCLOUD() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startGPUCLOUD() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForGPUCLOUD(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: gpucloudLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  // --profile wins over the inherited GPUCLOUD_HOME env (see _apply_profile_override
  // step 3 in gpucloud_cli/main.py), so the child re-homes to this profile.
  const dashboardArgs = ['--profile', profile, 'dashboard', '--no-open', '--host', '127.0.0.1', '--port', String(port)]
  const backend = await ensureRuntime(resolveGPUCLOUDBackend(dashboardArgs))
  const gpucloudCwd = resolveGPUCLOUDCwd()
  const webDist = resolveWebDist()

  rememberLog(`Starting GPUCLOUD backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(backend.command, backend.args, {
    ...
    env: {
      ...
      GPUCLOUD_HOME,
      ...backend.env,
      GPUCLOUD_DASHBOARD_SESSION_TOKEN: token,
      // Marks this dashboard backend as desktop-spawned so it runs the cron
      // scheduler tick loop (the gateway isn't running under the app).
      GPUCLOUD_DESKTOP: '1',
      GPUCLOUD_WEB_DIST: webDist
    },
    shell: backend.shell,
    stdio: ['ignore', 'pipe', 'pipe']
  })
  ...
  child.once('error', error => {
    rememberLog(`GPUCLOUD backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`GPUCLOUD backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(new Error(`GPUCLOUD backend for profile "${profile}" exited before it became ready (${signal || code}).`))
    }
  })

  const baseUrl = `http://127.0.0.1:${port}`
  await Promise.race([waitForGPUCLOUD(baseUrl, token), startFailed])
  ready = true
...
async function startGPUCLOUD() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startGPUCLOUD() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving GPUCLOUD backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote GPUCLOUD backend at ${remote.baseUrl}`, 24)
      await waitForGPUCLOUD(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote GPUCLOUD backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.gpucloud/active_profile file) and
    // resolves GPUCLOUD_HOME the same way `gpucloud -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving GPUCLOUD runtime', 28)
    const backend = await ensureRuntime(resolveGPUCLOUDBackend(dashboardArgs))
    const gpucloudCwd = resolveGPUCLOUDCwd()
    const webDist = resolveWebDist()

    await advanceBootProgress('backend.spawn', `Starting GPUCLOUD backend via ${backend.label}`, 84)
    rememberLog(`Starting GPUCLOUD backend via ${backend.label}`)

    gpucloudProcess = spawn(backend.command, backend.args, {
      cwd: gpucloudCwd,
      env: {
        ...process.env,
        // Explicitly pin GPUCLOUD_HOME for the child so Python's get_gpucloud_home()
        // resolves to the SAME location our resolveGPUCLOUDHome() picked. Without
        // this pin, Python falls back to ~/.gpucloud on every platform — fine on
        // mac/linux (where our default matches), but on Windows our default is
        // %LOCALAPPDATA%\gpucloud, which differs from C:\Users\<u>\.gpucloud.
        // Mismatch would split config / sessions / .env / logs across two
        // directories. install.ps1 sets GPUCLOUD_HOME via setx; the desktop
    ...
    gpucloudProcess.stdout.on('data', rememberLog)
    gpucloudProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    gpucloudProcess.once('error', error => {
      rememberLog(`GPUCLOUD backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `GPUCLOUD backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    gpucloudProcess.once('exit', (code, signal) => {
      rememberLog(`GPUCLOUD backend exited (${signal || code})`)
      gpucloudProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `GPUCLOUD backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `GPUCLOUD backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentGPUCLOUDLog()}`
          )
        )
      }
    })
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for GPUCLOUD backend to become ready', 90)
    await Promise.race([waitForGPUCLOUD(baseUrl, token), backendStartFailed])
    backendReady = true
    updateBootProgress({
      phase: 'backend.ready',
      message: 'GPUCLOUD backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startGPUCLOUD().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('gpucloud:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting GPUCLOUD…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('gpucloud:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetGPUCLOUDConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote GPUCLOUD backend failed liveness probe; dropping stale connection.')
    resetGPUCLOUDConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('gpucloud:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('gpucloud:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('gpucloud:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startGPUCLOUD() call restarts the
...
ipcMain.handle('gpucloud:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startGPUCLOUD() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('gpucloud:bootstrap:cancel', async () => {
...
ipcMain.handle('gpucloud:boot-progress:get', async () => bootProgressState)
ipcMain.handle('gpucloud:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('gpucloud:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('gpucloud:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('gpucloud:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('gpucloud:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('gpucloud:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('gpucloud:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('gpucloud:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('gpucloud:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('gpucloud:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new GPUCLOUD_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('gpucloud:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('gpucloud:requestMicrophoneAccess', async () => {
...
ipcMain.handle('gpucloud:api', async (_event, request) => {
...
ipcMain.handle('gpucloud:notify', (_event, payload) => {
  if (!Notification.isSupported()) return false
  new Notification({
    title: payload?.title || 'GPUCLOUD',
    body: payload?.body || '',
...
ipcMain.handle('gpucloud:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('gpucloud:readFileText', async (_event, filePath) => {
...
ipcMain.handle('gpucloud:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('gpucloud:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('gpucloud:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('gpucloud:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('gpucloud:saveClipboardImage', async () => {
...
ipcMain.handle('gpucloud:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('gpucloud:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('gpucloud:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('gpucloud:titlebar-theme', (_event, payload) => {
...
ipcMain.handle('gpucloud:openExternal', (_event, url) => {
...
// it via writeDefaultProjectDir so resolveGPUCLOUDCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('gpucloud:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: path.join(app.getPath('home'), 'gpucloud-projects')
}))

ipcMain.handle('gpucloud:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('gpucloud:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('gpucloud:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('gpucloud:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('gpucloud:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: gpucloudLog.slice(-200) }))
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip GPUCLOUD' TUI into
...
function terminalChannel(id, suffix) {
  return `gpucloud:terminal:${id}:${suffix}`
}
...
ipcMain.handle('gpucloud:fs:readDir', async (_event, dirPath) => {
...
ipcMain.handle('gpucloud:fs:gitRoot', async (_event, startPath) => {
...
ipcMain.handle('gpucloud:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart GPUCLOUD.')
  }
...
ipcMain.handle('gpucloud:terminal:write', (_event, id, data) => {
...
ipcMain.handle('gpucloud:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('gpucloud:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('gpucloud:updates:check', async () =>
...
ipcMain.handle('gpucloud:updates:apply', async (_event, payload) =>
...
ipcMain.handle('gpucloud:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('gpucloud:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical GPUCLOUD version (the one `release.py` bumps in
// gpucloud_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real GPUCLOUD version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveGPUCLOUDVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'gpucloud_cli', '__init__.py')
...
// Re-resolve the live GPUCLOUD version and push it into the native About panel
// just before showing it, so an in-place `gpucloud update` is reflected without
...
ipcMain.handle('gpucloud:version', async () => ({
  appVersion: resolveGPUCLOUDVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  gpucloudRoot: resolveUpdateRoot()
}))
...
// the actual removal via `gpucloud uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (gpucloud_cli/uninstall.py + gpucloud_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_GPUCLOUD_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isGPUCLOUDSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(py, ['-m', 'gpucloud_cli.main', 'uninstall', '--gui-summary'], {
        cwd: agentRoot,
        env: { ...process.env, GPUCLOUD_HOME, NO_COLOR: '1' },
        stdio: ['ignore', 'pipe', 'ignore']
      })
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no GPUCLOUD agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import gpucloud_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_GPUCLOUD_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_GPUCLOUD_ROOT, 'uninstall')
  } catch (error) {
  ...
  const scriptArgs = {
    ...
    pythonExe: py,
    pythonPath,
    agentRoot: ACTIVE_GPUCLOUD_ROOT,
    uninstallArgs,
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `gpucloud-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `gpucloud-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('gpucloud:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('gpucloud:uninstall:run', async (_event, payload) => {
