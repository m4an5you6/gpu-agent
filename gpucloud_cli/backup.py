...
def run_backup(args) -> None:
    ...
    hermes_root = get_default_gpucloud_root()

    if not hermes_root.is_dir():
        print(f"Error: GPUCLOUD home directory not found at {hermes_root}")
        sys.exit(1)
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print(f"  Run: gpucloud profile list  (after installing hermes)")

        # Guidance
        print()
        if not (hermes_root / "gpucloud-agent").is_dir():
...
def create_pre_update_backup(
    ...
    """
    hermes_root = gpucloud_home or get_default_gpucloud_root()
    if not hermes_root.is_dir():
        return None
