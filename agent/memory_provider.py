...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - gpucloud_home (str): The active GPUCLOUD_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.gpucloud``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "gpucloud").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'gpucloud memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], gpucloud_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'gpucloud memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``gpucloud_home`` is the active GPUCLOUD_HOME directory path.
