...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.gpucloudDesktop && ['audio', 'video'].includes(mediaKind(path))) {
  ...
  if (window.gpucloudDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.gpucloudDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.gpucloudDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function OpenMediaButton({ kind, path }: { kind: 'audio' | 'video'; path: string }) {
  return (
    <button
      ...
      onClick={() => void window.gpucloudDesktop?.openExternal(mediaExternalUrl(path))}
      type="button"
    >
      Open {kind} file
    </button>
