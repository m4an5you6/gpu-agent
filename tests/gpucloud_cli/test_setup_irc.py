"""Tests for IRC gateway configuration via `gpucloud setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh GPUCLOUD install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new GPUCLOUD install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#gpucloud")
            monkeypatch.setenv("IRC_NICKNAME", "gpucloud-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `gpucloud setup gateway` experience with IRC present."""
