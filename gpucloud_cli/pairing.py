...
Usage:
    gpucloud pairing list              # Show all pending + approved users
    gpucloud pairing approve <platform> <code>  # Approve a pairing code
    gpucloud pairing revoke <platform> <user_id> # Revoke user access
    gpucloud pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle gpucloud pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: gpucloud pairing {list|approve|revoke|clear-pending}")
        print("Run 'gpucloud pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.gpucloud/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'gpucloud pairing list' to see pending codes.\n")
