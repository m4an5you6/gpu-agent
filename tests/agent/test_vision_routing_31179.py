...
def isolated_home(monkeypatch):
    """Temp GPUCLOUD_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="gpucloud_test_31179_")
    gpucloud_home = os.path.join(test_home, ".gpucloud")
    os.makedirs(gpucloud_home)
...
def _fresh_modules():
    """Drop cached gpucloud modules so each test reloads against current env."""
