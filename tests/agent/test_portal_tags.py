...
def test_gpucloud_client_tag_includes_current_version():
    """The client tag must reflect gpucloud_cli.__version__ verbatim."""
    from gpucloud_cli import __version__
    from agent.portal_tags import gpucloud_client_tag

    assert gpucloud_client_tag() == f"client=gpucloud-client-v{__version__}"


def test_gpucloud_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import gpucloud_client_tag

    tag = gpucloud_client_tag()
    assert tag.startswith("client=gpucloud-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import gpucloud_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=gpucloud-agent" in tags
    assert gpucloud_client_tag() in tags
    assert len(tags) == 2
