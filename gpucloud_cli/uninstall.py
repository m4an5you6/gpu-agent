...
def _hermes_path_markers(gpucloud_home: Path) -> list[str]:
...
def remove_path_from_windows_registry(gpucloud_home: Path) -> list[str]:
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _hermes_path_markers(gpucloud_home)
            kept: list[str] = []
...
def _uninstall_profile(profile) -> None:
    ...
    hermes_invocation = [_sys.executable, "-m", "gpucloud_cli.main", "--profile", name]
...
def run_uninstall(args):
    ...
    print(color("│            GPUCLOUD Agent Uninstaller                  │", Colors.CYAN, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/NousResearch/gpucloud-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/gpucloud-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
