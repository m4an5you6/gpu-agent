...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/gpucloud-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    gpucloud_meta = (fm.get("metadata") or {}).get("gpucloud") or {}
    tags = gpucloud_meta.get("tags") or []
    related = gpucloud_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `gpucloud skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with GPUCLOUD Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "GPUCLOUD ships with a large built-in skill library copied into `~/.gpucloud/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "GPUCLOUD also syncs bundled skills on `gpucloud update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.gpucloud/skills/` tree, it is still shipped with GPUCLOUD; restore it with `gpucloud skills reset <name> --restore`.",
        "",
