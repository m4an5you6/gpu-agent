---
sidebar_position: 6
title: "Use MCP with GPUCLOUD"
description: "A practical guide to connecting MCP servers to GPUCLOUD Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with GPUCLOUD

This guide shows how to actually use MCP with GPUCLOUD Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native GPUCLOUD tool
- you want GPUCLOUD to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect GPUCLOUD to internal APIs, databases, or company systems without modifying GPUCLOUD core

Do not use MCP when:
- a built-in GPUCLOUD tool already solves the job well
...
- GPUCLOUD remains the agent
- MCP servers contribute tools
- GPUCLOUD discovers those tools at startup or reload time
...
If you installed GPUCLOUD with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- GPUCLOUD banner/status should show MCP integration when configured
- ask GPUCLOUD what tools it has available
...
## WSL2: bridge GPUCLOUD in WSL to Windows Chrome

This is the practical setup when:

- GPUCLOUD runs inside WSL2
...
In this setup, GPUCLOUD does **not** connect to Chrome directly. Instead:

- GPUCLOUD runs in WSL
- GPUCLOUD starts a local stdio MCP server
...
```text
GPUCLOUD (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- GPUCLOUD stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on GPUCLOUD core browser transport
...
```bash
gpucloud mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
gpucloud mcp test chrome-devtools-win
```

Then start a fresh GPUCLOUD session or run:
...
Once loaded, GPUCLOUD can use the MCP-prefixed browser tools directly. For example:
...
If GPUCLOUD runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start GPUCLOUD from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start GPUCLOUD from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in GPUCLOUD:
...
2. GPUCLOUD-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So GPUCLOUD will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want GPUCLOUD to reason over a bounded workspace.
...
Now GPUCLOUD can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing GPUCLOUD core.
...
Because GPUCLOUD now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
