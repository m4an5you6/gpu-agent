...
session-scoped state file under ``$GPUCLOUD_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $GPUCLOUD_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_gpucloud_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this gpucloud install, leave it first —
    ...
    env = os.environ.copy()
    env["GPUCLOUD_MEET_URL"] = url
    env["GPUCLOUD_MEET_OUT_DIR"] = str(out)
    env["GPUCLOUD_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["GPUCLOUD_MEET_HEADED"] = "1"
    if auth_state:
        env["GPUCLOUD_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["GPUCLOUD_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if GPUCLOUD_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["GPUCLOUD_MEET_MODE"] = mode
    if realtime_model:
        env["GPUCLOUD_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["GPUCLOUD_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["GPUCLOUD_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["GPUCLOUD_MEET_REALTIME_KEY"] = realtime_api_key
