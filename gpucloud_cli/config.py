...
def get_container_exec_info() -> Optional[dict]:
    ...
    hermes_bin = info.get("hermes_bin", "/data/current-package/bin/gpucloud")
...
from gpucloud_constants import get_gpucloud_home, migrate_hermes_home_to_gpucloud_home  # noqa: F811,E402
...
_KNOWN_ROOT_KEYS = {
    ...
    "auxiliary", "custom_providers", "context", "memory",
    "sessions",
    # Legacy keys still accepted when present in user config.yaml:
    "gateway", "dashboard", "kanban", "tts", "stt", "voice",
    "slack", "discord", "whatsapp", "telegram", "mattermost", "matrix",
}
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if result.get("missing_source"):
            print("No legacy ~/.hermes directory found; nothing to migrate.")
            return
        copied = list(result.get("copied", []))
        skipped = list(result.get("skipped", []))
        verb = "Would copy" if result.get("dry_run") else "Copied"
        ...
        if skipped:
            print(f"Skipped existing: {len(skipped)} item(s)")
            for path in skipped[:20]:
                print(f"  = {path}")
            if len(skipped) > 20:
                print(f"  ... {len(skipped) - 20} more")
        print("Legacy source was left untouched.")

    elif subcmd == "validate":
        from gpucloud_cli.gpucloud_config import run_config_validate
        ...
        print("  gpucloud config migrate-home  Copy ~/.hermes to ~/.gpucloud")
