...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "gpucloud_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    # save_config writes to ``get_gpucloud_home() / config.yaml`` — point it here.
    monkeypatch.setattr("gpucloud_constants.get_gpucloud_home", lambda: gpucloud_home)
    monkeypatch.setattr("gpucloud_cli.config.get_gpucloud_home", lambda: gpucloud_home)
    return cfg_path
