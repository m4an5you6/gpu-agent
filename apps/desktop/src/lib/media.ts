...
export function mediaStreamUrl(path: string): string {
  return `gpucloud-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// expose GET /api/media (gpucloud_cli/web_server.py).
export async function gatewayMediaDataUrl(path: string): Promise<string> {
  const file = filePathFromMediaPath(path)

  const result = await window.gpucloudDesktop!.api<{ data_url: string }>({
