...
The `execute_code` tool lets the agent write Python scripts that call GPUCLOUD tools programmatically, collapsing multi-step workflows into a single LLM turn. The script runs in a child process on the agent host, communicating with GPUCLOUD over a Unix domain socket RPC.

## How It Works

1. The agent writes a Python script using `from gpucloud_tools import ...`
2. GPUCLOUD generates a `gpucloud_tools.py` stub module with RPC functions
3. GPUCLOUD opens a Unix domain socket and starts an RPC listener thread
4. The script runs in a child process — tool calls travel over the socket back to GPUCLOUD
5. Only the script's `print()` output is returned to the LLM; intermediate tool results never enter the context window

```python
# The agent can write scripts like:
from gpucloud_tools import web_search, web_extract
...
```python
from gpucloud_tools import search_files, read_file
import json
...
```python
from gpucloud_tools import search_files, read_file, patch
...
```python
from gpucloud_tools import terminal, read_file
import json
...
`execute_code` has two execution modes controlled by `code_execution.mode` in `~/.gpucloud/config.yaml`:

| Mode | Working directory | Python interpreter |
|------|-------------------|--------------------|
| **`project`** (default) | The session's working directory (same as `terminal()`) | Active `VIRTUAL_ENV` / `CONDA_PREFIX` python, falling back to GPUCLOUD's own python |
| `strict` | A temp staging directory isolated from the user's project | `sys.executable` (GPUCLOUD's own python) |
...
### `GPUCLOUD_*` variables in the child

The child process receives only a small, fixed set of operational `GPUCLOUD_*`
...
(plus `GPUCLOUD_RPC_DIR` / `GPUCLOUD_RPC_SOCKET` / `TZ` / `HOME`, which GPUCLOUD
injects explicitly so the RPC channel works).

:::note Behavior change
Earlier versions passed **any** variable whose name began with `GPUCLOUD_`
through to the child. That broad prefix was removed for security hardening: it
could leak `GPUCLOUD_*`-named configuration that doesn't match a secret substring
(for example `GPUCLOUD_BASE_URL`, `GPUCLOUD_KANBAN_DB`, or a `GPUCLOUD_*_WEBHOOK`
endpoint) into arbitrary sandboxed code.

If an `execute_code` script — or a repo/plugin module it imports at import time
— relied on a `GPUCLOUD_*` variable outside the four operational names above, it
...
the secret-stripping guarantee (GPUCLOUD-managed provider credentials can never
...
**Diagnosing it.** When the child drops one or more non-allowlisted `GPUCLOUD_*`
variables, GPUCLOUD emits a one-line `debug` log naming them and pointing at the
`env_passthrough` escape hatch. Run with debug logging (`gpucloud logs --level
DEBUG`, or check `~/.gpucloud/logs/agent.log`) and look for
`execute_code: dropped N non-allowlisted GPUCLOUD_* var(s)` if a script behaves
as though a `GPUCLOUD_*` variable is missing.

GPUCLOUD always writes the script and the auto-generated `gpucloud_tools.py` RPC stub into a temp staging directory that is cleaned up after execution. In `strict` mode the script also *runs* there; in `project` mode it runs in the session's working directory (the staging directory stays on `PYTHONPATH` so imports still resolve). The child process runs in its own process group so it can be cleanly killed on timeout or interruption.
...
**Rule of thumb:** Use `execute_code` when you need to call GPUCLOUD tools programmatically with logic between calls. Use `terminal` for running shell commands, builds, and processes.
