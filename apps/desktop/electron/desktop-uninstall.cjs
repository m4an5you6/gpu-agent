 ...
 * the three user-facing uninstall modes to the `gpucloud uninstall` CLI flags,
 ...
 *              `gpucloud uninstall --gui --yes`
 *   - 'lite' → remove the GUI + agent code, KEEP user data (config / sessions
 *              / .env) for a future reinstall. `gpucloud uninstall --yes`
 *   - 'full' → remove everything: GUI + agent + all user data.
 *              `gpucloud uninstall --full --yes`
 *
 * Why a detached cleanup script: 'lite'/'full' delete the very venv the
 * `gpucloud` command runs from, and every mode may need to delete the running
...
/**
 * Map an uninstall mode to the `python -m gpucloud_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `gpucloud_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'gpucloud_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/GPUCLOUD.app/Contents/MacOS/GPUCLOUD  → …/GPUCLOUD.app
 *   Windows: …\GPUCLOUD\GPUCLOUD.exe                 → …\GPUCLOUD  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    // …/GPUCLOUD.app/Contents/MacOS/GPUCLOUD → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/GPUCLOUD.app
    if (appBundle.endsWith('.app')) return appBundle
    return null
  }

  if (platform === 'win32') {
    // NSIS per-user installs GPUCLOUD.exe directly in the install dir.
    const dir = p.dirname(exe)
    if (/[\\/]GPUCLOUD$/i.test(dir) || /[\\/]gpucloud-desktop$/i.test(dir)) return dir
    return null
  }

  // Linux: an AppImage exposes its own path via the APPIMAGE env var.
  if (env.APPIMAGE) return env.APPIMAGE
  // Unpacked electron-builder tree: …/linux-unpacked/gpucloud
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import gpucloud_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, gpucloudHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export GPUCLOUD_HOME=${q(gpucloudHome)}`
  ]
  if (pythonPath) {
 ...
 * modes + `pythonPath`=agentRoot so `import gpucloud_cli` resolves from source
 ...
 */
function buildWindowsCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, gpucloudHome }) {
  const pid = Number(desktopPid) || 0
  // cmd.exe has no string escaping inside quotes; strip embedded quotes (paths
  // under %LOCALAPPDATA% never contain them). `&`/`^` in a path would still be
  // a problem, but GPUCLOUD install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "GPUCLOUD_HOME=${String(gpucloudHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
  if (pythonPath) {
