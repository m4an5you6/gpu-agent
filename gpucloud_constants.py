...
def _get_platform_default_gpucloud_home() -> Path:
    """Return the platform-native default GPUCLOUD home path."""
...
def _get_platform_default_hermes_home() -> Path:
    """Return the legacy platform-native Hermes home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "hermes"
...
def _warn_legacy_home_fallback(path: str) -> None:
    """Emit a one-shot warning when using the deprecated HERMES_HOME path."""
    global _legacy_home_warned
    if _legacy_home_warned:
        return
    _legacy_home_warned = True
    msg = (
        "[GPUCLOUD migration] HERMES_HOME is deprecated. Using legacy "
        f"path {path!r} because GPUCLOUD_HOME is unset. Set GPUCLOUD_HOME "
        "or run `gpucloud config migrate-home` to copy data into ~/.gpucloud."
    )
    try:
...
def get_gpucloud_home() -> Path:
    """Return the GPUCLOUD home directory (default: platform-native path).

    Reads GPUCLOUD_HOME env var, falls back to the platform-native default.
    ...
    legacy_val = os.environ.get("HERMES_HOME", "").strip()
    if legacy_val:
        _warn_legacy_home_fallback(legacy_val)
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_gpucloud_home()
    ...
    return _get_platform_default_gpucloud_home()
...
def migrate_hermes_home_to_gpucloud_home(
    ...
    *,
    ...
    """Copy legacy Hermes user data into the GPUCLOUD home directory.

    The migration is intentionally non-destructive: it never deletes the
    legacy ``~/.hermes`` tree and it does not overwrite existing files in the
    destination.  Callers get a small structured report suitable for CLI
    display or tests.
    """
    import shutil

    src_path = Path(src) if src is not None else _get_platform_default_hermes_home()
    dst_path = Path(dst) if dst is not None else _get_platform_default_gpucloud_home()
    ...
    if not src_path.exists():
        report["missing"].append(str(src_path))
        return report
...
def get_default_gpucloud_root() -> Path:
    ...
    In standard deployments this is the platform-native GPUCLOUD home
    (``~/.gpucloud`` on POSIX, ``%LOCALAPPDATA%\\gpucloud`` on native Windows).
    ...
    """
    native_home = _get_platform_default_gpucloud_home()
...
def get_subprocess_home() -> str | None:
    ...
    **never** modified — only subprocess environments should inject this value.
    ...
    """
    gpucloud_home = get_gpucloud_home_override() or os.getenv("GPUCLOUD_HOME")
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_gpucloud_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._gpucloud_ipv4_patched = True  # type: ignore[attr-defined]
...
# Deprecated Hermes compatibility aliases.  Keep these at the bottom so old
# plugins/scripts can import old function names while new code uses GPUCLOUD.
