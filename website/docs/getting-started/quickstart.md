...
**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [GPUCLOUD Agent Tutorials & Use Cases](https://www.youtube.com/channel/UCqB1bhMwGsW-yefBxYwFCCg) playlist.
...
```bash
pip install gpucloud-agent
gpucloud postinstall     # optional: installs Node.js, browser, ripgrep, ffmpeg + runs setup
```
...
# ~/.gpucloud/gpucloud-agent on Linux/macOS or %LOCALAPPDATA%\hermes\gpucloud-agent on Windows):
...
```bash
gpucloud skills search kubernetes
