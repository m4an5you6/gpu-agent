...
When GPUCLOUD_UID is remapped at container boot, ``usermod -u`` only rewrites
files under the gpucloud user's home directory ($GPUCLOUD_HOME == /opt/data).
Runtime-writable trees under ``/opt/gpucloud`` must be explicitly chowned to the
new UID before services drop privileges. ``/opt/gpucloud/gateway`` is one such
