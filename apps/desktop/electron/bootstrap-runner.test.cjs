...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'gpucloud-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/gpucloud-runner-test',
    sourceRepoRoot: null,
    gpucloudHome: '/tmp/gpucloud-runner-test',
    logRoot: '/tmp/gpucloud-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'gpucloud-agent', 'scripts')
