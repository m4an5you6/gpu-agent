"""gpucloud hooks — inspect and manage shell-script hooks.

Usage::

    gpucloud hooks list
    gpucloud hooks test <event> [--for-tool X] [--payload-file F]
    gpucloud hooks revoke <command>
    gpucloud hooks doctor

Consent records live under ``~/.gpucloud/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.gpucloud/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``gpucloud hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: gpucloud hooks {list|test|revoke|doctor}")
        print("Run 'gpucloud hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.gpucloud/config.yaml.")
        print("See `gpucloud hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `gpucloud hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `gpucloud hooks test` and `gpucloud hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (GPUCLOUD wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `gpucloud hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `gpucloud hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `gpucloud hooks doctor`.")
