#!/bin/sh
# shellcheck shell=sh
# /opt/gpucloud/bin/gpucloud — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `gpucloud` user (UID 10000). When an operator runs `docker exec <c> gpucloud ...`
# the default UID is root (0), and any file the command writes under
# $GPUCLOUD_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> gpucloud login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: GPUCLOUD is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> gpucloud
...
# Fix
# ---
# This shim sits at /opt/gpucloud/bin/gpucloud and is placed earliest on PATH.
# When invoked as root, it drops to the gpucloud user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $GPUCLOUD_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user gpucloud`, kanban subagents, etc. — it short-circuits
...
# (/opt/gpucloud/.venv/bin/gpucloud), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set GPUCLOUD_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the gpucloud CLI. Default is to drop.

set -e

REAL=/opt/gpucloud/.venv/bin/gpucloud

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "gpucloud-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user gpucloud`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${GPUCLOUD_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the gpucloud user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "gpucloud-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "gpucloud-shim: re-run with --user gpucloud or set GPUCLOUD_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the gpucloud user's home before dropping privileges. Without
