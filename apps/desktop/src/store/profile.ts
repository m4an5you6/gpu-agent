...
import { getProfiles, setApiRequestProfile } from '@/gpucloud'
...
import type { ProfileInfo } from '@/types/gpucloud'
...
// /api/profiles/active `current`). "default" is the root ~/.gpucloud. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'gpucloud.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'gpucloud.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.gpucloudDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new GPUCLOUD_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.gpucloudDesktop.profile.set(name)
}
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'gpucloud.desktop.showAllProfiles'
...
// Switch to the default (root ~/.gpucloud) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.gpucloudDesktop?.touchBackend?.(target).catch(() => undefined)
}
