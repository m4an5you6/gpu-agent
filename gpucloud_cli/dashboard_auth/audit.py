...
Profile-aware location: ``$GPUCLOUD_HOME/logs/dashboard-auth.log``.
...
from ``gpucloud_constants`` or other gpucloud_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$GPUCLOUD_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``gpucloud_constants.get_gpucloud_home`` semantics: env var wins,
    else ``~/.gpucloud``. A local copy avoids an import cycle with the
    middleware which lives below ``gpucloud_cli``.
    """
    home = os.environ.get("GPUCLOUD_HOME") or str(Path.home() / ".gpucloud")
