...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    ...
    os.environ["GPUCLOUD_MODEL"] = result.new_model
    os.environ["GPUCLOUD_INFERENCE_MODEL"] = result.new_model
    ...
    # in config or env.
    #
    # GPUCLOUD_TUI_PROVIDER is the canonical "explicit-this-process" carrier
    ...
    if result.target_provider:
        os.environ["GPUCLOUD_INFERENCE_PROVIDER"] = result.target_provider
        os.environ["GPUCLOUD_TUI_PROVIDER"] = result.target_provider
    if persist_global:
