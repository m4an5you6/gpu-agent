...
description: "Diagnose and fix common GPUCLOUD cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`gpucloud gateway` for foreground, or `gpucloud gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `gpucloud cron tick`.
...
```bash
date
gpucloud cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.gpucloud/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.gpucloud/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.gpucloud/.env` |
...
| `local` | Write access to `~/.gpucloud/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `gpucloud cron list` for updated `last_error` field (if available).
...
```bash
gpucloud skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `gpucloud skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `gpucloud skills list`.
...
2. `~/.gpucloud/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `gpucloud cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the GPUCLOUD config directory). Verify:
```bash
ls ~/.gpucloud/scripts/your-script.py   # Must exist
gpucloud cron edit <job_id> --script ~/.gpucloud/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `gpucloud skills install <skill-name>`.
...
```bash
ps aux | grep gpucloud
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.gpucloud/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.gpucloud/cron/jobs.json
chmod 600 ~/.gpucloud/cron/jobs.json   # Your user should own it
```
...
```bash
gpucloud cron list                    # Show all jobs, states, next_run times
gpucloud cron run <job_id>            # Schedule for next tick (for testing)
gpucloud cron edit <job_id>           # Fix configuration issues
gpucloud logs                         # View recent GPUCLOUD logs
gpucloud skills list                  # Verify installed skills
```
...
1. Run the job with `gpucloud cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.gpucloud/logs/agent.log` for scheduler messages and `~/.gpucloud/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/gpucloud-agent](https://github.com/NousResearch/gpucloud-agent) with:
