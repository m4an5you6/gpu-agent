...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `gpucloud config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-hermes-config) below).
...
1. **`toolsets.py`** — Add `"hermes-newplat"` toolset definition with `_GPUCLOUD_CORE_TOOLS`
2. **`toolsets.py`** — Add `"hermes-newplat"` to the `"gpucloud-gateway"` includes list
