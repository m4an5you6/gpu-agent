...
const DESKTOP_COMMAND_META = [
  ...
  ['/profile', 'Switch the active GPUCLOUD profile'],
  ...
  ['/version', 'Show GPUCLOUD Agent version'],
...
export function isDesktopSlashCommand(command: string): boolean {
  ...
  return DESKTOP_COMMANDS.has(canonical) || !isKnownGPUCLOUDSlashCommand(normalized)
}

/**
 * An "extension" command is anything the backend surfaces that is NOT one of
 * GPUCLOUD' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownGPUCLOUDSlashCommand(normalized)
}
...
function isKnownGPUCLOUDSlashCommand(command: string): boolean {
