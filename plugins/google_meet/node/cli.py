"""`gpucloud meet node ...` subcommand tree.

Wired into the existing ``gpucloud meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``gpucloud meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="gpucloud-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``gpucloud meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             gpucloud meet node approve <name> ws://<host>:{args.port} {token}")
        try:
