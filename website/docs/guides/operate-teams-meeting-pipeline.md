...
```bash
gpucloud teams-pipeline validate
```
...
```bash
gpucloud teams-pipeline token-health
gpucloud teams-pipeline token-health --force-refresh
```
...
```bash
gpucloud teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
gpucloud teams-pipeline maintain-subscriptions
gpucloud teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: GPUCLOUD cron (recommended if you already run the GPUCLOUD gateway)

GPUCLOUD ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.gpucloud/scripts/`. First create the script:

```bash
mkdir -p ~/.gpucloud/scripts
cat > ~/.gpucloud/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec gpucloud teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.gpucloud/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
gpucloud cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/gpucloud-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=GPUCLOUD Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=gpucloud
EnvironmentFile=/etc/gpucloud/env
ExecStart=/usr/local/bin/gpucloud teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/gpucloud-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run GPUCLOUD Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now gpucloud-teams-pipeline-maintain.timer
systemctl list-timers gpucloud-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/gpucloud teams-pipeline maintain-subscriptions >> /var/log/gpucloud/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.gpucloud/.env` at the top of a wrapper script that crontab calls.
...
```bash
gpucloud teams-pipeline subscriptions   # should show expirationDateTime advanced
gpucloud teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
gpucloud teams-pipeline list
gpucloud teams-pipeline list --status failed
gpucloud teams-pipeline show <job-id>
```

### Replay a stored job

```bash
gpucloud teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
gpucloud teams-pipeline fetch --meeting-id <meeting-id>
gpucloud teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `gpucloud teams-pipeline maintain-subscriptions --dry-run`
- inspect `gpucloud teams-pipeline list --status failed`
...
- run `gpucloud teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `gpucloud teams-pipeline run`
...
- [ ] `gpucloud teams-pipeline validate` returns an OK snapshot
- [ ] `gpucloud teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (GPUCLOUD cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
