...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate GPUCLOUD_HOME under tmp_path."""
    monkeypatch.setenv("GPUCLOUD_HOME", str(tmp_path / ".gpucloud"))
    (tmp_path / ".gpucloud").mkdir()

    # Force the constants/image cache helpers to re-read GPUCLOUD_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("gpucloud_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_gpucloud_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under GPUCLOUD_HOME — gateway cleanup
