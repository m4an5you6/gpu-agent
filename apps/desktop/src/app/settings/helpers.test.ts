...
import type { GPUCLOUDConfigRecord } from '@/types/gpucloud'
...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: GPUCLOUDConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: GPUCLOUDConfigRecord = {}
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // GPUCLOUD_QWEN_ and GPUCLOUD_GEMINI_ both share the GPUCLOUD_ stem.
      expect(providerGroup('GPUCLOUD_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
      expect(providerGroup('GPUCLOUD_GEMINI_CLIENT_ID')).toBe('Gemini')
    })
