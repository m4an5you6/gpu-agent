...
class TestIsWriteDenied:
    ...
    def test_gpucloud_control_files_oauth_and_mcp_tokens_denied(self, path):
        """GPUCLOUD control files, PKCE creds, mcp-tokens, and pairing entries must be write-denied."""
        from gpucloud_constants import get_gpucloud_home
        gpucloud_home = get_gpucloud_home()
        full_path = str(gpucloud_home / path)
    ...
    def test_gpucloud_control_files_and_oauth_traversal_denied(self, path):
        """Path traversal attempts to protected GPUCLOUD files must be blocked."""
        from gpucloud_constants import get_gpucloud_home
        gpucloud_home = get_gpucloud_home()
        full_path = str(gpucloud_home / path)
        assert _is_write_denied(full_path) is True
    ...
    def test_control_files_and_oauth_protected_in_profile_mode(self, tmp_path, monkeypatch, name):
        ...
        global ~/.gpucloud/{auth.json,config.yaml,webhook_subscriptions.json,
        .anthropic_oauth.json} writable — the same gap PR #15981 fixed
        for .env.
        """
        # Simulate a profile-mode GPUCLOUD_HOME layout:
    ...
    def test_pairing_dir_denied(self, tmp_path, monkeypatch):
        ...
        PR #30383 introduced ~/.gpucloud/pairing/{platform}-approved.json as the
...
class TestShellFileOpsHelpers:
    ...
    def test_read_file_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "'\x07__GPUCLOUD_FENCE_a9f7b3__\x1b]0;cat "
            "'/tmp/test/a.py' 2> /dev/null\x07\n"
            "print('ok')\n"
            "__GPUCLOUD_FENCE_a9f7b3__\x07'\n"
        )
        ...
        assert "GPUCLOUD_FENCE" not in result.content
    ...
    def test_read_file_raw_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "__GPUCLOUD_FENCE_a9f7b3__\x07'\n"
            "alpha\n"
            "\x1b]0;cat '/tmp/test/a.txt'\x07__GPUCLOUD_FENCE_a9f7b3__\n"
        )
