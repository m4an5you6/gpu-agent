...
_DEFAULT_CONTAINER_TAG = "gpucloud"
...
def _load_supermemory_config(gpucloud_home: str) -> dict:
    config = _default_config()
    config_path = Path(gpucloud_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, gpucloud_home: str) -> None:
    config_path = Path(gpucloud_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "gpucloud"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "gpucloud", **(metadata or {})}
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `gpucloud memory setup`.
        # All other options are documented for $GPUCLOUD_HOME/supermemory.json
    ...
    def save_config(self, values, gpucloud_home):
        ...
        _save_supermemory_config(sanitized, gpucloud_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from gpucloud_constants import get_gpucloud_home
        self._gpucloud_home = kwargs.get("gpucloud_home") or str(get_gpucloud_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._gpucloud_home)
