...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("gpucloud/gpucloud-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned gpucloud/* and pr-* branches."""

    def test_prunes_orphaned_gpucloud_branch(self, git_repo):
        """gpucloud/gpucloud-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "gpucloud/gpucloud-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "gpucloud/gpucloud-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "gpucloud/gpucloud-deadbeef" in result.stdout
        ...
        assert "gpucloud/gpucloud-deadbeef" in orphaned
        ...
        assert "gpucloud/gpucloud-deadbeef" not in result.stdout
