"""Slash command definitions and autocomplete for the GPUCLOUD CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of GPUCLOUD config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal GPUCLOUD works on across turns until achieved", "Session",
               args_hint="[text | pause | resume | clear | status]"),
    CommandDef("autogoal", "Run a non-interactive ML training/inference/deployment loop", "Session",
               args_hint="[objective | pause | resume | clear | status]"),
    ...
    CommandDef("busy", "Control what Enter does while GPUCLOUD is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.gpucloud/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update GPUCLOUD Agent to the latest version", "Info"),
    CommandDef("version", "Show GPUCLOUD Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`gpucloud_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/gpucloud`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full GPUCLOUD
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``gpucloud skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``gpucloud skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``gpucloud skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/gpucloud <verb>`` subcommand.
    ...
    can still reach them via ``/gpucloud <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/gpucloud`` is always reserved as the first entry so the
    legacy ``/gpucloud <subcommand>`` form keeps working for anything that
    ...
    # Reserve /gpucloud as the catch-all top-level command.
    entries.append(("gpucloud", "Talk to GPUCLOUD or run a subcommand", "[subcommand] [args]"))
    seen.add("gpucloud")
...
def slack_app_manifest(request_url: str = "https://gpucloud-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /gpucloud handler.

    Maps both canonical names and aliases so /gpucloud bg do stuff works
    the same as /gpucloud background do stuff.

    Plugin-registered slash commands are included so ``/gpucloud <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from gpucloud_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
