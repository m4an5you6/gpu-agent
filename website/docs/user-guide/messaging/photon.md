...
Connect GPUCLOUD to **iMessage** through [Photon][photon], a managed
...
your first iMessage from GPUCLOUD — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, GPUCLOUD runs it in a
...
2. **Finds or creates** the `GPUCLOUD Agent` project on your account.
...
Runtime credentials are written to `~/.gpucloud/.env`
...
(device token, dashboard project id) lives in `~/.gpucloud/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other GPUCLOUD
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, GPUCLOUD replies with a pairing code. Approve it with:

```bash
gpucloud pairing approve photon <CODE>
```

Use `gpucloud pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.gpucloud/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.gpucloud/.env`):
...
By default GPUCLOUD responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `GPUCLOUD` and
`@GPUCLOUD agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and GPUCLOUD will reply.

## Status & troubleshooting

```bash
gpucloud photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line GPUCLOUD uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run gpucloud photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `gpucloud photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `gpucloud photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `gpucloud photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** GPUCLOUD sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | GPUCLOUD wake words  | JSON list / comma / newline regex patterns for group mentions |
