"""Tests for ACP Registry metadata shipped with GPUCLOUD."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "gpucloud-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "GPUCLOUD Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/gpucloud-agent"
    assert data["website"].startswith("https://gpucloud-agent.nousresearch.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"gpucloud-agent[acp]=={data['version']}"
    assert uvx["args"] == ["gpucloud-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"gpucloud-agent[acp]=={_pyproject_version()}"
    )
