...
GPUCLOUD runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. GPUCLOUD
...
1. GPUCLOUD captures a baseline of current diagnostics for the file.
...
…). GPUCLOUD auto-detects the binary on PATH or in
`<GPUCLOUD_HOME>/lsp/bin/`.
...
`node_modules` tree — GPUCLOUD installs both packages together when you
run `gpucloud lsp install typescript` or auto-install fires on first
use.

## CLI

```
gpucloud lsp status          # service state + per-server install status
gpucloud lsp list            # registry, optionally --installed-only
gpucloud lsp install <id>    # eagerly install one server
gpucloud lsp install-all     # try every server with a known recipe
gpucloud lsp restart         # tear down running clients
gpucloud lsp which <id>      # print resolved binary path
```

`gpucloud lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <GPUCLOUD_HOME>/lsp/bin
...
When `install_strategy: auto`, GPUCLOUD installs binaries into
`<GPUCLOUD_HOME>/lsp/bin/`. NPM packages land in
`<GPUCLOUD_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully GPUCLOUD-owned and is
...
Servers are kept alive for the life of the GPUCLOUD process. There's
...
**`gpucloud lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<GPUCLOUD_HOME>/lsp/bin/`. Run
`gpucloud lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `gpucloud lsp status`**
...
When `gpucloud lsp status` shows a `Backend warnings` section, install
...
`~/.gpucloud/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.gpucloud/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `gpucloud lsp restart` to clear the set;
