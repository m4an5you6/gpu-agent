...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      ...
      gpucloudVenv = gpucloud-agent.gpucloudVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "gpucloud-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${gpucloudVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "gpucloud-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "gpucloud-build-package" { } ''
          echo "PASS: package built at ${gpucloud-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "gpucloud-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "gpucloud-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${gpucloud-agent}/bin/gpucloud || (echo "FAIL: gpucloud binary missing"; exit 1)
          test -x ${gpucloud-agent}/bin/gpucloud-agent || (echo "FAIL: gpucloud-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${gpucloud-agent}/bin/gpucloud version 2>&1 | grep -qi "gpucloud" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "gpucloud-entry-points-sync" { } ''
          set -e
        ...
        cli-commands = pkgs.runCommand "gpucloud-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking gpucloud --help ==="
          ${gpucloud-agent}/bin/gpucloud --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${gpucloud-agent}/bin/gpucloud --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "gpucloud-bundled-skills" { } ''
          set -e
          ...
          grep -q "GPUCLOUD_BUNDLED_SKILLS" ${gpucloud-agent}/bin/gpucloud || \
        ...
        bundled-plugins = pkgs.runCommand "gpucloud-bundled-plugins" { } ''
          set -e
          ...
          grep -q "GPUCLOUD_BUNDLED_PLUGINS" ${gpucloud-agent}/bin/gpucloud || \
        ...
        bundled-locales = pkgs.runCommand "gpucloud-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${gpucloud-agent}/share/gpucloud-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${gpucloud-agent}/share/gpucloud-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${gpucloud-agent}/share/gpucloud-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "GPUCLOUD_BUNDLED_LOCALES" ${gpucloud-agent}/bin/gpucloud || \
            (echo "FAIL: GPUCLOUD_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: GPUCLOUD_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (GPUCLOUD_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && GPUCLOUD_BUNDLED_LOCALES=${gpucloud-agent}/share/gpucloud-agent/locales \
            ${gpucloudVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with GPUCLOUD_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${gpucloudVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${gpucloudVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without GPUCLOUD_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "gpucloud-bundled-tui" { } ''
          set -e
          ...
          grep -q "GPUCLOUD_TUI_DIR" ${gpucloud-agent}/bin/gpucloud || \
        ...
        gpucloud-node = pkgs.runCommand "gpucloud-node-version" { } ''
          set -e
          echo "=== Checking GPUCLOUD_NODE in wrapper ==="
          grep -q "GPUCLOUD_NODE" ${gpucloud-agent}/bin/gpucloud || \
            (echo "FAIL: GPUCLOUD_NODE not set in wrapper"; exit 1)
          echo "PASS: GPUCLOUD_NODE present in wrapper"

          GPUCLOUD_NODE=$(sed -n "s/^export GPUCLOUD_NODE='\(.*\)'/\1/p" ${gpucloud-agent}/bin/gpucloud)
        ...
        managed-guard = pkgs.runCommand "gpucloud-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked "config set" ${gpucloud-agent}/bin/gpucloud config set model foo
          check_blocked "config edit" ${gpucloud-agent}/bin/gpucloud config edit
        ...
        extra-python-packages = let
          ...
          gpucloudWithExtra = gpucloud-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "gpucloud-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${gpucloudWithExtra}/bin/gpucloud || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${gpucloudWithExtra}/bin/gpucloud || \
          ...
          if grep -q "PYTHONPATH" ${gpucloud-agent}/bin/gpucloud; then
        ...
        extra-dependency-groups = let
          gpucloudWithGroups = gpucloud-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "gpucloud-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${gpucloudWithGroups}"
          echo "venv: ${gpucloudWithGroups.gpucloudVenv}"
        ...
        messaging-variant = pkgs.runCommand "gpucloud-messaging-variant" { } ''
          set -e
        ...
        in pkgs.runCommand "gpucloud-config-roundtrip" {
