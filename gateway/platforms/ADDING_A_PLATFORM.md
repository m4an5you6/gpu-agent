...
There are two ways to add a platform to the GPUCLOUD gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.gpucloud/plugins/` (or under `plugins/platforms/`
...
**zero changes to core GPUCLOUD code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `gpucloud gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `gpucloud_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the GPUCLOUD core.
...
And add it to the `gpucloud-gateway` composite:

```python
"gpucloud-gateway": {
    "includes": [..., "gpucloud-your-platform"]
}
```
...
## 12. Status Display (`gpucloud_cli/status.py`)
...
## 13. Gateway Setup Wizard (`gpucloud_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ gpucloud_cli/ toolsets.py \
  --include="*.py" -l | sort -u
