"""
Microsoft Teams platform adapter for GPUCLOUD Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "GPUCLOUD"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        gpucloud_action = data.get("gpucloud_action", "")
        session_key = data.get("session_key", "")

        if not gpucloud_action or not session_key:
        ...
        choice = choice_map.get(gpucloud_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="gpucloud_approve",
                    data={**btn_data_base, "gpucloud_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="gpucloud_approve",
                    data={**btn_data_base, "gpucloud_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="gpucloud_approve",
                    data={**btn_data_base, "gpucloud_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="gpucloud_approve",
                    data={**btn_data_base, "gpucloud_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"GPUCLOUD\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.gpucloud/.env")
