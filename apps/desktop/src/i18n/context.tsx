...
import { getGPUCLOUDConfigRecord, type GPUCLOUDConfigRecord, saveGPUCLOUDConfig } from '@/gpucloud'
...
export interface I18nConfigClient {
  getConfig: () => Promise<GPUCLOUDConfigRecord>
  saveConfig: (config: GPUCLOUDConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.gpucloudDesktop?.api) {
      return Promise.resolve({})
    }

    return getGPUCLOUDConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.gpucloudDesktop?.api) {
    ...
    return saveGPUCLOUDConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: GPUCLOUDConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: GPUCLOUDConfigRecord, locale: Locale): GPUCLOUDConfigRecord {
