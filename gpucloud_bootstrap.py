"""Windows UTF-8 bootstrap for GPUCLOUD entry points.
...
should be imported at the very top of every GPUCLOUD entry point
(``gpucloud``, ``gpucloud-agent``, ``gpucloud-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import gpucloud_bootstrap``
# (or ``from gpucloud_bootstrap import apply_windows_utf8_bootstrap``) at
