"""Tests for the top-level `./gpucloud` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./gpucloud` should use `gpucloud_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "gpucloud"
    called = []

    fake_main_module = types.ModuleType("gpucloud_cli.main")

    def fake_main():
        called.append("gpucloud_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "gpucloud_cli.main", fake_main_module)
    ...
    assert called == ["gpucloud_cli.main"]
