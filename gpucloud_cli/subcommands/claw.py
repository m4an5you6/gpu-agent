"""``gpucloud claw`` subcommand parser.
...
def build_claw_parser(subparsers, *, cmd_claw: Callable) -> None:
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.gpucloud/ (by default a "
        "single restore-point archive is written to ~/.gpucloud/backups/ "
        "before apply; restorable with 'gpucloud import').",
    )
    claw_migrate.add_argument(
