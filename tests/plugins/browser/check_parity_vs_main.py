...
Run from the PR worktree:
    ...
    cd ~/.gpucloud/gpucloud-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.gpucloud/gpucloud-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.gpucloud/gpucloud-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a gpucloud-agent checkout"
)
...
# Isolated GPUCLOUD_HOME for the config write.
