...
title: "Git Worktrees"
description: "Run multiple GPUCLOUD agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

GPUCLOUD Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with GPUCLOUD so each session has a clean, isolated working directory.

## Why Use Worktrees with GPUCLOUD?

GPUCLOUD treats the **current working directory** as the project root:

- CLI: the directory where you run `gpucloud` or `gpucloud chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.gpucloud/config.yaml`
...
git worktree add ../repo-feature feature/gpucloud-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/gpucloud-experiment` checked out in that directory

Now you can `cd` into the new worktree and run GPUCLOUD there:

```bash
cd ../repo-feature

# Start GPUCLOUD in the worktree
gpucloud
```
...
git worktree add ../repo-experiment-a feature/gpucloud-a
git worktree add ../repo-experiment-b feature/gpucloud-b
```
...
Each GPUCLOUD process:

- Works on its own branch (`feature/gpucloud-a` vs `feature/gpucloud-b`).
...
- GPUCLOUD checkpoint data under `~/.gpucloud/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per GPUCLOUD experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/gpucloud-checkpoints-docs`, `feature/gpucloud-refactor-tests`.
- **Commit frequently**
...
- **Avoid running GPUCLOUD from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `gpucloud -w` (Automatic Worktree Mode)

GPUCLOUD has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `gpucloud/gpucloud-<hash>`).
...
```bash
gpucloud -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `gpucloud -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each GPUCLOUD session its own clean checkout.
