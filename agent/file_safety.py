...
def _gpucloud_home_path() -> Path:
    """Resolve the active GPUCLOUD_HOME (profile-aware) without circular imports."""
    try:
        from gpucloud_constants import get_gpucloud_home  # local import to avoid cycles
        return get_gpucloud_home()
    except Exception:
        return Path(os.path.expanduser("~/.gpucloud"))


def _gpucloud_root_path() -> Path:
    """Resolve the GPUCLOUD root dir (always the parent of any profile, never per-profile)."""
    try:
        from gpucloud_constants import get_default_gpucloud_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    gpucloud_home = _gpucloud_home_path()
    gpucloud_root = _gpucloud_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(gpucloud_home / ".env"),
            # Top-level .env, even when running under a profile — overwriting it
            # leaks credentials across every profile that inherits from root (#15981).
            str(gpucloud_root / ".env"),
            # Active profile Anthropic PKCE credential store.
            str(gpucloud_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(gpucloud_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved GPUCLOUD_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("GPUCLOUD_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # GPUCLOUD control-plane files: block both the ACTIVE profile's view
    # (gpucloud_home) AND the global root view. Without the root pass, a
    ...
    gpucloud_dirs = []
    for base in (_gpucloud_home_path(), _gpucloud_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in gpucloud_dirs:
                gpucloud_dirs.append(real)
        except Exception:
            continue

    for base_real in gpucloud_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied GPUCLOUD path.

    Three categories are blocked:

      * Internal GPUCLOUD cache files under ``GPUCLOUD_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under GPUCLOUD_HOME and the global GPUCLOUD
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.gpucloud/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active GPUCLOUD_HOME (profile-aware) AND the global
    # GPUCLOUD root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (GPUCLOUD_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    gpucloud_dirs: list[Path] = []
    for base in (_gpucloud_home_path(), _gpucloud_root_path()):
        try:
            real = base.resolve()
            if real not in gpucloud_dirs:
    ...
    for hd in gpucloud_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal GPUCLOUD cache file "
                "and cannot be read directly to prevent prompt injection. "
                "Use the skills_list or skill_view tools instead."
            )

    # Credential / secret stores. Exact-file matches under either
    # GPUCLOUD_HOME or <root>.
    ...
    for hd in gpucloud_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a GPUCLOUD credential store "
    ...
    for hd in gpucloud_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the GPUCLOUD MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a GPUCLOUD MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# GPUCLOUD profiles are separate GPUCLOUD_HOME dirs under
...
#
# Reference: May 2026 incident where a gpucloud-security profile session
# edited skills under both ``~/.gpucloud/profiles/gpucloud-security/skills/``
# AND ``~/.gpucloud/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under GPUCLOUD_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from GPUCLOUD_HOME.

    ``~/.gpucloud``              -> ``"default"``
    ``~/.gpucloud/profiles/X``  -> ``"X"``

    Falls back to ``"default"`` on any resolution failure so the guard
    never raises into the tool path.
    """
    try:
        home_real = _gpucloud_home_path().resolve()
        root_real = _gpucloud_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside GPUCLOUD scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    GPUCLOUD entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to GPUCLOUD profile {info['target_profile']!r}, but the "
...
#
#   <GPUCLOUD_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.gpucloud/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.gpucloud/…`` segment and warns
# regardless of which GPUCLOUD profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.gpucloud/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.gpucloud`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.gpucloud/…`` and returns the
    index where the inner GPUCLOUD-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .gpucloud / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".gpucloud":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative GPUCLOUD state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.gpucloud``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.gpucloud`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any GPUCLOUD resolver to
    succeed, so it works correctly even when called from contexts where
    GPUCLOUD_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host GPUCLOUD process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real GPUCLOUD_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.gpucloud/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.gpucloud/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host GPUCLOUD_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative GPUCLOUD state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host GPUCLOUD "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real GPUCLOUD_HOME. Use the "
