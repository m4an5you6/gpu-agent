...
async def test_run_agent_binds_api_session_context_for_tool_env(adapter, monkeypatch):
    ...
    monkeypatch.setenv("GPUCLOUD_SESSION_ID", "stale-session")
    observed = {}
    ...
    class FakeAgent:
        ...
        def run_conversation(self, user_message, conversation_history, task_id):
            ...
            observed["context_session_id"] = get_session_env("GPUCLOUD_SESSION_ID")
            observed["context_platform"] = get_session_env("GPUCLOUD_SESSION_PLATFORM")
            observed["context_session_key"] = get_session_env("GPUCLOUD_SESSION_KEY")
            observed["child_session_id"] = _make_run_env({}).get("GPUCLOUD_SESSION_ID")
...
async def test_session_crud_and_message_history(adapter, session_db):
    ...
    async with TestClient(TestServer(app)) as cli:
        ...
        assert created["object"] == "gpucloud.session"
        assert created["session"]["title"] == "Mobile chat"

        session_db.append_message(session_id, "user", "hello from phone")
        session_db.append_message(session_id, "assistant", "hello from gpucloud")
        ...
        assert deleted == {"object": "gpucloud.session.deleted", "id": session_id, "deleted": True}
...
async def test_session_fork_uses_current_sessiondb_branch_primitives(adapter, session_db):
    ...
    assert payload["object"] == "gpucloud.session"
...
async def test_session_chat_loads_history_and_preserves_session_headers(auth_adapter, session_db):
    ...
    assert payload["object"] == "gpucloud.session.chat.completion"
