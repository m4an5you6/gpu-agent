...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. GPUCLOUD passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("GPUCLOUD_HOME", "/users/alice/.gpucloud/profiles/backend-worker")
        monkeypatch.setenv("GPUCLOUD_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "GPUCLOUD_KANBAN_DB",
            "/users/alice/.gpucloud/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.gpucloud/kanban/boards/smoke"]'
            in cmd
        )
