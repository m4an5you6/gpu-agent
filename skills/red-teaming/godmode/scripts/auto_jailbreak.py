...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("GPUCLOUD_HOME", os.path.expanduser("~/.gpucloud")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("GPUCLOUD_HOME", Path.home() / ".gpucloud")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# GPUCLOUD config paths
# ═══════════════════════════════════════════════════════════════════

GPUCLOUD_HOME = Path(os.getenv("GPUCLOUD_HOME", Path.home() / ".gpucloud"))
CONFIG_PATH = GPUCLOUD_HOME / "config.yaml"
PREFILL_PATH = GPUCLOUD_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/GPUCLOUD models — already uncensored, just needs clean prompt
    "gpucloud": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "gpucloud" in model_lower or "nous" in model_lower:
        return "gpucloud"
...
def _get_current_model() -> tuple:
    """Read current model and provider from GPUCLOUD config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.gpucloud/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart GPUCLOUD for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart GPUCLOUD for changes to take effect.")
