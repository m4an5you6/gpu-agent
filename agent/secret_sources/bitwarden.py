...
GPUCLOUD pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.gpucloud/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<gpucloud_home>/bin/bws`` on
  first use.  GPUCLOUD pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.gpucloud/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``gpucloud`` invocations don't
  hammer the API.
* Failures NEVER block GPUCLOUD startup.  Missing binary, no network,
...
# In-process cache so repeated load_gpucloud_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `gpucloud chat -q ...`
...
# <gpucloud_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.gpucloud/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under gpucloud_home/cache/.

    `home_path` is what `load_gpucloud_dotenv()` already resolved; falling back
    to `$GPUCLOUD_HOME` / `~/.gpucloud` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("GPUCLOUD_HOME", Path.home() / ".gpucloud"))
...
def _gpucloud_bin_dir() -> Path:
    """Where GPUCLOUD stores its managed binaries.  Profile-aware."""
    from gpucloud_constants import get_gpucloud_home

    return get_gpucloud_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<gpucloud_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _gpucloud_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``gpucloud secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _gpucloud_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="gpucloud-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "gpucloud-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<gpucloud_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
    ...
    back to ``$GPUCLOUD_HOME`` / ``~/.gpucloud``.
...
# ---------------------------------------------------------------------------
# Public entry point — called from gpucloud_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_gpucloud_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `gpucloud secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `gpucloud secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `gpucloud secrets bitwarden setup` to install."
        )
        return result
