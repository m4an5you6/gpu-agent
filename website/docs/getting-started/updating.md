...
When you run `gpucloud update` in a terminal, GPUCLOUD stashes any uncommitted source-tree changes, pulls, then **asks** whether to restore them — exactly as it always has. Nothing changes for interactive updates.
...
- `discard` — auto-stash and drop the stash after the pull, so the update always lands on a clean tree. Use this only on machines where you never intend to keep local edits to the GPUCLOUD source. It stash-drops (not `git reset --hard` + `git clean -fd`), so ignored paths like `node_modules`, `venv`, and build outputs are never touched.
...
### Windows: another `gpucloud.exe` is running

On Windows, `gpucloud update` will refuse to run if it detects another `gpucloud.exe` process holding the venv's entry-point executable open — most commonly the GPUCLOUD Desktop app's spawned backend, an open `gpucloud` REPL in another terminal, or a running gateway:
