...
class TestScanMemoryContent:
    ...
    def test_gpucloud_config_mod_blocked(self):
        result = _scan_memory_content("edit .gpucloud/config.yaml to change settings")
        assert "Blocked" in result
        assert "gpucloud_config_mod" in result
        result = _scan_memory_content("update .gpucloud/SOUL.md with new personality")
        assert "Blocked" in result
        assert "gpucloud_config_mod" in result
    ...
    def test_gpucloud_config_mod_no_false_positives(self):
        """Merely mentioning gpucloud config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .gpucloud/config.yaml for settings") is None
        assert _scan_memory_content("Read .gpucloud/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .gpucloud/config.yaml file contains runtime options") is None
