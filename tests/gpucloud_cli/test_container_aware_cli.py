...
def container_env(tmp_path, monkeypatch):
    """Set up a fake GPUCLOUD_HOME with .container-mode file."""
    gpucloud_home = tmp_path / ".gpucloud"
    gpucloud_home.mkdir()
    monkeypatch.setenv("GPUCLOUD_HOME", str(gpucloud_home))
    monkeypatch.delenv("GPUCLOUD_DEV", raising=False)

    container_mode = gpucloud_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=gpucloud-agent\n"
        "exec_user=gpucloud\n"
        "gpucloud_bin=/data/current-package/bin/gpucloud\n"
    )
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("gpucloud_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "gpucloud-agent"
    assert info["exec_user"] == "gpucloud"
    assert info["gpucloud_bin"] == "/data/current-package/bin/gpucloud"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("gpucloud_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_gpucloud_dev(container_env, monkeypatch):
    """Returns None when GPUCLOUD_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("GPUCLOUD_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_gpucloud_dev_zero(container_env, monkeypatch):
    """GPUCLOUD_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("GPUCLOUD_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        gpucloud_home = Path(tmpdir) / ".gpucloud"
        gpucloud_home.mkdir()
        (gpucloud_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("gpucloud_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_gpucloud_home": lambda: gpucloud_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("GPUCLOUD_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=gpucloud-custom\n"
        "exec_user=myuser\n"
        "gpucloud_bin=/opt/gpucloud/bin/gpucloud\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "gpucloud-custom"
    assert info["exec_user"] == "myuser"
    assert info["gpucloud_bin"] == "/opt/gpucloud/bin/gpucloud"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from gpucloud_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "gpucloud"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "gpucloud-agent" in cmd
    assert "/data/current-package/bin/gpucloud" in cmd
    assert "chat" in cmd
