...
```bash
gpucloud setup --portal     # Nous OAuth, set Nous as provider, and turn on the Tool Gateway in one go
```

Already have GPUCLOUD configured? Just switch your provider:

```bash
gpucloud model              # Pick Nous Portal — GPUCLOUD will offer to turn on the Tool Gateway
```

When you select Nous Portal, GPUCLOUD offers to turn on the Tool Gateway. Accept, and you're done — every supported tool is live on the next run.

Check what's active at any time:

```bash
gpucloud portal status      # Portal auth + Tool Gateway routing summary
...
`gpucloud portal status` shows a section like:
