# GPUCLOUD Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: gpucloud-achievements plugin
...
### `GET /api/plugins/gpucloud-achievements/achievements`
...
### `POST /api/plugins/gpucloud-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/gpucloud-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/gpucloud-achievements/overview`
...
- Persist plugin data under:
  - `~/.gpucloud/plugins/gpucloud-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.gpucloud/plugins/gpucloud-achievements/scan_checkpoint.json`
...
- `~/.gpucloud/plugins/gpucloud-achievements/`
