"""
GPUCLOUD MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (GPUCLOUD-specific extra)

Usage:
    gpucloud mcp serve
    gpucloud mcp serve --verbose
...
logger = logging.getLogger("gpucloud.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using GPUCLOUD_HOME."""
    try:
        from gpucloud_constants import get_gpucloud_home
        return get_gpucloud_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("GPUCLOUD_HOME", Path.home() / ".gpucloud")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from gpucloud_constants import get_gpucloud_home
        directory_file = get_gpucloud_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("GPUCLOUD_HOME", Path.home() / ".gpucloud")
...
class EventBridge:
    ...
    This is the GPUCLOUD equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from gpucloud_constants import get_gpucloud_home
            db_file = get_gpucloud_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("GPUCLOUD_HOME", Path.home() / ".gpucloud")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the GPUCLOUD MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "gpucloud",
        instructions=(
            "GPUCLOUD Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the GPUCLOUD MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
