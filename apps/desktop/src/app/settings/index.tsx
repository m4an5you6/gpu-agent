...
import { getGPUCLOUDConfigDefaults, getGPUCLOUDConfigRecord, saveGPUCLOUDConfig } from '@/gpucloud'
...
export function SettingsView({ gateway, onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getGPUCLOUDConfigRecord()
      const blob = new Blob([JSON.stringify(cfg, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'gpucloud-config.json'
      a.click()
  ...
  const resetConfig = async () => {
    ...
    try {
      await saveGPUCLOUDConfig(await getGPUCLOUDConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
