...
description: "How GPUCLOUD stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `gpucloud_cli/cron.py` | CLI `gpucloud cron` subcommands |
...
Jobs are stored in `~/.gpucloud/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `gpucloud cron` commands are run or during active CLI sessions.
...
```python
# ~/.gpucloud/scripts/check_competitors.py
...
2. **Environment variable** — `GPUCLOUD_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.gpucloud/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `gpucloud cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `gpucloud cron` CLI provides direct job management:

```bash
gpucloud cron list                    # Show all jobs
gpucloud cron create                  # Interactive job creation (alias: add)
gpucloud cron edit <job_id>           # Edit job configuration
gpucloud cron pause <job_id>          # Pause a running job
gpucloud cron resume <job_id>         # Resume a paused job
gpucloud cron run <job_id>            # Trigger immediate execution
gpucloud cron remove <job_id>         # Delete a job
```
