...
Used by ``gpucloud profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``gpucloud_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the GPUCLOUD Agent kanban board.
...
Rules:
  ...
  - Never write "GPUCLOUD Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_gpucloud_home())
        else:
