    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_gpucloud_home()}/gpucloud-agent && npm install")
                if result.stderr:
        ...
        if not camofox_dir.exists() and _npm_bin:
            ...
            # package.json so that `gpucloud update` does not silently pull
...
def _normalize_toolset_name(name: object) -> str:
    """Return the canonical GPUCLOUD toolset name for config values."""
    value = str(name)
    if value.startswith("hermes-"):
        return "gpucloud-" + value[len("hermes-"):]
    return value
...
def _get_platform_tools(
    ...
    *,
    ...
    toolset_names = [_normalize_toolset_name(ts) for ts in toolset_names]
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (hermes-cli, gpucloud-telegram, etc.)
    ...
    if not isinstance(existing_toolsets, list):
        existing_toolsets = []
    existing_toolsets = [_normalize_toolset_name(ts) for ts in existing_toolsets]
