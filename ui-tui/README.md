...
React + Ink terminal UI for GPUCLOUD. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `GPUCLOUD_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.gpucloud/.gpucloud_history` or under `GPUCLOUD_HOME`.
...
```text
ui-tui/
  packages/gpucloud-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      gpucloud-ink.d.ts    type declarations for @gpucloud/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent GPUCLOUDCLI subprocess for slash commands
```
