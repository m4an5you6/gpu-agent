...
//!
//! Mirrors `gpucloud_constants.get_gpucloud_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\gpucloud
//!   macOS:   ~/.gpucloud
//!   Linux:   ~/.gpucloud  (override via $GPUCLOUD_HOME)
//!
//! NOTE (macOS): Python's get_gpucloud_home(), scripts/install.sh, and the
//! Electron desktop's resolveGPUCLOUDHome() ALL use ~/.gpucloud on macOS — there
...
/// Returns the canonical GPUCLOUD home directory, respecting $GPUCLOUD_HOME if set.
pub fn gpucloud_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("GPUCLOUD_HOME") {
    ...
    {
        // %LOCALAPPDATA%\gpucloud — matches scripts/install.ps1's $GPUCLOUDHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("gpucloud");
        }
    }

    // macOS + Linux + fallback: ~/.gpucloud (matches Python get_gpucloud_home(),
    // install.sh, and the Electron desktop's resolveGPUCLOUDHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".gpucloud");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".gpucloud")
}

pub fn log_dir() -> PathBuf {
    gpucloud_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    gpucloud_home().join("bootstrap-cache")
}
...
/// GPUCLOUD_HOME so it survives repo checkout deletion (unlike anything under
/// gpucloud-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\gpucloud\gpucloud-setup.exe`; on other
...
pub fn copy_self_to_gpucloud_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to GPUCLOUD_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_GPUCLOUD_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".gpucloud-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under GPUCLOUD_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[gpucloud-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("GPUCLOUD_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_gpucloud_home() -> String {
    gpucloud_home().to_string_lossy().into_owned()
}
