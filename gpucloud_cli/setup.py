"""
Interactive setup wizard for GPUCLOUD Agent.
...
_DOCS_BASE = "https://gpucloud-agent.nousresearch.com/docs"
...
# display_gpucloud_home imported lazily at call sites (stale-module safety during gpucloud update)
...
from gpucloud_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from gpucloud_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ GPUCLOUD Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure GPUCLOUD using environment variables or config commands:")
    print_info("  gpucloud config set model.provider custom")
    print_info("  gpucloud config set model.base_url http://localhost:8080/v1")
    print_info("  gpucloud config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'gpucloud setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from gpucloud_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'gpucloud setup')")


def _print_setup_summary(config: dict, gpucloud_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'gpucloud setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from gpucloud_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `gpucloud tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from gpucloud_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'gpucloud setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'gpucloud setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'gpucloud setup terminal'"))
    ...
    # Spotify (OAuth via gpucloud auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'gpucloud setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {gpucloud_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('gpucloud setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('gpucloud setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('gpucloud setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('gpucloud setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('gpucloud setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('gpucloud config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('gpucloud config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('gpucloud config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('gpucloud', Colors.GREEN)}              Start chatting")
    print(f"   {color('gpucloud gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('gpucloud doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `gpucloud tools` and `gpucloud setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``gpucloud model``)
    ...
    provider added to ``gpucloud model`` is automatically available here.
    ...
    # Delegate to the shared gpucloud model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from gpucloud_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: gpucloud model")
    ...
    # on demand via `gpucloud auth add`, `gpucloud setup` vision, and
    # `gpucloud setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``gpucloud model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from gpucloud_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.gpucloud/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the GPUCLOUD auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via gpucloud setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'gpucloud setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where GPUCLOUD runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `gpucloud setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `gpucloud setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into GPUCLOUD_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("GPUCLOUD_MAX_ITERATIONS")
    ...
    print_info("  Run `gpucloud setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # GPUCLOUD_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from gpucloud_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
    ...
    try:
        ...
        if "/profiles/" in gpucloud_home:
            profile_name = gpucloud_home.rstrip("/").rsplit("/", 1)[-1]
    except Exception:
        pass
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where GPUCLOUD delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `gpucloud update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://gpucloud-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under GPUCLOUD_HOME, and print
    ...
    """
    try:
        from gpucloud_cli.slack_cli import _build_full_manifest
        from gpucloud_constants import get_gpucloud_home

        manifest = _build_full_manifest(
            bot_name="GPUCLOUD",
            bot_description="Your GPUCLOUD agent on Slack",
        )
        target = Path(get_gpucloud_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `gpucloud slack manifest --write` anytime to refresh after "
            "GPUCLOUD adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where GPUCLOUD delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects GPUCLOUD to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from gpucloud_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://gpucloud-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://gpucloud-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  gpucloud config edit")
    print_info("   Open config in your editor:  gpucloud config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from gpucloud_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with GPUCLOUD from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'gpucloud setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     gpucloud config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_gpucloud_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  gpucloud gateway")
        else:
            from gpucloud_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   gpucloud gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   gpucloud gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `gpucloud setup tools` and `gpucloud tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``gpucloud_cli.auth`` — lists every supported
    ...
    """
    try:
        from gpucloud_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from gpucloud_cli.config
    ...
    if section_key == "model":
        ...
        from gpucloud_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_gpucloud.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_gpucloud migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_gpucloud", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and GPUCLOUD.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure GPUCLOUD to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point GPUCLOUD at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point GPUCLOUD at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point GPUCLOUD at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point GPUCLOUD at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to GPUCLOUD equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing GPUCLOUD config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in GPUCLOUD.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ GPUCLOUD's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(gpucloud_home: Path) -> bool:
    ...
    print_info("GPUCLOUD can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: gpucloud claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=gpucloud_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: gpucloud claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing GPUCLOUD configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing GPUCLOUD config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in GPUCLOUD (use gpucloud claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``gpucloud setup --portal`` and ``gpucloud portal``. This is the
    ...
    GPUCLOUD session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``gpucloud setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``gpucloud model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``gpucloud portal`` always offers a model picker,
    ...
    print(color("│     ⚕ GPUCLOUD Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `gpucloud portal` == quick setup's Nous step.
    try:
        from gpucloud_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `gpucloud portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `gpucloud portal`: %s", exc)
        print()
    ...
    print_info("  Run `gpucloud portal info` to inspect routing.")
    print_info("  Run `gpucloud` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      gpucloud setup           — full or quick (auto-detected)
      gpucloud setup model     — just model/provider
      gpucloud setup tts       — just text-to-speech
      gpucloud setup terminal  — just terminal backend
      gpucloud setup gateway   — just messaging platforms
      gpucloud setup tools     — just tool configuration
      gpucloud setup agent     — just agent settings
    """
    from gpucloud_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ GPUCLOUD Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ GPUCLOUD Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your GPUCLOUD Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, gpucloud_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have GPUCLOUD configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'gpucloud setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(gpucloud_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up GPUCLOUD?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, gpucloud_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {gpucloud_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'gpucloud config edit'")
    ...
    # Tune later with `gpucloud setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, gpucloud_home)


def _run_first_time_quick_setup(config: dict, gpucloud_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``gpucloud setup <section>``
    or switch providers with ``gpucloud model``.
    """
    from gpucloud_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'gpucloud setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    gpucloud setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  gpucloud setup gateway")
    print()

    _print_setup_summary(config, gpucloud_home)


def _run_quick_setup(config: dict, gpucloud_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'gpucloud setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect GPUCLOUD to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'gpucloud setup gateway'.")
