"""
Configuration management for GPUCLOUD Agent.

Config files are stored in ~/.gpucloud/ for easy access:
- ~/.gpucloud/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.gpucloud/.env         - API keys and secrets

This module provides:
- gpucloud config          - Show current configuration
- gpucloud config edit     - Open config in editor
- gpucloud config set      - Set a specific value
- gpucloud config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``gpucloud config set`` (which rewrites
    ...
    ``config.yaml`` in place: gpucloud never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.gpucloud/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``gpucloud logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  gpucloud config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` GPUCLOUD makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. GPUCLOUD itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``gpucloud update``, the TUI build.
...
#   on every plugin install / ``gpucloud update``.
...
# * ``GPUCLOUD_HOME`` / ``GPUCLOUD_PROFILE`` / ``GPUCLOUD_CONFIG`` /
#   ``GPUCLOUD_ENV`` — GPUCLOUD runtime location flags. Writing these into
...
#
# IMPORTANT: ``GPUCLOUD_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (GPUCLOUD_GEMINI_CLIENT_ID,
# GPUCLOUD_LANGFUSE_PUBLIC_KEY, GPUCLOUD_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # GPUCLOUD runtime location — never via dashboard env writer.
    # NOT a GPUCLOUD_* blanket: integration credentials (GPUCLOUD_GEMINI_*,
    # GPUCLOUD_LANGFUSE_*, GPUCLOUD_SPOTIFY_*, ...) ARE allowed.
    "GPUCLOUD_HOME", "GPUCLOUD_PROFILE", "GPUCLOUD_CONFIG", "GPUCLOUD_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or GPUCLOUD runtime location "
            "(GPUCLOUD_HOME, GPUCLOUD_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.gpucloud/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `gpucloud plugins enable
    # observability/langfuse` or `gpucloud tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "GPUCLOUD_LANGFUSE_ENV",
    "GPUCLOUD_LANGFUSE_RELEASE",
    "GPUCLOUD_LANGFUSE_SAMPLE_RATE",
    "GPUCLOUD_LANGFUSE_MAX_CHARS",
    "GPUCLOUD_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from gpucloud_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("GPUCLOUD_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_gpucloud_home() / ".managed"
...
def is_managed() -> bool:
    """Check if GPUCLOUD is running in package-manager-managed mode.

    Two signals: the GPUCLOUD_MANAGED env var (set by the systemd service),
    or a .managed marker file in GPUCLOUD_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade gpucloud-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how GPUCLOUD was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.gpucloud/.install_method`` file (written by installers)
    2. GPUCLOUD_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``nousresearch/gpucloud-agent`` image stamps ``docker``
    ...
    so ``gpucloud update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_gpucloud_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.gpucloud/.install_method."""
    stamp = get_gpucloud_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* GPUCLOUD lives in a ``uv tool`` layout.

    ``uv tool install gpucloud-agent`` places the install at
    ``.../uv/tools/gpucloud-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``gpucloud-agent`` happens to be uv-tool-installed on the machine while
    the *active* GPUCLOUD is a regular pip/venv install, causing
    ``gpucloud update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/gpucloud-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade gpucloud-agent"
    if method == "docker":
        return "docker pull nousresearch/gpucloud-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade gpucloud-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade gpucloud-agent"
        return "pip install --upgrade gpucloud-agent"
    return "gpucloud update"
...
# Long-form text for ``gpucloud update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# gpucloud_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side GPUCLOUD, it doesn't update
...
✗ ``gpucloud update`` doesn't apply inside the Docker container.

GPUCLOUD Agent runs as a published image (nousresearch/gpucloud-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/gpucloud-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate gpucloud-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/gpucloud-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/gpucloud-agent/tags
  • Your config and session history live under ``$GPUCLOUD_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``gpucloud update`` inside Docker.
...
def format_managed_message(action: str = "modify this GPUCLOUD installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("GPUCLOUD_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this GPUCLOUD installation is managed by NixOS "
            f"(GPUCLOUD_MANAGED={env_hint}).\n"
            "Edit services.gpucloud-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this GPUCLOUD installation is managed by Homebrew "
            f"(GPUCLOUD_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade gpucloud-agent"
        )

    return (
        f"Cannot {action}: this GPUCLOUD installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall GPUCLOUD."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from GPUCLOUD_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, gpucloud_bin
    or None if container mode is not active, we're already inside the
    container, or GPUCLOUD_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("GPUCLOUD_DEV") == "1":
        return None
    ...
    container_mode_file = get_gpucloud_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "gpucloud-agent")
    exec_user = info.get("exec_user", "gpucloud")
    gpucloud_bin = info.get("gpucloud_bin", "/data/current-package/bin/gpucloud")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "gpucloud_bin": gpucloud_bin,
    }
...
# Re-export from gpucloud_constants — canonical definition lives there.
from gpucloud_constants import get_gpucloud_home  # noqa: F811,E402
...
def _resolve_gpucloud_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the GPUCLOUD_UID / GPUCLOUD_GID env vars set by Docker deployments.

    Docker containers running GPUCLOUD commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level GPUCLOUD_HOME once, but
    subdirectories created at runtime by ``ensure_gpucloud_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("GPUCLOUD_UID", "").strip()
    gid_str = os.environ.get("GPUCLOUD_GID", "").strip()
    try:
...
def _chown_to_gpucloud_uid(path) -> None:
    """Chown ``path`` to ``GPUCLOUD_UID:GPUCLOUD_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_gpucloud_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_gpucloud_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the gpucloud group can
    share state with the gateway service.

    The mode can be overridden via the GPUCLOUD_HOME_MODE environment variable
    (e.g. GPUCLOUD_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse GPUCLOUD_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``GPUCLOUD_UID``/``GPUCLOUD_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("GPUCLOUD_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_gpucloud_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When GPUCLOUD runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("GPUCLOUD_CONTAINER") or os.environ.get("GPUCLOUD_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set GPUCLOUD_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into GPUCLOUD_HOME if the user doesn't have one yet."""
...
def ensure_gpucloud_home():
    """Ensure ~/.gpucloud directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_gpucloud_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_gpucloud_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"GPUCLOUD_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the GPUCLOUD-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps GPUCLOUD
        ...
        # identity slot (SOUL.md). Empty by default. The GPUCLOUD_ENVIRONMENT_HINT
    ...
    "terminal": {
        ...
        # skipped. When empty, GPUCLOUD auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), GPUCLOUD sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when GPUCLOUD
        ...
        # bundled GPUCLOUD image, which drops to the `gpucloud` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, GPUCLOUD sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and GPUCLOUD should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``gpucloud chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.gpucloud/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: gpucloud sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, GPUCLOUD truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``gpucloud kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``gpucloud kanban decompose`` and the kanban
        ...
        # ``gpucloud profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `gpucloud model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `gpucloud` (and `gpucloud chat`) launches by default:
        ...
        # REPL and `--tui` (or GPUCLOUD_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `gpucloud --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `gpucloud -c` muscle memory.  Default off so existing
        # users aren't surprised.  GPUCLOUD_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `gpucloud --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/gpucloud`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``GPUCLOUD_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``GPUCLOUD_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``GPUCLOUD_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``GPUCLOUD_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/gpucloud``) —
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.gpucloud/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.gpucloud/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, GPUCLOUD feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before GPUCLOUD auto-pauses the goal and
    ...
    "skills": {
        ...
        # Substitute ${GPUCLOUD_SKILL_DIR} and ${GPUCLOUD_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `gpucloud curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `gpucloud update`, so pruning them only
        ...
        # skipped), snapshot ~/.gpucloud/skills/ into
        # ~/.gpucloud/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `gpucloud curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for gpucloud-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *GPUCLOUD Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (GPUCLOUD_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.gpucloud/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or GPUCLOUD_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `gpucloud doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``gpucloud_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow GPUCLOUD to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via GPUCLOUD_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `gpucloud -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `gpucloud` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `gpucloud kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     gpucloud-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core GPUCLOUD tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.gpucloud/logs/.
    ...
    # update model picker lists without shipping a gpucloud-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://gpucloud-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `gpucloud model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.gpucloud/.env,
        ...
        # behavior: files must live under the GPUCLOUD cache, under
        ...
        # user. Bridged to GPUCLOUD_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the GPUCLOUD
        # cache (~/.gpucloud/cache/{documents,images,audio,video,screenshots})
        ...
        # to GPUCLOUD_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # GPUCLOUD_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to GPUCLOUD_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.gpucloud/state.db.
    ...
    "sessions": {
        ...
        # default of ``gpucloud sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.gpucloud/sessions/session_{sid}.json`` on every turn
    ...
    # ``gpucloud update`` behaviour.
    "updates": {
        # Run a full ``gpucloud backup``-style zip of GPUCLOUD_HOME before every
        # ``gpucloud update``.  Backups land in ``<GPUCLOUD_HOME>/backups/`` and
        # can be restored with ``gpucloud import <path>``.  Off by default —
        # on large GPUCLOUD_HOME directories the zip can add minutes to every
        ...
        "backup_keep": 5,
        # What `gpucloud update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<GPUCLOUD_HOME>/lsp/bin/`` on first use.
    ...
    # `gpucloud tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.gpucloud/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.gpucloud/.env (or your shell) and never in config.yaml.
            ...
            # ~/.gpucloud/bin/ on first use.  When False you must install
            ...
            # `gpucloud secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'gpucloud model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the GPUCLOUD auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @gpucloud:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. GPUCLOUD_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #gpucloud)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: gpucloud-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'gpucloud-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote GPUCLOUD API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote GPUCLOUD API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote GPUCLOUD API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # GPUCLOUD_TOOL_PROGRESS and GPUCLOUD_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "GPUCLOUD_TOOL_PROGRESS": {
    ...
    "GPUCLOUD_PREFILL_MESSAGES_FILE": {
    ...
    "GPUCLOUD_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.gpucloud.config`` entries, then checks
    ...
    try:
        ...
        # should never break `gpucloud update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older GPUCLOUD versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``gpucloud_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'gpucloud setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — GPUCLOUD won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'gpucloud doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("GPUCLOUD_HOME", "~/.gpucloud")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("GPUCLOUD_TOOL_PROGRESS")
            old_mode = get_env_value("GPUCLOUD_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from GPUCLOUD_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from GPUCLOUD_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("GPUCLOUD_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from GPUCLOUD_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$GPUCLOUD_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_gpucloud_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `gpucloud plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `gpucloud curator status` has no
    ...
    #   3. Creates `~/.gpucloud/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_gpucloud_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_gpucloud_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: gpucloud config set <key> <value>")
    ...
    # metadata.gpucloud.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _gpucloud_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.gpucloud/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.gpucloud/config.yaml.
    ...
    (which change ``GPUCLOUD_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — gpucloud auth) — OpenAI Codex
#   nous         (OAuth — gpucloud auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.gpucloud/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.gpucloud/.env.
    ...
    interactive menu render (`gpucloud tools`, `gpucloud setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `gpucloud tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real GPUCLOUD env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.gpucloud/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.gpucloud/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.gpucloud/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.gpucloud/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to GPUCLOUD — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known GPUCLOUD vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.gpucloud/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ GPUCLOUD Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Warn on stale GPUCLOUD_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("GPUCLOUD_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale GPUCLOUD_MAX_ITERATIONS={_env_ghost} "
                f"(run 'gpucloud doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  gpucloud config edit     # Edit config file", Colors.DIM))
    print(color("  gpucloud config set <key> <value>", Colors.DIM))
    print(color("  gpucloud setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: gpucloud config set <key> <value>")
            print()
            print("Examples:")
            print("  gpucloud config set model anthropic/claude-sonnet-4")
            print("  gpucloud config set terminal.backend docker")
            print("  gpucloud config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
    ...
    elif subcmd == "migrate-home":
        from gpucloud_constants import migrate_hermes_home_to_gpucloud_home

        result = migrate_hermes_home_to_gpucloud_home(
            src=getattr(args, "src", None),
            dst=getattr(args, "dst", None),
            dry_run=bool(getattr(args, "dry_run", False)),
        )
        action = "Would copy" if getattr(args, "dry_run", False) else "Copied"
        ...
        if missing_config:
            ...
            print("    Run 'gpucloud config migrate' to add them")
        ...
        print("  gpucloud config           Show current configuration")
        print("  gpucloud config edit      Open config in editor")
        print("  gpucloud config set <key> <value>   Set a config value")
        print("  gpucloud config check     Check for missing/outdated config")
        print("  gpucloud config migrate   Update config with new options")
        print("  gpucloud config path      Show config file path")
        print("  gpucloud config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``gpucloud config`` UI so users
