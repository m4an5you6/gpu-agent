...
# shellcheck shell=sh
# /opt/gpucloud/docker/main-wrapper.sh — wraps the container's CMD with
...
drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid gpucloud "$@"; exec "$@"; }
...
# an arbitrary non-root, non-gpucloud UID, the bootstrap was skipped and the
# baked image dirs (owned by the gpucloud build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u gpucloud)" ]; then
    cat >&2 <<EOF
[gpucloud] ERROR: container started with --user $cur_uid (an arbitrary, non-gpucloud UID) — not supported.
...
_gpucloud_orig_cwd="${GPUCLOUD_ORIG_CWD:-$PWD}"
...
# the user's command so `gpucloud chat` starts in the Docker -w
# directory, not /opt/data.
cd "$_gpucloud_orig_cwd"
