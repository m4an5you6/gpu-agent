...
on Japanese locales, etc.).  GPUCLOUD's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``gpucloud_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. GPUCLOUD is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``GPUCLOUD_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("GPUCLOUD_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the GPUCLOUD-managed Git install directories so
    ...
    # already-running shells don't see the change — which means gpucloud
    ...
    # install without GPUCLOUD-managed PortableGit).
    ...
    # stdin is re-configured for completeness; GPUCLOUD's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and GPUCLOUD's
    ``gpucloud config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching GPUCLOUD (User env var in Windows Settings, or
    export in a PowerShell profile) and GPUCLOUD picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known GPUCLOUD-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\gpucloud\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``gpucloud`` from right after install) retain their old PATH.

    Any subprocess GPUCLOUD spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known GPUCLOUD-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "gpucloud", "git", "cmd"),
        os.path.join(local_appdata, "gpucloud", "git", "bin"),
        os.path.join(local_appdata, "gpucloud", "git", "usr", "bin"),
        # GPUCLOUD venv Scripts directory — host of the gpucloud.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes gpucloud, but harmless to include.
        os.path.join(local_appdata, "gpucloud", "gpucloud-agent", "venv", "Scripts"),
