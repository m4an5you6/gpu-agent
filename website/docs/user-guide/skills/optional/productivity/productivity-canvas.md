...
| | |
|---|---|
| Source | Optional — install with `gpucloud skills install official/productivity/canvas` |
...
4. Name the token (e.g., "GPUCLOUD Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.gpucloud/.env`:
...
```bash
CANVAS="python $GPUCLOUD_HOME/skills/productivity/canvas/scripts/canvas_api.py"
