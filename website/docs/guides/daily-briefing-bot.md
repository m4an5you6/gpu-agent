...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `gpucloud setup --portal`. See [Nous Portal](/integrations/nous-portal).
:::
...
2. **GPUCLOUD spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.gpucloud/cron/output/` and you can read them anytime.
:::
...
GPUCLOUD will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by GPUCLOUD Agent
```
...
Before creating cron jobs, ensure GPUCLOUD has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell GPUCLOUD what you want:
...
GPUCLOUD will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell GPUCLOUD to delegate each topic to a sub-agent:
...
GPUCLOUD will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
