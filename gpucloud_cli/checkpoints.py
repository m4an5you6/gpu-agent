"""`gpucloud checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.gpucloud/checkpoints/``.  Actions:

    gpucloud checkpoints               # same as `status`
    gpucloud checkpoints status        # total size, project count, breakdown
    gpucloud checkpoints list          # per-project checkpoint counts + workdir
    gpucloud checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    gpucloud checkpoints clear [-f]    # nuke the entire base (asks first)
    gpucloud checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    gpucloud checkpoints
    gpucloud checkpoints prune --retention-days 3 --max-size-mb 200
    gpucloud checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: gpucloud checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``gpucloud checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `gpucloud checkpoints` → status
