#!/usr/bin/env python3
"""Google Workspace API CLI for GPUCLOUD Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing GPUCLOUD-facing JSON contract and falls back to the Python client
...
from _gpucloud_home import get_gpucloud_home
...
def _gws_binary() -> str | None:
    override = os.getenv("GPUCLOUD_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for GPUCLOUD Agent")
