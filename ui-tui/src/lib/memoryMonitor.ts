...
// Deferred @gpucloud/ink import: loading `@gpucloud/ink` at module top-level
...
async function _ensureEvictInkCaches(): Promise<(level: 'all' | 'half') => unknown> {
  ...
  _evictInkCachesPromise ??= import('@gpucloud/ink')
    .then(mod => {
...
export function startMemoryMonitor({
  ...
  const cooldownRaw = process.env.GPUCLOUD_AUTO_HEAPDUMP_COOLDOWN_MS?.trim()
  ...
  const tick = async () => {
    ...
    // Deferred import keeps `@gpucloud/ink` off the cold-start critical path;
