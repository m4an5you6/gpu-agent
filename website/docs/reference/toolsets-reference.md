...
| `kanban` | `kanban_block`, `kanban_comment`, `kanban_complete`, `kanban_create`, `kanban_heartbeat`, `kanban_link`, `kanban_list`, `kanban_show`, `kanban_unblock` | Multi-agent coordination tools. Registered for dispatcher-spawned task workers (`GPUCLOUD_KANBAN_TASK`) and for profiles that explicitly enable the `kanban` toolset. Workers mark tasks done, block, heartbeat, comment, and create/link follow-up tasks; orchestrator profiles additionally get board-routing tools like list/unblock. |
...
| `gpucloud-telegram` | Same as `hermes-cli`. |
| `gpucloud-discord` | Adds `discord` and `discord_admin` on top of `hermes-cli`. |
| `gpucloud-slack` | Same as `hermes-cli`. |
| `gpucloud-whatsapp` | Same as `hermes-cli`. |
...
| `gpucloud-gateway` | Internal gateway orchestrator toolset — union of every `hermes-<platform>` toolset; used when the gateway needs to accept any message source. |
