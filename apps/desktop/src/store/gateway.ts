import type { ConnectionState, GatewayEvent } from '@gpucloud/shared'
...
const isOpen = (gateway: GPUCLOUDGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<GPUCLOUDGateway | null>(null)
...
let primaryGateway: GPUCLOUDGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: GPUCLOUDGateway | null, profile = 'default'): void {
...
export function activeGateway(): GPUCLOUDGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<GPUCLOUDGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
