...
def test_init_env_args_uses_gpucloud_dotenv_for_allowlisted_env(monkeypatch):
    ...
    monkeypatch.setattr(docker_env, "_load_gpucloud_env_vars", lambda: {"DATABASE_URL": "value_from_dotenv"})
...
def test_init_env_args_prefers_shell_env_over_gpucloud_dotenv(monkeypatch):
...
def test_init_env_args_uses_gpucloud_dotenv_for_empty_shell_env(monkeypatch):
    ...
    MY_SECRET=`, clobbering the correct value sitting in ~/.gpucloud/.env.
    """
    env = _make_execute_only_env(["MY_SECRET"])

    monkeypatch.setenv("MY_SECRET", "")
    monkeypatch.setattr(docker_env, "_load_gpucloud_env_vars", lambda: {"MY_SECRET": "value_from_dotenv"})
...
def test_init_env_args_never_forwards_blank_secret(monkeypatch):
    ...
    monkeypatch.setenv("MY_SECRET", "")
    monkeypatch.setattr(docker_env, "_load_gpucloud_env_vars", lambda: {})
...
def test_run_command_tags_gpucloud_agent_label(monkeypatch):
...
def test_run_command_tags_task_and_profile_labels(monkeypatch):
    ...
    labels = _labels_in_run_args(_run_args_from_calls(calls))
    assert "gpucloud-task-id=kanban-42" in labels, (
        f"gpucloud-task-id=kanban-42 missing; got: {sorted(labels)}"
    )
    assert "gpucloud-profile=research-bot" in labels, (
        f"gpucloud-profile=research-bot missing; got: {sorted(labels)}"
    )
...
def test_run_command_sanitizes_unsafe_task_id(monkeypatch):
    ...
    assert "gpucloud-task-id=task_with_weird_chars" in labels, (
...
def test_labels_attribute_populated_after_init(monkeypatch):
    ...
    assert env._labels == {
        "gpucloud-agent": "1",
        "gpucloud-task-id": "abc",
        "gpucloud-profile": "default",
    }
...
def test_failed_docker_run_cleans_up_orphaned_container(monkeypatch):
    ...
    assert rm_cmd[3].startswith("gpucloud-"), "should remove the container by its generated name"
...
def test_reap_orphan_scopes_to_profile_filter_via_label(monkeypatch):
    """The reaper must pass ``--filter label=gpucloud-profile=<profile>`` to
    ...
    assert "label=gpucloud-profile=research-bot" in flat, (
...
def test_image_uses_init_entrypoint_detects_s6_init(monkeypatch):
    ...
    monkeypatch.setattr(docker_env.subprocess, "run", _run)
    assert docker_env._image_uses_init_entrypoint("/usr/bin/docker", "gpucloud-agent:latest") is True
...
def test_s6_image_skips_docker_init_and_mounts_run_exec(monkeypatch):
    ...
    _make_dummy_env(image="gpucloud-agent:latest")
...
def test_is_container_gone_matches_removal_errors(monkeypatch):
    ...
    assert env._is_container_gone(
        "Error response from daemon: No such container: gpucloud-abc123"
    )
...
def test_execute_recovers_from_out_of_band_removal(monkeypatch):
    ...
    outputs = iter([
        {"output": "Error response from daemon: No such container: gpucloud-x", "returncode": 1},
