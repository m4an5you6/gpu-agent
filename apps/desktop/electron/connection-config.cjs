 ...
 *     `X-GPUCLOUD-Session-Token` header; WS uses `?token=`.
...
// gpucloud_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - gpucloud_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - gpucloud_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / gpucloud #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-gpucloud_session_at', '__Secure-gpucloud_session_at', 'gpucloud_session_at']
const RT_COOKIE_VARIANTS = ['__Host-gpucloud_session_rt', '__Secure-gpucloud_session_rt', 'gpucloud_session_rt']
...
/**
 * True if any cookie in `cookies` is a gpucloud session ACCESS-token cookie
