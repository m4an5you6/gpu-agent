...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("GPUCLOUD_YOLO_MODE", ""))
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    try:
        from gpucloud_cli.plugins import invoke_hook
    except Exception:
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("GPUCLOUD_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("GPUCLOUD_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("GPUCLOUD_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set GPUCLOUD_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind GPUCLOUD_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds GPUCLOUD_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("GPUCLOUD_CRON_SESSION"):
        return False
    if env_var_enabled("GPUCLOUD_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $GPUCLOUD_HOME, or by the resolved absolute
# active profile home path such as /home/gpucloud/.gpucloud/config.yaml. The
# resolved-absolute form is folded into the ~/.gpucloud/ patterns at detection
...
# go stale when GPUCLOUD_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_GPUCLOUD_ENV_PATH = (
    r'(?:~\/\.gpucloud/|'
    r'(?:\$home|\$\{home\})/\.gpucloud/|'
    r'(?:\$gpucloud_home|\$\{gpucloud_home\})/)'
    r'\.env\b'
)
# ~/.gpucloud/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _GPUCLOUD_ENV_PATH; matches the GPUCLOUD_HOME override form as
# well as ~/.gpucloud/.
_GPUCLOUD_CONFIG_PATH = (
    r'(?:~\/\.gpucloud/|'
    r'(?:\$home|\$\{home\})/\.gpucloud/|'
    r'(?:\$gpucloud_home|\$\{gpucloud_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_GPUCLOUD_ENV_PATH}|'
    rf'{_GPUCLOUD_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bgpucloud\s+gateway\s+(stop|restart)\b', "stop/restart gpucloud gateway (kills running agents)"),
    (r'\bgpucloud\s+update\b', "gpucloud update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `gpucloud gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart gpucloud-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart gpucloud-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(gpucloud|gateway|cli\.py)\b', "kill gpucloud/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill gpucloud` but not
    # `kill -9 $(pgrep -f gpucloud)` because the substitution is opaque
    ...
    # In-place edit of a GPUCLOUD-managed security file (~/.gpucloud/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_GPUCLOUD_CONFIG_PATH}|{_GPUCLOUD_ENV_PATH})', "in-place edit of GPUCLOUD config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_GPUCLOUD_CONFIG_PATH}|{_GPUCLOUD_ENV_PATH})', "in-place edit of GPUCLOUD config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_GPUCLOUD_CONFIG_PATH}|{_GPUCLOUD_ENV_PATH})', "in-place edit of GPUCLOUD config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.gpucloud/ form so the GPUCLOUD config/env patterns catch it. In Docker and
    # gateway deployments the agent often references the resolved absolute path
    # directly (e.g. `sed -i ... /home/gpucloud/.gpucloud/config.yaml`) rather than
    # ~, $HOME, or $GPUCLOUD_HOME. Done at detection time (not via an import-time
    # pattern snapshot) so it tracks the live GPUCLOUD_HOME even when that is set
    # after this module is imported — as the hermetic test conftest does.
    command = _rewrite_resolved_gpucloud_home(command)
    return command


def _rewrite_resolved_gpucloud_home(command: str) -> str:
    """Rewrite the resolved absolute GPUCLOUD home prefix to ``~/.gpucloud/``.

    Resolves the active ``GPUCLOUD_HOME`` at call time (and its symlink-resolved
    form) and replaces an occurrence of ``<home>/`` in *command* with
    ``~/.gpucloud/`` so the static ``_GPUCLOUD_CONFIG_PATH`` / ``_GPUCLOUD_ENV_PATH``
    patterns match. No-op when the path can't be resolved or doesn't appear.
    """
    try:
        from gpucloud_constants import get_gpucloud_home
        home = get_gpucloud_home().expanduser()
        candidates = [
            str(home).rstrip("/"),
    ...
    for path in candidates:
        if not path or path in seen:
            continue
        seen.add(path)
        # Guard against a degenerate GPUCLOUD_HOME (e.g. "/" or "") rewriting
        ...
        # /home/gpucloud/.gpucloud or a per-test tempdir, never a bare root.
        normalized = path.rstrip("/")
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.gpucloud/")
    return command
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["GPUCLOUD_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "GPUCLOUD_SPINNER_PAUSE" in os.environ:
            del os.environ["GPUCLOUD_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set GPUCLOUD_INTERACTIVE or GPUCLOUD_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("GPUCLOUD_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("GPUCLOUD_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("GPUCLOUD_EXEC_ASK")
