...
Auto-tracks and cleans up ephemeral files created during GPUCLOUD Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$GPUCLOUD_HOME` and `/tmp/gpucloud-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `GPUCLOUD_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `GPUCLOUD_HOME` or `/tmp/gpucloud-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$GPUCLOUD_HOME/disk-cleanup/` is itself excluded
- `$GPUCLOUD_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
