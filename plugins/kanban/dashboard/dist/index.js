/**
 * GPUCLOUD Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.gpucloud/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__GPUCLOUD_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__GPUCLOUD_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "gpucloud-kanban-dot-triage",
    todo: "gpucloud-kanban-dot-todo",
    ready: "gpucloud-kanban-dot-ready",
    running: "gpucloud-kanban-dot-running",
    blocked: "gpucloud-kanban-dot-blocked",
    done: "gpucloud-kanban-dot-done",
    archived: "gpucloud-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-gpucloud-task";
  ...
  const DOCS_URL = "https://gpucloud-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://gpucloud-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "gpucloud.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="gpucloud-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "gpucloud-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "gpucloud-kanban-md",
  ...
  // standard `drop` event and our `gpucloud-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("gpucloud-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("gpucloud-kanban-column--drop");
          if (target) target.classList.add("gpucloud-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("gpucloud-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("gpucloud-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("gpucloud-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__GPUCLOUD_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "gpucloud-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "gpucloud-kanban-attention",
        "gpucloud-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "gpucloud-kanban-attention-bar" },
        h("span", { className: "gpucloud-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "gpucloud-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "gpucloud-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "gpucloud-kanban-attention-row",
                  "gpucloud-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "gpucloud-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "gpucloud-kanban-attention-row-id" }, task.id),
                h("span", { className: "gpucloud-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "gpucloud-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "gpucloud-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "gpucloud-kanban-diag-action-btn",
      action.suggested ? "gpucloud-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " gpucloud-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".gpucloud-kanban-drawer-comment-row input, .gpucloud-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "gpucloud-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("gpucloud-kanban-diag", sevClass) },
      h("div", { className: "gpucloud-kanban-diag-header" },
        h("span", { className: "gpucloud-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "gpucloud-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "gpucloud-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "gpucloud-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "gpucloud-kanban-diag-data-row" },
                  h("span", { className: "gpucloud-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "gpucloud-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "gpucloud-kanban-diag-data-row" },
                h("span", { className: "gpucloud-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "gpucloud-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "gpucloud-kanban-diag-reassign-row" },
            h("span", { className: "gpucloud-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "gpucloud-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "gpucloud-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "gpucloud-kanban-diag-msg",
              msg.ok ? "gpucloud-kanban-diag-msg--ok" : "gpucloud-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "gpucloud-kanban-section" },
      h("div", { className: "gpucloud-kanban-section-head-row" },
        h("span", { className: "gpucloud-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "gpucloud-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "gpucloud-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "gpucloud-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "gpucloud-kanban-docs-link",
      title: "Open GPUCLOUD Kanban docs in a new tab",
      "aria-label": "GPUCLOUD Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `gpucloud kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "gpucloud-kanban-msg-ok" : "gpucloud-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "gpucloud-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "gpucloud-kanban-boardswitcher" },
      h("div", { className: "gpucloud-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "gpucloud-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "gpucloud-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "gpucloud-kanban-dialog-title" },
        ...
        h("div", { className: "gpucloud-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned GPUCLOUD profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "gpucloud-kanban-bulk" },
      h("span", { className: "gpucloud-kanban-bulk-count" },
      ...
      h("div", { className: "gpucloud-kanban-bulk-priority",
      ...
      h("div", { className: "gpucloud-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different GPUCLOUD profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "gpucloud-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("gpucloud-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("gpucloud-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "gpucloud-kanban-trash",
        dragOver ? "gpucloud-kanban-trash--drop" : "",
        props.draggingTaskId ? "gpucloud-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "gpucloud-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "gpucloud-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".gpucloud-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "gpucloud-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("gpucloud-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("gpucloud-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "gpucloud-kanban-column",
        dragOver ? "gpucloud-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "gpucloud-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "gpucloud-kanban-col-check",
        ...
        h("span", { className: cn("gpucloud-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "gpucloud-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "gpucloud-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "gpucloud-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "gpucloud-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "gpucloud-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "gpucloud-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "gpucloud-kanban-lane" },
                  h("div", { className: "gpucloud-kanban-lane-head" },
                    h("span", { className: "gpucloud-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "gpucloud-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "gpucloud-kanban-card--stale-red";
    if (age >= tier.amber) return "gpucloud-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".gpucloud-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "gpucloud-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      "data-task-id": t.id,
      className: cn(
        "gpucloud-kanban-card",
        props.selected ? "gpucloud-kanban-card--selected" : "",
        props.failed ? "gpucloud-kanban-card--failed" : "",
        props.draggingSource ? "gpucloud-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "gpucloud-kanban-card-content" },
          h("div", { className: "gpucloud-kanban-card-row" },
            h("label", {
              className: "gpucloud-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "gpucloud-kanban-card-check",
            ...
            h("span", { className: "gpucloud-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or gpucloud kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "gpucloud-kanban-warning-badge",
                    "gpucloud-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "gpucloud-kanban-needs-assignee",
          ...
          h("div", { className: "gpucloud-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "gpucloud-kanban-card-row gpucloud-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "gpucloud-kanban-assignee",
                            title: `Assigned to GPUCLOUD profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "gpucloud-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "gpucloud-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "gpucloud-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "gpucloud-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "GPUCLOUD profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "GPUCLOUD profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "gpucloud-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "gpucloud-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "gpucloud-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "gpucloud-kanban-drawer-close",
        ...
        data ? h("div", { className: "gpucloud-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "gpucloud-kanban-section" },
      h("div", { className: "gpucloud-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "gpucloud-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "gpucloud-kanban-drawer-body" },
      h("div", { className: "gpucloud-kanban-drawer-title" },
        h("span", { className: cn("gpucloud-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "gpucloud-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "gpucloud-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "gpucloud-kanban-section" },
        h("div", { className: "gpucloud-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "gpucloud-kanban-section" },
        h("div", { className: "gpucloud-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "gpucloud-kanban-comment" },
                h("div", { className: "gpucloud-kanban-comment-head" },
                  h("span", { className: "gpucloud-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "gpucloud-kanban-comment-ago" },
      ...
      h("div", { className: "gpucloud-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "gpucloud-kanban-event",
              isDiag ? "gpucloud-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "gpucloud-kanban-event-header" },
                  h("span", { className: "gpucloud-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "gpucloud-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "gpucloud-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "gpucloud-kanban-event-header-plain" },
                  h("span", { className: "gpucloud-kanban-event-kind" }, e.kind),
                  h("span", { className: "gpucloud-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "gpucloud-kanban-event-phantom-row" },
                  h("span", { className: "gpucloud-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "gpucloud-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "gpucloud-kanban-event-payload" },
    ...
    return h("div", { className: "gpucloud-kanban-section" },
      h("div", { className: "gpucloud-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "gpucloud-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `gpucloud-kanban-run--${r.outcome || r.status || "ended"}`
          : "gpucloud-kanban-run--active";
        return h("div", { key: r.id, className: cn("gpucloud-kanban-run", outcomeClass) },
          h("div", { className: "gpucloud-kanban-run-head" },
            h("span", { className: "gpucloud-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "gpucloud-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "gpucloud-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "gpucloud-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "gpucloud-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "gpucloud-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "gpucloud-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "gpucloud-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "gpucloud-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "gpucloud-kanban-pre gpucloud-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "gpucloud-kanban-meta-row" },
      h("span", { className: "gpucloud-kanban-meta-label" }, props.label),
      h("span", { className: "gpucloud-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "gpucloud-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "gpucloud-kanban-meta-row" },
        h("span", { className: "gpucloud-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "gpucloud-kanban-meta-value gpucloud-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "gpucloud-kanban-meta-row" },
        h("span", { className: "gpucloud-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "gpucloud-kanban-meta-value gpucloud-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "gpucloud-kanban-section" },
      h("div", { className: "gpucloud-kanban-section-head-row" },
        h("span", { className: "gpucloud-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "gpucloud-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "gpucloud-kanban-section" },
      h("div", { className: "gpucloud-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "gpucloud-kanban-deps-row" },
        h("span", { className: "gpucloud-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "gpucloud-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "gpucloud-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "gpucloud-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "gpucloud-kanban-dep-chip-x",
      ...
      h("div", { className: "gpucloud-kanban-deps-row" },
        h("span", { className: "gpucloud-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "gpucloud-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "gpucloud-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "gpucloud-kanban-msg-ok"
          : "gpucloud-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "gpucloud-kanban-section" },
      ...
      h("div", { className: "gpucloud-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "gpucloud-kanban-home-sub gpucloud-kanban-home-sub--on"
              : "gpucloud-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__GPUCLOUD_PLUGINS__ && typeof window.__GPUCLOUD_PLUGINS__.register === "function") {
    window.__GPUCLOUD_PLUGINS__.register("kanban", KanbanPage);
  }
})();
