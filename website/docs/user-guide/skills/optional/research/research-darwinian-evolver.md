...
| | |
|---|---|
| Source | Optional — install with `gpucloud skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), GPUCLOUD Agent |
| License | MIT |
...
import upstream classes into GPUCLOUD itself.
...
```bash
mkdir -p ~/.gpucloud/cache/darwinian-evolver && cd ~/.gpucloud/cache/darwinian-evolver
...
```bash
cd ~/.gpucloud/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.gpucloud/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.gpucloud/skills/research/darwinian-evolver
DE_DIR=~/.gpucloud/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside GPUCLOUD core.
   Custom driver scripts under `~/.gpucloud/skills/...` are user-side and fine.
