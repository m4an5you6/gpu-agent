"""Shared fixtures for the gpucloud-agent test suite.
...
2. **Isolated GPUCLOUD_HOME.** GPUCLOUD_HOME points to a per-test tempdir so
   code reading ``~/.gpucloud/*`` via ``get_gpucloud_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".gpucloud"`` instead of the canonical
   ``get_gpucloud_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No GPUCLOUD_SESSION_* inheritance** — the agent's current gateway
...
# GPUCLOUD_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_GPUCLOUD_BEHAVIORAL_VARS = frozenset({
    "GPUCLOUD_YOLO_MODE",
    "GPUCLOUD_INTERACTIVE",
    "GPUCLOUD_QUIET",
    "GPUCLOUD_TOOL_PROGRESS",
    "GPUCLOUD_TOOL_PROGRESS_MODE",
    "GPUCLOUD_MAX_ITERATIONS",
    "GPUCLOUD_SESSION_PLATFORM",
    "GPUCLOUD_SESSION_CHAT_ID",
    "GPUCLOUD_SESSION_CHAT_NAME",
    "GPUCLOUD_SESSION_THREAD_ID",
    "GPUCLOUD_SESSION_SOURCE",
    "GPUCLOUD_SESSION_KEY",
    "GPUCLOUD_GATEWAY_SESSION",
    "GPUCLOUD_CRON_SESSION",
    "_GPUCLOUD_GATEWAY",
    "GPUCLOUD_PLATFORM",
    "GPUCLOUD_MODEL",
    "GPUCLOUD_INFERENCE_MODEL",
    "GPUCLOUD_INFERENCE_PROVIDER",
    "GPUCLOUD_TUI_PROVIDER",
    "GPUCLOUD_MANAGED",
    "GPUCLOUD_DEV",
    "GPUCLOUD_CONTAINER",
    "GPUCLOUD_EPHEMERAL_SYSTEM_PROMPT",
    "GPUCLOUD_TIMEZONE",
    "GPUCLOUD_REDACT_SECRETS",
    "GPUCLOUD_BACKGROUND_NOTIFICATIONS",
    "GPUCLOUD_EXEC_ASK",
    "GPUCLOUD_HOME_MODE",
    "GPUCLOUD_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.gpucloud/kanban.db instead of the per-test GPUCLOUD_HOME.
    "GPUCLOUD_KANBAN_DB",
    "GPUCLOUD_KANBAN_BOARD",
    "GPUCLOUD_KANBAN_HOME",
    "GPUCLOUD_KANBAN_WORKSPACES_ROOT",
    "GPUCLOUD_KANBAN_LOGS_ROOT",
    "GPUCLOUD_KANBAN_TASK",
    "GPUCLOUD_KANBAN_WORKSPACE",
    "GPUCLOUD_KANBAN_RUN_ID",
    "GPUCLOUD_KANBAN_CLAIM_LOCK",
    "GPUCLOUD_KANBAN_DISPATCH_IN_GATEWAY",
    "GPUCLOUD_TENANT",
    ...
    "GPUCLOUD_DASHBOARD_OAUTH_CLIENT_ID",
    "GPUCLOUD_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and GPUCLOUD_HOME to per-test tempdirs so code that
    reads ``~/.gpucloud/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral GPUCLOUD_* vars that could change test semantics.
    for name in _GPUCLOUD_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect GPUCLOUD_HOME to a per-test tempdir. Code that reads
    #    ``~/.gpucloud/*`` via ``get_gpucloud_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.gpucloud/*`` via
    #    ``Path.home() / ".gpucloud"`` instead of ``get_gpucloud_home()``
    #    is a bug to fix at the callsite.
    fake_gpucloud_home = tmp_path / "gpucloud_test"
    fake_gpucloud_home.mkdir()
    (fake_gpucloud_home / "sessions").mkdir()
    (fake_gpucloud_home / "cron").mkdir()
    (fake_gpucloud_home / "memories").mkdir()
    (fake_gpucloud_home / "skills").mkdir()
    monkeypatch.setenv("GPUCLOUD_HOME", str(fake_gpucloud_home))
    ...
    #    ~/.gpucloud/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import gpucloud_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_gpucloud_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal gpucloud config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``gpucloud-gateway`` process
...
# correlated with a ``tests/gpucloud_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> gpucloud-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart gpucloud-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting gpucloud/python patterns are also blocked.
    """
    ...
    def _matches_gpucloud_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _GPUCLOUD_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_gpucloud_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch gpucloud-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m gpucloud_cli.main".
                if (
                    "gpucloud" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live gpucloud-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting gpucloud/python could hit the live gateway. "
