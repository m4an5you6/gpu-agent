...
| **Auto-threading** | By default, GPUCLOUD auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. |
...
2. Choose a username like `gpucloud` — the full user ID will be `@hermes:your-server.org`.
