"""Tests for `gpucloud memory setup [provider]` routing.
...
``gpucloud memory setup honcho``) without the interactive picker — which matters
because the per-provider ``gpucloud <provider>`` subcommand is only registered
...
from gpucloud_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "gpucloud memory setup" in out
