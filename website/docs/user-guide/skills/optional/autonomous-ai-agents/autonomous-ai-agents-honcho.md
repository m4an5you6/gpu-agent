...
description: "Configure and use Honcho memory with GPUCLOUD -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with GPUCLOUD -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `gpucloud skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for GPUCLOUD

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every GPUCLOUD profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/gpucloud#running-honcho-locally-with-gpucloud

### Verify

```bash
gpucloud honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this GPUCLOUD profile's AI peer
...
Honcho models conversations as interactions between **peers**. GPUCLOUD creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this GPUCLOUD instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each GPUCLOUD run |
| `global` | Single session across all directories |

Manual override: `gpucloud honcho map my-project-name`
...
Each GPUCLOUD profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block gpucloud.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `gpucloud.coder` host block in `honcho.json`
...
```bash
gpucloud honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for GPUCLOUD when Honcho memory is active.
...
Config file: `$GPUCLOUD_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `gpucloud honcho setup`. Ensure `memory.provider: honcho` is in `~/.gpucloud/config.yaml`.

### Memory not persisting across sessions
Check `gpucloud honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `gpucloud profile create <name> --clone`. For existing profiles: `gpucloud honcho sync`.
...
| Command | Description |
|---------|-------------|
| `gpucloud honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `gpucloud honcho status` | Show resolved config, connection test, peer info for active profile |
| `gpucloud honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `gpucloud honcho disable` | Disable Honcho for the active profile |
| `gpucloud honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `gpucloud honcho peers` | Show peer identities across all profiles |
| `gpucloud honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `gpucloud honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `gpucloud honcho sessions` | List known directory-to-session-name mappings |
| `gpucloud honcho map <name>` | Map current working directory to a Honcho session name |
| `gpucloud honcho identity` | Seed AI peer identity or show both peer representations |
| `gpucloud honcho sync` | Create host blocks for all GPUCLOUD profiles that don't have one yet |
| `gpucloud honcho migrate` | Step-by-step migration guide from OpenClaw native memory to GPUCLOUD + Honcho |
| `gpucloud memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `gpucloud memory status` | Show active memory provider and config |
| `gpucloud memory off` | Disable external memory provider |
