...
from gpucloud_cli.auth import AuthError, resolve_provider
from gpucloud_cli.colors import Colors, color
from gpucloud_cli.config import get_env_path, get_env_value, get_gpucloud_home, load_config
...
from gpucloud_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``gpucloud config``'s
...
def _brand_status_message(value) -> str:
    ...
    replacements = {
        "`gpucloud ": "`gpucloud ",
        "'hermes ": "'gpucloud ",
        "Run `gpucloud ": "Run `gpucloud ",
        "run `gpucloud ": "run `gpucloud ",
        "run: gpucloud ": "run: gpucloud ",
        "gpucloud auth": "gpucloud auth",
        "gpucloud model": "gpucloud model",
        "GPUCLOUD": "GPUCLOUD",
    }
...
def _display_auth_path(value) -> str:
    ...
    if not text:
        return text
    try:
        home = str(get_gpucloud_home())
        if home and text.startswith(home):
            return text.replace(home, "<GPUCLOUD home>", 1)
    except Exception:
        pass
    legacy_home = str(Path.home() / ".gpucloud")
...
def show_status(args):
    ...
    project_display = str(PROJECT_ROOT).replace("gpucloud-agent", "gpucloud-agent")
    ...
    if _effective_provider_label() == "LM Studio":
        from gpucloud_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from gpucloud_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_gpucloud_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_gpucloud_home() / "sessions" / "sessions.json"
