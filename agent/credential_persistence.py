...
``auth.json``.  They intentionally have no dependency on ``gpucloud_cli.auth`` so
...
# Sources GPUCLOUD owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and GPUCLOUD-owned OAuth/device-code state)
