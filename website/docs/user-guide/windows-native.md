...
description: "Early BETA: run GPUCLOUD Agent natively on Windows 10 / 11 — install, feature matrix, UTF-8 console, Git Bash, gateway as a Scheduled Task, editor handling, PATH, uninstall, and common pitfalls"
...
:::warning Early BETA
Native Windows support is **early beta**. It installs, runs, and passes our Windows-footgun lint, but it hasn't been road-tested at the scale our Linux/macOS/WSL2 paths have. Expect rough edges — especially around subprocess handling, path quirks, and non-ASCII console output. Please [file issues](https://github.com/NousResearch/gpucloud-agent/issues) with repro steps when you hit something. If you want a battle-tested setup today, use the [Linux/macOS installer under WSL2](./windows-wsl-quickstart.md) instead.
:::
...
If you prefer a real POSIX environment (for the dashboard's embedded terminal, `fork` semantics, Linux-style file watchers, etc.), see the **[Windows (WSL2) Guide](./windows-wsl-quickstart.md)**. Both coexist cleanly: native data lives under `%LOCALAPPDATA%\hermes`, WSL data lives under `~/.gpucloud`.
:::
...
No admin rights required. The installer goes to `%LOCALAPPDATA%\hermes\` and adds `gpucloud` to your **User PATH** — open a new terminal after it finishes.
...
| `-GPUCLOUDHome` | `%LOCALAPPDATA%\hermes` | Override data directory |
| `-InstallDir` | `%LOCALAPPDATA%\hermes\gpucloud-agent` | Override code location |
...
5. **Clones the repo** to `%LOCALAPPDATA%\hermes\gpucloud-agent` and creates a virtualenv inside it.
...
9. **Adds `%LOCALAPPDATA%\hermes\bin` to User PATH** — exposes the `gpucloud` command after you open a new terminal.
...
| Path | Contents |
|---|---|
| `%LOCALAPPDATA%\hermes\gpucloud-agent\` | Git checkout + venv. Safe to `Remove-Item -Recurse` and reinstall. |
...
The split is deliberate: `%LOCALAPPDATA%\hermes` is disposable infrastructure (you can blow it away and the one-liner restores it). `%USERPROFILE%\.gpucloud` is your data — config, memory, skills, session history — and is identical in shape to a Linux install. Mirror it between machines and your GPUCLOUD moves with you.
...
```powershell
Get-Command gpucloud        # should print C:\Users\<you>\AppData\Local\hermes\bin\hermes.cmd
gpucloud --version
```
...
That's the clean path — removes the schtasks entry, Startup folder shortcut, `hermes.cmd` shim, deletes `%LOCALAPPDATA%\hermes\gpucloud-agent\`, and trims the User PATH. It leaves `%USERPROFILE%\.gpucloud\` alone (your config, auth, skills, sessions, logs) in case you're reinstalling.
