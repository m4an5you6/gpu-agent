...
//! GPUCLOUD_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for GPUCLOUD_HOME. Tests use this; production
    /// almost always falls back to the OS default.
    pub gpucloud_home: Option<String>,
}
...
/// Spawn the locally-built GPUCLOUD desktop binary, then close the installer
...
#[tauri::command]
pub async fn launch_gpucloud_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_gpucloud_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built GPUCLOUD desktop at {}. The desktop build step \
             may have been skipped or failed. Run `gpucloud desktop` from a \
             terminal to build and launch it.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching GPUCLOUD desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/GPUCLOUD
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_gpucloud_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    let release_dir = install_root.join("apps").join("desktop").join("release");
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "GPUCLOUD.exe"),
            ("win-arm64-unpacked", "GPUCLOUD.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/GPUCLOUD.app/Contents/MacOS", "GPUCLOUD"),
            ("mac-arm64/GPUCLOUD.app/Contents/MacOS", "GPUCLOUD"),
        ]
    } else {
        &[("linux-unpacked", "gpucloud")]
    };
...
pub(crate) fn resolve_gpucloud_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_gpucloud_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
        // .../GPUCLOUD.app/Contents/MacOS/GPUCLOUD -> .../GPUCLOUD.app
...
/// path so a bare re-open just opens GPUCLOUD instead of re-running setup.
pub(crate) fn gpucloud_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".gpucloud-bootstrap-complete").exists()
        && resolve_gpucloud_desktop_exe(install_root).is_some()
}
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_gpucloud_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built GPUCLOUD desktop app")
    })?;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_gpucloud_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::gpucloud_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $GPUCLOUDHome which Stage-Repository clones
    // the repo INTO at $GPUCLOUDHome\gpucloud-agent. Mirrors gpucloud_constants.
    let gpucloud_home = args
        .gpucloud_home
        .clone()
        .unwrap_or_else(|| crate::paths::gpucloud_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&gpucloud_home).join("gpucloud-agent");

    // Copy ourselves to GPUCLOUD_HOME/gpucloud-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_gpucloud_home() {
        tracing::warn!(?err, "failed to copy installer into GPUCLOUD_HOME (non-fatal)");
        emit_log(&format!(
...
async fn run_install_script(
    ...
    args: &[String],
    gpucloud_home_override: Option<&str>,
    ...
    powershell::run_script(script_path, args, sink, gpucloud_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("GPUCLOUD"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../GPUCLOUD.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("GPUCLOUD.exe");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        } else {
            let dir = release.join("linux-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("gpucloud");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        }
    }
    ...
    // what the updater ditto's over /Applications/GPUCLOUD.app). A regression in
    // this derivation breaks the post-update auto-relaunch, so guard it.
    #[test]
    fn resolve_gpucloud_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_gpucloud_desktop_app(&root)
    ...
    #[test]
    fn resolve_gpucloud_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_gpucloud_desktop_app(&root).is_none(),
