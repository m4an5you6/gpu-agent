"""Tests for gpucloud_constants module."""
...
class TestGetDefaultGPUCLOUDRoot:
    """Tests for get_default_gpucloud_root() — Docker/custom deployment awareness."""

    def test_no_gpucloud_home_returns_native(self, tmp_path, monkeypatch):
        """When GPUCLOUD_HOME is not set, returns ~/.gpucloud."""
        monkeypatch.delenv("GPUCLOUD_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_gpucloud_root() == tmp_path / ".gpucloud"

    def test_gpucloud_home_is_native(self, tmp_path, monkeypatch):
        """When GPUCLOUD_HOME = ~/.gpucloud, returns ~/.gpucloud."""
        native = tmp_path / ".gpucloud"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("GPUCLOUD_HOME", str(native))
        assert get_default_gpucloud_root() == native

    def test_gpucloud_home_is_profile(self, tmp_path, monkeypatch):
        """When GPUCLOUD_HOME is a profile under ~/.gpucloud, returns ~/.gpucloud."""
        native = tmp_path / ".gpucloud"
    ...
    def test_gpucloud_home_is_docker(self, tmp_path, monkeypatch):
        """When GPUCLOUD_HOME points outside ~/.gpucloud (Docker), returns GPUCLOUD_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("GPUCLOUD_HOME", str(docker_home))
        assert get_default_gpucloud_root() == docker_home

    def test_gpucloud_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any GPUCLOUD_HOME outside ~/.gpucloud is treated as the root."""
        custom = tmp_path / "my-gpucloud-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("GPUCLOUD_HOME", str(custom))
        assert get_default_gpucloud_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (GPUCLOUD_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("GPUCLOUD_HOME", str(profile))
        assert get_default_gpucloud_root() == docker_root

    def test_no_gpucloud_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\gpucloud, not ~/.gpucloud."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("GPUCLOUD_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(gpucloud_constants.sys, "platform", "win32")

        assert get_default_gpucloud_root() == local_appdata / "gpucloud"

    def test_no_gpucloud_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/gpucloud without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("GPUCLOUD_HOME", raising=False)
        ...
        assert get_default_gpucloud_root() == home / "AppData" / "Local" / "gpucloud"


class TestGetGPUCLOUDHome:
    """Tests for get_gpucloud_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When GPUCLOUD_HOME is unset on Windows, use %LOCALAPPDATA%\\gpucloud."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("GPUCLOUD_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(gpucloud_constants.sys, "platform", "win32")
        monkeypatch.setattr(gpucloud_constants, "_profile_fallback_warned", False)

        assert get_gpucloud_home() == local_appdata / "gpucloud"
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(gpucloud_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(gpucloud_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".gpucloud"
