...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from gpucloud_cli.auth_commands import auth_add_command
            ...
            print_info(
                "GPUCLOUD can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one GPUCLOUD can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'gpucloud setup' or configure AUXILIARY_VISION_* settings")
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``gpucloud setup --portal``. Does NOT prompt for anything
    besides what the underlying OAuth + Tool Gateway prompts already need.
    Designed to be shareable as a single command (``gpucloud setup --portal``)
    that gets a brand-new user from zero to a fully working GPUCLOUD session
    ...
    # runs `gpucloud setup --portal` after a successful first run).
    ...
    if already_logged_in:
        ...
        # identical to `gpucloud auth add nous --type oauth`. SimpleNamespace
        ...
        try:
            ...
            print_info("  You can retry later with `gpucloud auth add nous --type oauth`.")
            return
        except (KeyboardInterrupt, EOFError):
            print()
    ...
    # from `gpucloud model` after picking Nous.
    print()
    try:
    ...
    print_info("  Run `gpucloud portal status` to inspect routing.")
...
def run_setup_wizard(args):
    ...
    print(
        color(
            "│             GPUCLOUD Agent Setup Wizard                │", Colors.CYAN
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your GPUCLOUD Agent installation.     │", Colors.CYAN
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        setup_mode = prompt_choice("How would you like to set up GPUCLOUD?", [
...
def _run_first_time_quick_setup(config: dict, gpucloud_home, is_existing: bool):
    ...
    the user can customize later via ``gpucloud setup <section>``.
    """
