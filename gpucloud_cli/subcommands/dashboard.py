"""``gpucloud dashboard`` subcommand parser.

Extracted verbatim from ``gpucloud_cli/main.py:main()`` (god-file Phase 2).
...
def build_dashboard_parser(
    ...
    # Backward-compat shim: older GPUCLOUD desktop app shells (<= 0.15.x) spawn the
    # backend as `gpucloud dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "GPUCLOUD couldn't start" with no actionable cause. Accept and silently
    ...
    # `gpucloud dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.gpucloud/.env.
    # Nested subparser so bare `gpucloud dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "GPUCLOUD_DASHBOARD_OAUTH_CLIENT_ID into ~/.gpucloud/.env, and prints "
            "how to engage the login gate. Requires being logged in (gpucloud setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://gpucloud.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via GPUCLOUD_DASHBOARD_PORTAL_URL. Mainly for "
