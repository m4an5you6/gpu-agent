...
class TestToolsetConsistency:
    ...
    def test_gpucloud_platforms_share_core_tools(self):
        """All gpucloud-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on gpucloud-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["gpucloud-cli", "gpucloud-telegram", "gpucloud-discord", "gpucloud-whatsapp", "gpucloud-slack", "gpucloud-signal", "gpucloud-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_gpucloud_whatsapp_toolset_includes_gpucloud_core(self):
        assert "gpucloud_train_start" in resolve_toolset("gpucloud-whatsapp")

    def test_gpucloud_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("gpucloud-api-server")
