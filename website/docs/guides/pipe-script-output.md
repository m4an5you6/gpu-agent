...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `gpucloud send`."
---

# Pipe Script Output to Messaging Platforms

`gpucloud send` is a small, scriptable CLI that pushes a message to any
messaging platform GPUCLOUD is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | gpucloud send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `gpucloud gateway` already uses, so there's no second configuration surface to
maintain.
...
gpucloud send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
gpucloud send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
gpucloud send --list

# Filter by platform
gpucloud send --list telegram
```
...
Any platform GPUCLOUD ships adapters for works as a target:
...
`gpucloud send` resolves the message body in this order:

1. **Positional argument** — `gpucloud send --to telegram "hi"`
2. **`--file PATH`** — `gpucloud send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | gpucloud send --to telegram`

When stdin is a TTY (no pipe), GPUCLOUD does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  gpucloud send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `gpucloud send` reuses your GPUCLOUD config, the same script works on
any host where GPUCLOUD is installed — no need to export bot tokens into
...
`gpucloud send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  gpucloud send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | gpucloud send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.gpucloud/bin/gpucloud send \
...
```bash
./train.py --epochs 200 && \
  gpucloud send --to telegram "training done" || \
  gpucloud send --to telegram "training failed (exit $?)"
```
...
gpucloud send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(gpucloud send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `gpucloud send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `gpucloud send` calls
the platform's REST endpoint directly using credentials from
`~/.gpucloud/.env` and `~/.gpucloud/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `gpucloud gateway start` and retry.
...
# Machine-readable
gpucloud send --list --json
```

The listing is built from `~/.gpucloud/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`gpucloud gateway
...
| Approach | Multi-platform | Reuses GPUCLOUD creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `gpucloud send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`gpucloud send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `gpucloud send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `gpucloud send` shares with cron delivery.
