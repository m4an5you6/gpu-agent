...
# coverage category (gpucloud_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `gpucloud tools` picker scope its entitlement gate to the selected backend, so a
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `gpucloud model`, so direct credentials should NOT
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from gpucloud_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`gpucloud tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``gpucloud tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``gpucloud model``'s Nous
    login, this:
