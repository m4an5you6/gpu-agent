...
# Python-driven tool session (e.g. GPUCLOUD terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/gpucloud-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/gpucloud-agent.git"
GPUCLOUD_HOME="${GPUCLOUD_HOME:-$HOME/.gpucloud}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${GPUCLOUD_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$GPUCLOUD_INSTALL_DIR"
...
#   code at /usr/local/lib/gpucloud-agent, command at /usr/local/bin/gpucloud,
#   data still at /root/.gpucloud (GPUCLOUD_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$GPUCLOUD_HOME/.no-bundled-skills so future"
            echo "                   'gpucloud update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> GPUCLOUD.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.gpucloud/gpucloud-agent"
            echo "                   default (root, Linux): /usr/local/lib/gpucloud-agent"
            echo "  --gpucloud-home PATH  Data directory (default: ~/.gpucloud, or \$GPUCLOUD_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, GPUCLOUD installs the code under"
            echo "  /usr/local/lib/gpucloud-agent and links the command into"
            echo "  /usr/local/bin/gpucloud (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$GPUCLOUD_HOME"
            echo "  (default /root/.gpucloud).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$GPUCLOUD_HOME/gpucloud-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs gpucloud setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `gpucloud update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (GPUCLOUD-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download GPUCLOUD Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install gpucloud command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `gpucloud` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $GPUCLOUD_HOME/gpucloud-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $GPUCLOUD_HOME/gpucloud-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/gpucloud-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $GPUCLOUD_HOME/gpucloud-agent — then preserve it)
#
# Always no-op when the user set --dir or $GPUCLOUD_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in GPUCLOUD_HOME.
    if is_termux; then
        INSTALL_DIR="$GPUCLOUD_HOME/gpucloud-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$GPUCLOUD_HOME/gpucloud-agent/.git" ]; then
            INSTALL_DIR="$GPUCLOUD_HOME/gpucloud-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/gpucloud-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/gpucloud-agent"
        ...
        # /usr/local/bin/gpucloud wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/gpucloud"
        log_info "  Data:    $GPUCLOUD_HOME (unchanged)"
...
get_gpucloud_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/gpucloud" ]; then
        echo "$link_dir/gpucloud"
    else
        echo "gpucloud"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://gpucloud-agent.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # GPUCLOUD owns its own uv at $GPUCLOUD_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (gpucloud_cli/managed_uv.py) looks in the same
    # place, so install.sh and `gpucloud update` stay in sync.
    local _managed_uv="$GPUCLOUD_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $GPUCLOUD_HOME/bin ..."
    mkdir -p "$GPUCLOUD_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/gpucloud-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/gpucloud-uv-installer.$$.sh")"
    ...
    # directly into $GPUCLOUD_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$GPUCLOUD_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the GPUCLOUD-
...
check_node() {
    ...
    # Prefer a GPUCLOUD-managed Node from a previous run over a too-old system one.
    if [ -x "$GPUCLOUD_HOME/node/bin/node" ] && node_satisfies_build "$("$GPUCLOUD_HOME/node/bin/node" --version)"; then
        export PATH="$GPUCLOUD_HOME/node/bin:$PATH"
        log_success "Node.js $("$GPUCLOUD_HOME/node/bin/node" --version) found (GPUCLOUD-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing GPUCLOUD-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.gpucloud/node/..."
    ...
    # Place into ~/.gpucloud/node/ and symlink binaries into the same bin dir
    # the gpucloud command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$GPUCLOUD_HOME/node"
    mkdir -p "$GPUCLOUD_HOME"
    mv "$extracted_dir" "$GPUCLOUD_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$GPUCLOUD_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$GPUCLOUD_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$GPUCLOUD_HOME/node/bin/npx"  "$node_link_dir/npx"

    export PATH="$GPUCLOUD_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$GPUCLOUD_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.gpucloud/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. GPUCLOUD install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "GPUCLOUD Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="gpucloud-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if GPUCLOUD behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"gpucloud-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up gpucloud command..."

    if [ "$USE_VENV" = true ]; then
        GPUCLOUD_BIN="$INSTALL_DIR/venv/bin/gpucloud"
    else
        GPUCLOUD_BIN="$(which gpucloud 2>/dev/null || echo "")"
        if [ -z "$GPUCLOUD_BIN" ]; then
            log_warn "gpucloud not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$GPUCLOUD_BIN" ]; then
        log_warn "gpucloud entry point not found at $GPUCLOUD_BIN"
    ...
    # Create a user-facing shim for the gpucloud command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $GPUCLOUD_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$GPUCLOUD_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/gpucloud"
    cat > "$command_link_dir/gpucloud" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$GPUCLOUD_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/gpucloud"
    log_success "Installed gpucloud launcher → $command_link_display_dir/gpucloud"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "gpucloud command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v gpucloud' \
                >/dev/null 2>&1; then
        ...
        log_info "gpucloud not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# GPUCLOUD Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# GPUCLOUD Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# GPUCLOUD Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so gpucloud works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.gpucloud directory structure (config at top level, code in subdir)
    mkdir -p "$GPUCLOUD_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.gpucloud/.env (top level, easy to find)
    if [ ! -f "$GPUCLOUD_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$GPUCLOUD_HOME/.env"
            log_success "Created ~/.gpucloud/.env from template"
        else
            touch "$GPUCLOUD_HOME/.env"
            log_success "Created ~/.gpucloud/.env"
        fi
    else
        log_info "~/.gpucloud/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$GPUCLOUD_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.gpucloud/config.yaml (top level, easy to find)
    if [ ! -f "$GPUCLOUD_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$GPUCLOUD_HOME/config.yaml"
            log_success "Created ~/.gpucloud/config.yaml from template"
        fi
    else
        log_info "~/.gpucloud/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$GPUCLOUD_HOME/SOUL.md" ]; then
        cat > "$GPUCLOUD_HOME/SOUL.md" << 'SOUL_EOF'
# GPUCLOUD Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how GPUCLOUD communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.gpucloud/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.gpucloud/"

    # Seed bundled skills into ~/.gpucloud/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `gpucloud update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'gpucloud update'." \
            > "$GPUCLOUD_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $GPUCLOUD_HOME/.no-bundled-skills"
        log_info "  Future 'gpucloud update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.gpucloud/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.gpucloud/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$GPUCLOUD_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$GPUCLOUD_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.gpucloud/skills/"
            fi
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$GPUCLOUD_HOME/.env"
    ...
    {
        echo ""
        echo "# GPUCLOUD Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; GPUCLOUD will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (gpucloud --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `gpucloud update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'gpucloud setup' after install."
        return 0
    fi
    ...
    # Run gpucloud setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m gpucloud_cli.main setup < /dev/tty
    else
        python -m gpucloud_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$GPUCLOUD_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for GPUCLOUD to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$GPUCLOUD_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'gpucloud whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                GPUCLOUD_CMD="$(get_gpucloud_command_path)"
                $GPUCLOUD_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'gpucloud whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'gpucloud gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $GPUCLOUD_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $GPUCLOUD_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: gpucloud gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: gpucloud gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $GPUCLOUD_CMD gateway > "$GPUCLOUD_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.gpucloud/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: gpucloud gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: gpucloud gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $GPUCLOUD_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $GPUCLOUD_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $GPUCLOUD_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}gpucloud${NC}              Start chatting"
    echo -e "   ${GREEN}gpucloud setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}gpucloud config${NC}       View/edit configuration"
    echo -e "   ${GREEN}gpucloud config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}gpucloud gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}gpucloud update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'gpucloud' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'gpucloud' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'gpucloud' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$GPUCLOUD_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$GPUCLOUD_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$GPUCLOUD_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$GPUCLOUD_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$GPUCLOUD_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up GPUCLOUD for pip install"
    ...
    GPUCLOUD_CMD="$(command -v gpucloud 2>/dev/null || echo "")"
    if [ -n "$GPUCLOUD_CMD" ]; then
        log_info "Running gpucloud setup..."
        "$GPUCLOUD_CMD" setup
    else
        log_warn "gpucloud command not found on PATH"
        log_info "Try: python -m gpucloud_cli.main setup"
    fi
}
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the GPUCLOUD-managed Node to PATH, so
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/GPUCLOUD" ]; then
            app="$desktop_dir/release/linux-unpacked/GPUCLOUD"
        elif [ -x "$desktop_dir/release/linux-unpacked/gpucloud" ]; then
            app="$desktop_dir/release/linux-unpacked/gpucloud"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/GPUCLOUD.app" \
            "$desktop_dir/release/mac/GPUCLOUD.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("GPUCLOUD is damaged and can't be opened").
    ...
    # checkout stays clean for the next `gpucloud update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the GPUCLOUD-managed Node
            # provisioned during prerequisites/node-deps (at $GPUCLOUD_HOME/node/bin)
        ...
        complete)
            ...
            resolve_install_layout
            print_success
            echo "git" > "$GPUCLOUD_HOME/.install_method"
            ;;
        *)
