import type { RunExternalProcess } from '@gpucloud/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`gpucloud ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching gpucloud: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`gpucloud ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
