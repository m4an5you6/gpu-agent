...
def _clean_state():
    ...
    saved = {}
    for k in ("GPUCLOUD_INTERACTIVE", "GPUCLOUD_GATEWAY_SESSION", "GPUCLOUD_EXEC_ASK", "GPUCLOUD_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["GPUCLOUD_INTERACTIVE"] = "1"
        session_key = os.getenv("GPUCLOUD_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No GPUCLOUD_INTERACTIVE or GPUCLOUD_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
