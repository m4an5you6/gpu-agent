"""Tests for gpucloud_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every GPUCLOUD entry point
(gpucloud, gpucloud-agent, gpucloud-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every GPUCLOUD entry point imports gpucloud_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported gpucloud_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("gpucloud_bootstrap", None)
    import gpucloud_bootstrap  # noqa: WPS433
    return gpucloud_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the GPUCLOUD CLI
...
class TestEntryPointsImportBootstrap:
    """Every GPUCLOUD entry point must import gpucloud_bootstrap as its
    ...
    # Entry points that invoke GPUCLOUD as a process.  Each one must
    # import gpucloud_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "gpucloud_cli/main.py",   # gpucloud CLI (console_script)
        "run_agent.py",          # gpucloud-agent (console_script)
        "acp_adapter/entry.py",  # gpucloud-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import gpucloud_bootstrap' and that
        ...
        half-finished ``gpucloud update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``gpucloud_bootstrap`` as a top-level module) leaves gpucloud
        ...
        """
        # Resolve relative to the gpucloud-agent repo root.  Tests live
        # at tests/test_gpucloud_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # gpucloud start even when gpucloud_bootstrap hasn't been
        ...
        assert first_import_name == "gpucloud_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'gpucloud_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import gpucloud_bootstrap' line to be the first import."
        )
