"""Windows ConPTY bridge for the `gpucloud dashboard` chat tab.

Drop-in counterpart to ``gpucloud_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``gpucloud_cli.web_server`` consumes: ``spawn``, ``read``,
