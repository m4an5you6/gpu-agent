...
chat completions — use your GPUCLOUD-managed provider subscription as their
...
| Use case | "Use GPUCLOUD as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. GPUCLOUD stores
the refresh token in `~/.gpucloud/auth.json` — the same place all GPUCLOUD
...
Model:      GPUCLOUD-4-70B    # or GPUCLOUD-4.3-36B, GPUCLOUD-4-405B
```
...
interface in `gpucloud_cli/proxy/adapters/`.
...
If you see `not logged in`, run `gpucloud auth add nous`. If you see
...
`gpucloud_cli/proxy/adapters/<provider>.py` and registering it in
