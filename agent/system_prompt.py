...
upstream prefix cache warm.  See ``gpucloud-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  GPUCLOUD never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want GPUCLOUD_HOME persona while keeping
    ...
    # Pointer to the gpucloud-agent skill + docs for user questions about GPUCLOUD itself.
    stable_parts.append(GPUCLOUD_AGENT_HELP_GUIDANCE)
    ...
    # GPUCLOUD_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Active-profile hint — names the GPUCLOUD profile the agent is running
    # under so it doesn't conflate ~/.gpucloud/skills/ (default profile) with
    # ~/.gpucloud/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active GPUCLOUD profile: default. Other profiles (if any) live "
            "under ~/.gpucloud/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active GPUCLOUD profile: {active_profile}. This session reads "
            f"and writes ~/.gpucloud/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.gpucloud/skills/, ~/.gpucloud/plugins/, "
            f"~/.gpucloud/cron/, ~/.gpucloud/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # mode).  The gateway process runs from the gpucloud-agent install
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — GPUCLOUD never rebuilds or reinjects parts of it
