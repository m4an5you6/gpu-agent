...
When running GPUCLOUD inside Docker, the default `network_mode: host` gives the
...
The GPUCLOUD [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ gpucloud-agent │   │ gpucloud-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   GPUCLOUD_UID=$(id -u) GPUCLOUD_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=gpucloud,gpucloud-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://gpucloud-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — GPUCLOUD trust model and vulnerability reporting
