...
Owns one Codex thread per GPUCLOUD session. Drives `turn/start`, consumes
...
# GPUCLOUD' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_GPUCLOUD_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse GPUCLOUD/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per GPUCLOUD session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _GPUCLOUD_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("GPUCLOUD_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="gpucloud",
            client_title="GPUCLOUD Agent",
            client_version=_get_gpucloud_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but GPUCLOUD' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into GPUCLOUD' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own gpucloud-tools callback we
            # auto-accept — the user already approved GPUCLOUD' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map GPUCLOUD approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    GPUCLOUD returns 'once', 'session', 'always', or 'deny'.
...
def _get_gpucloud_version() -> str:
    """Best-effort GPUCLOUD version string for codex's userAgent line."""
    try:
