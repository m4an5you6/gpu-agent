"""``gpucloud gui`` subcommand parser.
...
def build_gui_parser(subparsers, *, cmd_gui: Callable) -> None:
    ...
    gui_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the GPUCLOUD Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        "--ignore-existing",
        action="store_true",
        help="Force Desktop to ignore any gpucloud CLI already on PATH during backend resolution",
    )
    gui_parser.add_argument(
        "--gpucloud-root",
        help="Override the GPUCLOUD source root used by Desktop (sets GPUCLOUD_DESKTOP_GPUCLOUD_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
        help="Initial project directory for Desktop chat sessions (sets GPUCLOUD_DESKTOP_CWD)",
    )
    gui_parser.add_argument(
        "--skip-build",
        action="store_true",
