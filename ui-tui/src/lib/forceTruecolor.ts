...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.GPUCLOUD_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep GPUCLOUD on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via GPUCLOUD_TUI_TRUECOLOR=1.
