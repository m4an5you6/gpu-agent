...
That single command runs the Portal OAuth, sets Nous as your inference provider in `config.yaml`, and turns on the Tool Gateway. You're ready to `gpucloud chat` immediately after.
...
| **GPUCLOUD** | GPUCLOUD-4-70B, GPUCLOUD-4-405B (chat, see [note below](#a-note-on-hermes-4)) |
...
3. Sets Nous as your inference provider in `~/.gpucloud/config.yaml`
4. Turns on the Tool Gateway (web, image, TTS, browser routing)
5. Returns you to your terminal ready to `gpucloud chat`
...
```bash
gpucloud portal status     # login status, subscription info, model + gateway routing
...
`gpucloud portal status` (or just `gpucloud portal`) gives you the high-level overview:
...
### `gpucloud portal status` shows "not logged in"

You haven't completed the OAuth flow, or your refresh token was wiped. Run:

```bash
gpucloud auth add nous --type oauth
```
...
Check `gpucloud portal status` first — if it shows you're using a different provider (`Model: currently openrouter` instead of `using Nous as inference provider`), your local config has drifted. Run `gpucloud model`, pick Nous Portal, and the next request will route through your subscription.
