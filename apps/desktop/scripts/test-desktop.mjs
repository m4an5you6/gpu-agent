...
// no longer bundles the GPUCLOUD Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'GPUCLOUD.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'GPUCLOUD'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'GPUCLOUD.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'gpucloud'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default GPUCLOUD_HOME for non-sandboxed runs -- matches main.cjs's
// resolveGPUCLOUDHome(). On Windows it's %LOCALAPPDATA%\gpucloud; elsewhere
// it's ~/.gpucloud. The fresh-install sandbox launchFresh() sets its own
// GPUCLOUD_HOME and never touches this.
const DEFAULT_GPUCLOUD_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'gpucloud')
  }
  return path.join(os.homedir(), '.gpucloud')
})()
const VENV_ROOT = path.join(DEFAULT_GPUCLOUD_HOME, 'gpucloud-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'gpucloud-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.GPUCLOUD_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `GPUCLOUD-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `GPUCLOUD-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `GPUCLOUD-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'GPUCLOUD-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.GPUCLOUD_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.GPUCLOUD_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const gpucloudHome = path.join(sandbox, 'gpucloud-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(gpucloudHome, { recursive: true })
  ...
  env.GPUCLOUD_DESKTOP_CWD = cwd
  env.GPUCLOUD_DESKTOP_IGNORE_EXISTING = '1'
  env.GPUCLOUD_DESKTOP_TEST_MODE = 'fresh-install'
  env.GPUCLOUD_DESKTOP_USER_DATA_DIR = userDataDir
  env.GPUCLOUD_HOME = gpucloudHome
  delete env.GPUCLOUD_DESKTOP_GPUCLOUD
  delete env.GPUCLOUD_DESKTOP_GPUCLOUD_ROOT
  ...
  console.log(`  GPUCLOUD_HOME: ${gpucloudHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(gpucloudHome, 'gpucloud-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The GPUCLOUD Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of gpucloud_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'gpucloud-agent', 'gpucloud_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing GPUCLOUD
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + GPUCLOUD_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  GPUCLOUD_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
