...
Copies bundled skills from the repo's skills/ directory into ~/.gpucloud/skills/
...
The manifest lives at ~/.gpucloud/skills/.bundled_manifest.
"""
...
from gpucloud_constants import get_bundled_skills_dir, get_gpucloud_home, get_optional_skills_dir
...
# Marker file written by `gpucloud profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.gpucloud profile).
# When present in GPUCLOUD_HOME, sync_skills() is a no-op so neither the
# installer, `gpucloud update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# gpucloud_cli.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _get_bundled_dir() -> Path:
    ...
    Checks GPUCLOUD_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.gpucloud/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.gpucloud/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    ...
    """
    # Opt-out: a profile (named or the default ~/.gpucloud) that wrote the
    ...
    if (GPUCLOUD_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.gpucloud/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `gpucloud update` would resurrect a skill the user
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `gpucloud skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `gpucloud update` runs "
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes GPUCLOUD_HOME/.no-bundled-skills so the
    installer, ``gpucloud update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``gpucloud skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = GPUCLOUD_HOME / NO_BUNDLED_SKILLS_MARKER
    ...
    try:
        if enabled:
            ...
            changed = existed
            message = (
                "Opted back in. The next `gpucloud update` (or `gpucloud skills "
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (GPUCLOUD_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.gpucloud/skills/ ...")
