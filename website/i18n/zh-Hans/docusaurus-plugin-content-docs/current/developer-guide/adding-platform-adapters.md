...
Plugin 元数据。`requires_env` 和 `optional_env` 块会自动填充 `gpucloud config` UI 条目（参见下方[在 gpucloud config 中暴露环境变量](#surfacing-env-vars-in-hermes-config)）。
...
## 在 `gpucloud config` 中暴露环境变量 {#surfacing-env-vars-in-hermes-config}
...
1. **`toolsets.py`** — 添加带 `_GPUCLOUD_CORE_TOOLS` 的 `"hermes-newplat"` toolset 定义
2. **`toolsets.py`** — 将 `"hermes-newplat"` 添加到 `"gpucloud-gateway"` 的 includes 列表
