...
Let the gpucloud agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where gpucloud runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `gpucloud meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `gpucloud meet` CLI |
...
| `cli.py` | `gpucloud meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$GPUCLOUD_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `gpucloud meet node {run,list,approve,remove,status,ping}` |
...
gpucloud meet install                                      # pip + Chromium
gpucloud meet setup                                        # preflight
gpucloud meet auth                                         # optional
gpucloud meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
gpucloud meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.gpucloud/.env
gpucloud meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
gpucloud meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
gpucloud meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.gpucloud/.env
gpucloud meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, gpucloud will **not** switch your system audio input automatically — the
...
gpucloud plugins enable google_meet
gpucloud meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
gpucloud meet node ping my-mac
