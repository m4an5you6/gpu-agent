...
as ``kind: backend``) or ``~/.gpucloud/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``gpucloud tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$GPUCLOUD_HOME/cache/videos/``, creating parents as needed."""
    from gpucloud_constants import get_gpucloud_home

    path = get_gpucloud_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$GPUCLOUD_HOME/cache/videos/``.
