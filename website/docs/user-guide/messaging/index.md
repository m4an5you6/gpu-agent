...
description: "Chat with GPUCLOUD from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with GPUCLOUD from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with GPUCLOUD](/guides/use-voice-mode-with-gpucloud).
...
```mermaid
flowchart TB
    subgraph Gateway["GPUCLOUD Gateway"]
...
```bash
gpucloud gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
gpucloud gateway              # Run in foreground
gpucloud gateway setup        # Configure messaging platforms interactively
gpucloud gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo gpucloud gateway install --system   # Linux only: install a boot-time system service
gpucloud gateway start        # Start the default service
gpucloud gateway stop         # Stop the default service
gpucloud gateway status       # Check default service status
gpucloud gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update GPUCLOUD Agent to the latest version |
...
Configure per-platform overrides in `~/.gpucloud/gateway.json`:
...
gpucloud pairing approve telegram XKGH5N7P

# Other pairing commands:
gpucloud pairing list          # View pending + approved users
gpucloud pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, GPUCLOUD appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.gpucloud/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.gpucloud/config.yaml`:
...
```bash
GPUCLOUD_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
gpucloud gateway install               # Install as user service
gpucloud gateway start                 # Start the service
gpucloud gateway stop                  # Stop the service
gpucloud gateway status                # Check status
journalctl --user -u gpucloud-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo gpucloud gateway install --system
sudo gpucloud gateway start --system
sudo gpucloud gateway status --system
journalctl -u gpucloud-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. GPUCLOUD will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple GPUCLOUD installations on the same machine (with different `GPUCLOUD_HOME` directories), each gets its own systemd service name. The default `~/.gpucloud` uses `gpucloud-gateway`; other installations use `gpucloud-gateway-<hash>`. The `gpucloud gateway` commands automatically target the correct service for your current `GPUCLOUD_HOME`.
:::

### macOS (launchd)

```bash
gpucloud gateway install               # Install as launchd agent
gpucloud gateway start                 # Start the service
gpucloud gateway stop                  # Stop the service
gpucloud gateway status                # Check status
tail -f ~/.gpucloud/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.gpucloud.gateway.plist`. It includes three environment variables:
...
- **GPUCLOUD_HOME** — scopes the gateway to your GPUCLOUD installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `gpucloud gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `GPUCLOUD_HOME` directory gets its own launchd label. The default `~/.gpucloud` uses `ai.gpucloud.gateway`; other installations use `ai.gpucloud.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `gpucloud-cli` | Full access |
| Telegram | `gpucloud-telegram` | Full tools including terminal |
| Discord | `gpucloud-discord` | Full tools including terminal |
| WhatsApp | `gpucloud-whatsapp` | Full tools including terminal |
| Slack | `gpucloud-slack` | Full tools including terminal |
| Google Chat | `gpucloud-google_chat` | Full tools including terminal |
| Signal | `gpucloud-signal` | Full tools including terminal |
| SMS | `gpucloud-sms` | Full tools including terminal |
| Email | `gpucloud-email` | Full tools including terminal |
| Home Assistant | `gpucloud-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `gpucloud-mattermost` | Full tools including terminal |
| Matrix | `gpucloud-matrix` | Full tools including terminal |
| DingTalk | `gpucloud-dingtalk` | Full tools including terminal |
| Feishu/Lark | `gpucloud-feishu` | Full tools including terminal |
| WeCom | `gpucloud-wecom` | Full tools including terminal |
| WeCom Callback | `gpucloud-wecom-callback` | Full tools including terminal |
| Weixin | `gpucloud-weixin` | Full tools including terminal |
| BlueBubbles | `gpucloud-bluebubbles` | Full tools including terminal |
| QQBot | `gpucloud-qqbot` | Full tools including terminal |
| Yuanbao | `gpucloud-yuanbao` | Full tools including terminal |
| Microsoft Teams | `gpucloud-teams` | Full tools including terminal |
| API Server | `gpucloud-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `gpucloud-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.gpucloud/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
