...
| | |
|---|---|
| Source | Optional — install with `gpucloud skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by GPUCLOUD Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during GPUCLOUD terminal calls (desktop app flow only)
...
### Service Account (recommended for GPUCLOUD)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.gpucloud/.env` (the skill will prompt for this on first load).
...
## GPUCLOUD Execution Pattern (desktop app flow)

GPUCLOUD terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/gpucloud-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/gpucloud-op.sock"
