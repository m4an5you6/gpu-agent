...
The default after `gpucloud setup --portal` is a sensible general-purpose model, but the whole point of the subscription is access to the full catalog. Switch with `/model` mid-session:
...
### `gpucloud portal status` shows "not logged in" after `gpucloud setup --portal`
...
Re-verify with `gpucloud portal status`.
...
`gpucloud portal status` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
