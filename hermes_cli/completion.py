"""Shell completion script generation for GPUCLOUD CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_gpucloud_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# GPUCLOUD Agent bash completion
# Add to ~/.bashrc:
#   eval "$(gpucloud completion bash)"

_gpucloud_profiles() {{
    local profiles_dir="$HOME/.[h]ermes/profiles"
...
_gpucloud_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_gpucloud_profiles)" -- "$cur"))
        return
    fi
...
complete -F _gpucloud_completion gpucloud
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef gpucloud
# GPUCLOUD Agent zsh completion
# Add to ~/.zshrc:
...
_gpucloud_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.[h]ermes/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.[h]ermes/profiles 2>/dev/null)}}")
    fi
...
_gpucloud() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_gpucloud_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'gpucloud command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# GPUCLOUD Agent fish completion",
        "# Add to your config:",
        "#   gpucloud completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __gpucloud_profiles",
        "    echo default",
        "    if test -d $HOME/.[h]ermes/profiles",
        "        ls $HOME/.[h]ermes/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c gpucloud -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c gpucloud -f -s p -l profile"
        " -d 'Profile name' -xa '(__gpucloud_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__gpucloud_profiles)' -d 'Profile name'"
                )
