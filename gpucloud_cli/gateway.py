"""
Gateway subcommand for gpucloud CLI.

Handles: gpucloud gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_gpucloud_home is imported lazily at call sites to avoid ImportError
# when gpucloud_constants is cached from a pre-update version during `gpucloud update`.
...
from gpucloud_cli.colors import Colors, color
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "gpucloud CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``gpucloud gateway status`` from falsely counting the ``gpucloud`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``gpucloud gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "gpucloud_cli.main gateway",
        "gpucloud_cli.main --profile",
        "gpucloud_cli.main -p",
        "gpucloud_cli/main.py gateway",
        "gpucloud_cli/main.py --profile",
        "gpucloud_cli/main.py -p",
        "gpucloud gateway",
        ...
        # ``gpucloud.exe --profile`` / ``gpucloud.exe -p`` would also match
        # ``gpucloud.exe --profile foo dashboard`` and other CLI subcommands,
        # producing false-positive gateway PIDs (Copilot review).
        "gpucloud.exe gateway",
        "gpucloud-gateway.exe",
        "gateway/run.py",
    ]
    current_home = str(get_gpucloud_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"gpucloud_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. GPUCLOUD_HOME
        ...
        # GPUCLOUD_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "gpucloud_home=" in command_lc
            and f"gpucloud_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``gpucloud update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            GPUCLOUD profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to GPUCLOUD profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from gpucloud_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "gpucloud_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``gpucloud update`` users
    # with no running gateway until they re-invoke ``gpucloud gateway``
    ...
    from gpucloud_cli._subprocess_compat import (
...
def _sync_gpucloud_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``GPUCLOUD_HOME``.

    Under ``sudo``, ``GPUCLOUD_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_gpucloud_home` falls back to ``/root/.gpucloud`` — the wrong
    profile. The unit file pins ``GPUCLOUD_HOME`` for the actual gateway
    ...
    unit_home = env.get("GPUCLOUD_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("GPUCLOUD_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["GPUCLOUD_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}gpucloud gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}gpucloud gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from gpucloud_constants import is_container
    ...
    if is_linux() and is_container():
        ...
        try:
            from gpucloud_cli.service_manager import detect_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `gpucloud gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``gpucloud gateway status`` output so users with
    ...
    """
    try:
        from gpucloud_cli.profiles import get_active_profile_name
...
def _gateway_list() -> None:
    ...
    """
    try:
        from gpucloud_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (GPUCLOUD_HOME-scoped).
...
from gpucloud_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our GPUCLOUD Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``gpucloud gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``GPUCLOUD_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("GPUCLOUD_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "gpucloud-gateway"
SERVICE_DESCRIPTION = "GPUCLOUD Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current GPUCLOUD_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (GPUCLOUD_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from gpucloud_constants import get_default_gpucloud_root

    home = get_gpucloud_home().resolve()
    default = get_default_gpucloud_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary GPUCLOUD_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(gpucloud_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when GPUCLOUD_HOME is a named profile.

    For ``~/.gpucloud/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        gpucloud_home: Optional explicit GPUCLOUD_HOME path. Defaults to the current
            ``get_gpucloud_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from gpucloud_constants import get_default_gpucloud_root

    home = Path(gpucloud_home or str(get_gpucloud_home())).resolve()
    default = get_default_gpucloud_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this GPUCLOUD_HOME.

    Default ``~/.gpucloud`` returns ``gpucloud-gateway`` (backward compatible).
    Profile ``~/.gpucloud/profiles/coder`` returns ``gpucloud-gateway-coder``.
    Any other GPUCLOUD_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older GPUCLOUD installs that predate the
# gpucloud-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (gpucloud-gateway-*.service) and unrelated third-party
# "gpucloud" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("gpucloud.service",)
...
def _find_legacy_gpucloud_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy GPUCLOUD gateway units.

    Detects unit files installed by older GPUCLOUD versions that used a
    different service name (e.g. ``gpucloud.service`` before the rename to
    ``gpucloud-gateway.service``). When both a legacy unit and the current
    ``gpucloud-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``gpucloud-gateway-coder.service`` and unrelated third-party
      ``gpucloud-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``gpucloud.service`` running an unrelated
...
def has_legacy_gpucloud_units() -> bool:
    """Return True when any legacy GPUCLOUD gateway unit files exist."""
    return bool(_find_legacy_gpucloud_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy GPUCLOUD gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_gpucloud_units()
    if not legacy:
        return
    print_warning("Legacy GPUCLOUD gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current gpucloud-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    gpucloud gateway migrate-legacy")


def remove_legacy_gpucloud_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy GPUCLOUD gateway unit files.

    Iterates over whatever ``_find_legacy_gpucloud_units()`` returns — which is
    ...
    legacy = _find_legacy_gpucloud_units()
    if not legacy:
        print("No legacy GPUCLOUD gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy GPUCLOUD gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: gpucloud gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo gpucloud gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    gpucloud gateway uninstall")
    print_info("    sudo gpucloud gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so GPUCLOUD can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo gpucloud gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo gpucloud gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo gpucloud gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode GPUCLOUD often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.gpucloud`` → ``ai.gpucloud.gateway.plist`` (backward compatible).
    Profile ``~/.gpucloud/profiles/coder`` → ``ai.gpucloud.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.gpucloud.gateway-{suffix}" if suffix else "ai.gpucloud.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.gpucloud/gpucloud-agent  -> /home/alice/.gpucloud/gpucloud-agent
      /opt/gpucloud                 -> /opt/gpucloud  (kept as-is)
...
def _gpucloud_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current GPUCLOUD_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_gpucloud_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.gpucloud                    → /home/alice/.gpucloud
      /root/.gpucloud/profiles/coder     → /home/alice/.gpucloud/profiles/coder
      /opt/custom-gpucloud               → /opt/custom-gpucloud  (kept as-is)
    """
    current_gpucloud = get_gpucloud_home().resolve()
    current_default = (Path.home() / ".gpucloud").resolve()
    target_default = Path(target_home_dir) / ".gpucloud"

    # Default ~/.gpucloud → remap to target user's default
    if current_gpucloud == current_default:
        return str(target_default)

    # Profile or subdir of ~/.gpucloud → preserve the relative structure
    try:
        relative = current_gpucloud.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.gpucloud) — keep as-is
        return str(current_gpucloud)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    gpucloud_home = get_gpucloud_home()
    gpucloud_node = gpucloud_home / "node" / "bin"
    if _is_dir(gpucloud_node):
        candidates.append(str(gpucloud_node))
    gpucloud_nm = gpucloud_home / "node_modules" / ".bin"
    if _is_dir(gpucloud_nm):
        candidates.append(str(gpucloud_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m gpucloud_cli.main``, so module
    ...
    dir, or a clone that ``gpucloud update`` later relocates/removes — the path
    ...
    ``GPUCLOUD_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if GPUCLOUD_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        gpucloud_home = _gpucloud_home_for_target_user(home_dir)
        profile_arg = _profile_arg(gpucloud_home)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's GPUCLOUD_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(gpucloud_home) if gpucloud_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m gpucloud_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="GPUCLOUD_HOME={gpucloud_home}"
Restart=always
RestartSec=5
...
"""
    ...
    gpucloud_home = str(get_gpucloud_home().resolve())
    profile_arg = _profile_arg(gpucloud_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only GPUCLOUD_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp GPUCLOUD_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../gpucloud_test`` path into
    # ``Environment="GPUCLOUD_HOME=..."``. Writing that to the developer's
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current GPUCLOUD install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo gpucloud gateway uninstall --system")
    print_info("         gpucloud gateway install")
    print_info("         gpucloud gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("GPUCLOUD_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_install(
    ...
    # Offer to remove legacy units (gpucloud.service from pre-rename installs)
    # before installing the new gpucloud-gateway.service. If both remain, they
    ...
    if has_legacy_gpucloud_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_gpucloud_units(interactive=False)
            print()
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}gpucloud gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}gpucloud gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}gpucloud gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_gpucloud_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `gpucloud gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `gpucloud gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}gpucloud gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}gpucloud gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}gpucloud gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}gpucloud gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.gpucloud.gateway-{suffix}" if suffix else "ai.gpucloud.gateway"
...
# (the documented `nohup gpucloud gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m gpucloud_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active GPUCLOUD_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "gpucloud_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup gpucloud gateway run --replace`
    ...
    """
    from gpucloud_cli._subprocess_compat import windows_detach_popen_kwargs

    log_dir = get_gpucloud_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    """
    from gpucloud_constants import display_gpucloud_home as _dhh
    ...
    if _spawn_detached_gateway():
        ...
        print("  Stop it with: gpucloud gateway stop")
        return True
    print_error("Failed to start the gateway as a background process.")
    print(
        f"  Try manually: nohup gpucloud gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>gpucloud_cli.main</string>",
    ]
    if profile_arg:
...
<dict>
    ...
    <dict>
        ...
        <key>GPUCLOUD_HOME</key>
        <string>{gpucloud_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current GPUCLOUD install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  gpucloud gateway status             # Check status")
    from gpucloud_constants import display_gpucloud_home as _dhh
...
def launchd_stop():
    ...
    # `gpucloud gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current GPUCLOUD install")
    else:
        print("⚠ Service definition is stale relative to the current GPUCLOUD install")
        print("  Run: gpucloud gateway start")
    ...
    if deep:
        log_file = get_gpucloud_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("GPUCLOUD_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the GPUCLOUD gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'gpucloud' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/gpucloud/docker/entrypoint.sh before the GPUCLOUD command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$GPUCLOUD_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set GPUCLOUD_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `gpucloud gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set GPUCLOUD_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `gpucloud gateway restart` which already
    ...
    # `gpucloud gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ GPUCLOUD Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # GPUCLOUD_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("GPUCLOUD_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from gpucloud_constants import get_gpucloud_home as _ghh
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and GPUCLOUD will log in directly",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Full Matrix user ID, e.g. @gpucloud:matrix.example.org",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. gpucloud) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)",
                "password": False,
                "help": "Channel ID where GPUCLOUD delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your GPUCLOUD agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address GPUCLOUD will use (e.g., gpucloud@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. GPUCLOUD connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: gpucloud pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and GPUCLOUD will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``gpucloud setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run gpucloud under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.gpucloud/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_gpucloud_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from gpucloud_cli.telegram_managed_bot import (
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'gpucloud pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: gpucloud pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped — configure later with 'gpucloud gateway setup'"
                    )
            continue
...
def _setup_whatsapp():
    ...
    from gpucloud_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from gpucloud_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_wecom():
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("  Approve with: gpucloud pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("  Direct messages disabled.")
        else:
            print_info("  Skipped — configure later with 'gpucloud gateway setup'")
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from gpucloud_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. GPUCLOUD will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. GPUCLOUD will store the returned account_id/token in ~/.gpucloud/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `gpucloud gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_gpucloud_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `gpucloud pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `gpucloud pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "GPUCLOUDAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``gpucloud_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from gpucloud_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.gpucloud/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.gpucloud/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: gpucloud gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: gpucloud gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: gpucloud gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo gpucloud gateway install --system"
                        )
                    print_info("  Or run in foreground:  gpucloud gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: gpucloud gateway run")
                print_info(
                    "  For persistence:   tmux new -s gpucloud 'gpucloud gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: gpucloud gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup gpucloud gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'gpucloud gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`gpucloud_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from gpucloud_cli.service_manager import (
    ...
    if profile is None:
        ...
        # GPUCLOUD_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``gpucloud gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``gpucloud gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``gpucloud gateway run`` are unaffected.
      2. ``GPUCLOUD_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``gpucloud gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``GPUCLOUD_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("GPUCLOUD_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("GPUCLOUD_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${GPUCLOUD_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if GPUCLOUD_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or GPUCLOUD_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-gpucloud service's
    # pattern in docker/s6-rc.d/main-gpucloud/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: gpucloud gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: gpucloud gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s gpucloud 'gpucloud gateway run'"
                )
                print()
            ...
            print()
            print(
                "  gpucloud gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s gpucloud 'gpucloud gateway run'         # persistent via tmux"
            )
            print(
                "  nohup gpucloud gateway run > ~/.gpucloud/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  gpucloud profile create <name>     # creates the s6 service slot")
                print("  gpucloud -p <name> gateway start   # bring it up via s6")
                print("  gpucloud status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: gpucloud gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: gpucloud gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: gpucloud gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  gpucloud profile delete <name>     # tears down the s6 service slot")
                print("  gpucloud -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `gpucloud -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: gpucloud gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_GPUCLOUD_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `gpucloud gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_GPUCLOUD_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `gpucloud gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if supports_systemd_services():
                ...
                if linger_ok is not True:
                    ...
                    print()
                    print("  Then restart the gateway:")
                    print("    gpucloud gateway restart")
                    return
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: gpucloud gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  gpucloud gateway install")
                else:
                    print("To install as a service:")
                    print("  gpucloud gateway install")
                    print("  sudo gpucloud gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  gpucloud gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup gpucloud gateway run > ~/.gpucloud/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s gpucloud 'gpucloud gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  gpucloud gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  gpucloud gateway install  # Install as user service")
                    print(
                        "  sudo gpucloud gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy GPUCLOUD gateway unit files from
        # pre-rename installs (e.g. gpucloud.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_gpucloud_units(interactive=not yes, dry_run=dry_run)
