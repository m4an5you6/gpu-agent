...
sidebar_position: 4
title: "贡献指南"
description: "如何为 GPUCLOUD Agent 做贡献 — 开发环境配置、代码风格、PR 流程"
---

# 贡献指南

感谢您为 GPUCLOUD Agent 做贡献！本指南涵盖开发环境配置、代码库结构说明以及 PR 合并流程。
...
- 构建自定义/本地工具而不修改 GPUCLOUD 核心？从 [构建 GPUCLOUD 插件](../guides/build-a-gpucloud-plugin.md) 开始
- 为 GPUCLOUD 本身构建新的内置核心工具？从 [添加工具](./adding-tools.md) 开始
...
- **Profile 安全路径**：不得硬编码 `~/.gpucloud` — 代码路径使用 `gpucloud_constants` 中的 `get_gpucloud_home()`，面向用户的消息使用 `display_gpucloud_home()`。完整规则参见 [AGENTS.md](https://github.com/NousResearch/gpucloud-agent/blob/main/AGENTS.md#profiles-multi-instance-support)。

## 跨平台兼容性

GPUCLOUD 官方支持 **Linux、macOS、WSL2 以及原生 Windows（通过 PowerShell 安装）**。原生 Windows 使用 [Git for Windows](https://git-scm.com/download/win) 提供的 Git Bash 执行 shell 命令。部分功能依赖 POSIX 内核原语，已做条件限制：dashboard 内嵌的 PTY 终端面板（`/chat` 标签页）仅支持 WSL2。如果您主要在 Windows 上开发，推送前请运行 Windows 陷阱（footgun）lint（`scripts/check-windows-footguns.py`）。
...
2. **手动测试**：运行 `gpucloud` 并验证您修改的代码路径
...
- 使用 [GitHub Issues](https://github.com/NousResearch/gpucloud-agent/issues)
- 请包含：操作系统、Python 版本、GPUCLOUD 版本（`gpucloud version`）、完整错误堆栈
- 包含复现步骤
- 创建前请检查是否已有重复 issue
- 安全漏洞请私下报告
...
提交贡献即表示您同意您的贡献将以 [MIT 许可证](https://github.com/NousResearch/gpucloud-agent/blob/main/LICENSE) 授权。