...
projection into GPUCLOUD' display, approval bridging, transcript projection into
...
"codex_app_server"`. GPUCLOUD' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        if spawn_env.get("GPUCLOUD_KANBAN_TASK"):
            kanban_db = spawn_env.get("GPUCLOUD_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "GPUCLOUD_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("GPUCLOUD_HOME", os.path.expanduser("~/.gpucloud")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "gpucloud",
        client_title: str = "GPUCLOUD Agent",
