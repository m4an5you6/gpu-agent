...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time GPUCLOUD starts and finds ``~/.openclaw/``.

    Points users at ``gpucloud claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``gpucloud claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to GPUCLOUD, run "
        "`gpucloud claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `gpucloud claw cleanup` (renames it to ~/.openclaw.pre-migration — "
