...
Storage
-------
Bundles live in ``~/.gpucloud/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``GPUCLOUD_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``gpucloud bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under GPUCLOUD_HOME.

    Honors ``GPUCLOUD_BUNDLES_DIR`` for tests; falls back to
    ``<GPUCLOUD_HOME>/skill-bundles``.
    """
    override = os.environ.get("GPUCLOUD_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_gpucloud_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `gpucloud bundles` CLI subcommand.
# ---------------------------------------------------------------------------
