"""Persistent session goals — the Ralph loop for GPUCLOUD.

A goal is a free-form user objective that stays active across turns. After
each turn completes, a small judge call asks an auxiliary model "is this
goal satisfied by the assistant's last response?". If not, GPUCLOUD feeds a
...
- When a real user message arrives mid-loop it preempts the continuation
  ...
  will say ``done``).
- This module has zero hard dependency on ``cli.GPUCloudCLI`` or the gateway
...
def _get_session_db() -> Optional[Any]:
    """Return a SessionDB instance for the current GPUCLOUD_HOME.

    SessionDB has no built-in singleton, but opening a new connection per
    /goal call would thrash the file. We cache one instance per
    ``gpucloud_home`` path so profile switches still pick up the right DB.
...
def _looks_like_gpucloud_goal(goal: str) -> bool:
    try:
        from gpucloud_cli.gpucloud_goal import is_gpucloud_goal
...
class GoalManager:
    ...
    def _clear_gpucloud_context(self) -> None:
        from gpucloud_cli.gpucloud_context import clear_goal_gpucloud_config
    ...
    def _load_gpucloud_context_for_goal(self, goal: str = "") -> None:
        ...
        try:
            ...
            raise ValueError(str(exc)) from exc
        from gpucloud_cli.gpucloud_context import set_goal_gpucloud_config
