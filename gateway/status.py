...
The PID file lives at ``{GPUCLOUD_HOME}/gateway.pid``.  GPUCLOUD_HOME defaults to
``~/.gpucloud`` but can be overridden via the environment variable.  This means
separate GPUCLOUD_HOME directories naturally get separate PID files — a property
...
_GATEWAY_KIND = "gpucloud-gateway"
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting GPUCLOUD_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("GPUCLOUD_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "gpucloud" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the GPUCLOUD gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different GPUCLOUD_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``gpucloud.service`` + ``gpucloud-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``gpucloud gateway stop`` on Windows would be misclassified as an
