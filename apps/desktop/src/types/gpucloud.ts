...
export type GPUCLOUDConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `gpucloud model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
export interface ToolProvider {
   ...
   *  CLI `gpucloud tools` active-provider detection). */
  is_active: boolean
}
...
/** Shape of `GET /api/gpucloud/update/check` — the backend's own update state.
