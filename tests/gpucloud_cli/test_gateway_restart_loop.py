...
Covers:
- Defense 1: gateway stop/restart refuse when _GPUCLOUD_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "gpucloud  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "GPUCLOUD GATEWAY RESTART",           # uppercase
    ])
    def test_gpucloud_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.gpucloud.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.gpucloud.gateway.plist",
        "launchctl stop ai.gpucloud.gateway",
        "systemctl restart gpucloud-gateway",
        "systemctl stop gpucloud-gateway.service",
        "systemctl start gpucloud-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill gpucloud gateway process",
        "pkill -f gpucloud.*gateway",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "gpucloud cron list",
        "gpucloud update",
        "gpucloud config set model claude",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_gpucloud_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade gpucloud then run gpucloud gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.gpucloud.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify gpucloud gateway stop/restart refuse when _GPUCLOUD_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_GPUCLOUD_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_GPUCLOUD_GATEWAY", raising=False)
