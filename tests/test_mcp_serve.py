"""
Tests for mcp_serve — GPUCLOUD MCP server.
...
def _isolate_gpucloud_home(tmp_path, monkeypatch):
    """Redirect GPUCLOUD_HOME to a temp directory."""
    monkeypatch.setenv("GPUCLOUD_HOME", str(tmp_path))
    try:
        import gpucloud_constants
        monkeypatch.setattr(gpucloud_constants, "get_gpucloud_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking gpucloud_state schema."""
