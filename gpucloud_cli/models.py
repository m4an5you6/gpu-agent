...
_GPUCLOUD_USER_AGENT = f"hermes-cli/{_GPUCLOUD_VERSION}"
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``gpucloud model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``gpucloud auth add openai-codex`` without
    ``gpucloud model``).
    """
