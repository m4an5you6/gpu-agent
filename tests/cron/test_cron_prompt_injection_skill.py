...
def cron_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("GPUCLOUD_BUNDLES_DIR", str(gpucloud_home / "skill-bundles"))
...
def _plant_bundle(gpucloud_home: Path, name: str, skills: list[str], instruction: str = "") -> None:
    """Drop a bundle YAML into ~/.gpucloud/skill-bundles/ and refresh cache."""
    bundles_dir = gpucloud_home / "skill-bundles"
...
class TestBuildJobPromptScansSkillContent:
    ...
    def test_skill_bundle_in_job_skills_loads_referenced_skills(self, cron_env):
        gpucloud_home, scheduler = cron_env
        _plant_skill(gpucloud_home, "alpha-skill", "Alpha guidance for the cron task.")
        _plant_skill(gpucloud_home, "beta-skill", "Beta guidance for the cron task.")
        _plant_bundle(
            gpucloud_home,
            "article-pipeline",
    ...
    def test_bundle_name_shadows_skill_name_for_cron_jobs(self, cron_env):
        gpucloud_home, scheduler = cron_env
        _plant_skill(gpucloud_home, "article-pipeline", "Standalone skill should not win.")
        _plant_skill(gpucloud_home, "bundle-member", "Bundle member should win.")
        _plant_bundle(gpucloud_home, "article-pipeline", ["bundle-member"])
