...
This page covers all commands related to [GPUCLOUD profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `gpucloud profile` without a subcommand shows help.
...
## `gpucloud profile use`

```bash
gpucloud profile use <name>
```

Sets `<name>` as the active profile. All subsequent `gpucloud` commands (without `-p`) will use this profile.
...
gpucloud profile use default
```

## `gpucloud profile create`

```bash
gpucloud profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `gpucloud profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `gpucloud update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.gpucloud`), use `gpucloud skills opt-out` / `gpucloud skills opt-in`. |
...
gpucloud profile create mybot

# Clone config only from current profile
gpucloud profile create work --clone

# Clone everything from current profile
gpucloud profile create backup --clone-all

# Clone config from a specific profile
gpucloud profile create work2 --clone --clone-from work
```

## `gpucloud profile describe`

```bash
gpucloud profile describe [<name>] [options]
```
...
gpucloud profile describe researcher

# Set it explicitly
gpucloud profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
gpucloud profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
gpucloud profile describe --all --auto
```

## `gpucloud profile delete`

```bash
gpucloud profile delete <name> [options]
```
...
```bash
gpucloud profile delete mybot
gpucloud profile delete mybot --yes
```
...
## `gpucloud profile show`

```bash
gpucloud profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's GPUCLOUD home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ gpucloud profile show work
Profile: work
Path:    ~/.gpucloud/profiles/work
...
## `gpucloud profile alias`

```bash
gpucloud profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your GPUCLOUD installation.
...
```bash
gpucloud profile alias work
# Creates/updates ~/.local/bin/work

gpucloud profile alias work --name mywork
# Creates ~/.local/bin/mywork

gpucloud profile alias work --remove
# Removes the wrapper script
```

## `gpucloud profile rename`

```bash
gpucloud profile rename <old-name> <new-name>
```
...
```bash
gpucloud profile rename mybot assistant
# ~/.gpucloud/profiles/mybot → ~/.gpucloud/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `gpucloud profile export`

```bash
gpucloud profile export <name> [options]
```
...
```bash
gpucloud profile export work
# Creates work.tar.gz in the current directory

gpucloud profile export work -o ./work-2026-03-29.tar.gz
```

## `gpucloud profile import`

```bash
gpucloud profile import <archive> [options]
```
...
```bash
gpucloud profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

gpucloud profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`gpucloud profile export` / `import` are still the right commands for
...
### `gpucloud profile install`

```bash
gpucloud profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `gpucloud -p telemetry`). |
...
gpucloud profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
gpucloud profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
gpucloud profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
gpucloud profile install ./telemetry/
```

### `gpucloud profile update`

```bash
gpucloud profile update <name> [--force-config] [--yes]
```
...
### `gpucloud profile info`

```bash
gpucloud profile info <name>
```

Prints the profile's distribution manifest — name, version, required
GPUCLOUD version, author, env var requirements, the source URL/path, and
...
`gpucloud profile list` also shows the distribution name and version in a
`Distribution` column, and `gpucloud profile show <name>` / `delete <name>`
...
gpucloud profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
gpucloud profile install https://github.com/your-org/internal-assistant.git
```
...
`gpucloud_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
GPUCLOUD version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host GPUCLOUD can clone from.
3. Tell recipients to run `gpucloud profile install <your-repo-url>`.
...
## `gpucloud -p` / `gpucloud --profile`

```bash
gpucloud -p <name> <command> [options]
gpucloud --profile <name> <command> [options]
```

Global flag to run any GPUCLOUD command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
gpucloud -p work chat -q "Check the server status"
gpucloud --profile dev gateway start
gpucloud -p personal skills list
gpucloud -p work config edit
```

## `gpucloud completion`

```bash
gpucloud completion <shell>
```
...
- `gpucloud profile <TAB>` — subcommands (list, use, create, etc.)
- `gpucloud profile use <TAB>` — profile names
- `gpucloud -p <TAB>` — profile names
