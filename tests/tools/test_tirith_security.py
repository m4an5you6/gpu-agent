...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._gpucloud_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(gpucloud_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(gpucloud_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_gpucloud_bin(self):
        """After _INSTALL_FAILED, manual install in GPUCLOUD_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        gpucloud_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(gpucloud_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(gpucloud_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._gpucloud_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == gpucloud_bin
            assert _tirith_mod._resolved_path == gpucloud_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# GPUCLOUD_HOME isolation
# ---------------------------------------------------------------------------

class TestGPUCLOUDHomeIsolation:
    def test_gpucloud_bin_dir_respects_gpucloud_home(self):
        """_gpucloud_bin_dir must use GPUCLOUD_HOME, not hardcoded ~/.gpucloud."""
        from tools.tirith_security import _gpucloud_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"GPUCLOUD_HOME": tmpdir}):
            result = _gpucloud_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_gpucloud_home(self):
        """_failure_marker_path must use GPUCLOUD_HOME, not hardcoded ~/.gpucloud."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"GPUCLOUD_HOME": "/custom/gpucloud"}):
            result = _failure_marker_path()
        assert result == "/custom/gpucloud/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets GPUCLOUD_HOME; verify it's active."""
        gpucloud_home = os.getenv("GPUCLOUD_HOME")
        assert gpucloud_home is not None, "GPUCLOUD_HOME should be set by conftest"
        assert "gpucloud_test" in gpucloud_home, "Should point to test temp dir"

    def test_get_gpucloud_home_fallback(self):
        """Without GPUCLOUD_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_gpucloud_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove GPUCLOUD_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".gpucloud")
            result = _get_gpucloud_home()
