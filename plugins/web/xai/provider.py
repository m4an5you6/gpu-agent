...
``{title, url, description, position}`` rows every other GPUCLOUD web
provider produces.
...
prefers GPUCLOUD-managed xAI Grok OAuth (via ``gpucloud auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.gpucloud/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``gpucloud tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `gpucloud auth` to sign in with "
