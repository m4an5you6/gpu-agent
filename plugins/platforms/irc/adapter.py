"""
IRC Platform Adapter for GPUCLOUD Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the GPUCLOUD agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "gpucloud-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :GPUCLOUD Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :GPUCLOUD Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: gpucloud_, gpucloud_1, gpucloud_2...
...
def check_requirements() -> bool:
    ...
    # gpucloud setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `gpucloud gateway setup` flow for the IRC platform.

    Lazy-imports ``gpucloud_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect GPUCLOUD to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #gpucloud — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.gpucloud/.env")
    print_info("Restart the gateway for changes to take effect: gpucloud gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "gpucloud-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "gpucloud-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :GPUCLOUD Agent (cron)")
