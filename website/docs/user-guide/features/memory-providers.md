...
GPUCLOUD Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
gpucloud memory setup      # interactive picker + configuration
gpucloud memory status     # check what's active
gpucloud memory off        # disable external provider
```

You can also select the active memory provider via `gpucloud plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.gpucloud/config.yaml`:
...
When a memory provider is active, GPUCLOUD automatically:
...
**Setup Wizard:**
```bash
gpucloud memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `gpucloud honcho setup` command still works (it now redirects to `gpucloud memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$GPUCLOUD_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$GPUCLOUD_HOME/honcho.json` > `~/.gpucloud/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/NousResearch/gpucloud-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/gpucloud).
...
:::tip Migrating from `gpucloud honcho`
If you previously used `gpucloud honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per GPUCLOUD profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All GPUCLOUD profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per GPUCLOUD profile. Host key `gpucloud` → default; `gpucloud.<profile>` for others. |
...
```bash
gpucloud profile create coder --clone
```

`--clone` creates a `gpucloud.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.

### Existing profiles, backfill Honcho peers

```bash
gpucloud honcho sync
```

Scans every GPUCLOUD profile, creates host blocks for any profile without one, inherits settings from the default `gpucloud` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
See the [config reference](https://github.com/NousResearch/gpucloud-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/gpucloud).
...
gpucloud memory setup    # select "openviking"
# Or manually:
gpucloud config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.gpucloud/.env
```
...
**Setup:**
```bash
gpucloud memory setup    # select "mem0"
# Or manually:
gpucloud config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.gpucloud/.env
```

**Config:** `$GPUCLOUD_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `gpucloud-user` | User identifier |
| `agent_id` | `gpucloud` | Agent identifier |
...
**Setup:**
```bash
gpucloud memory setup    # select "hindsight"
# Or manually:
gpucloud config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.gpucloud/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p gpucloud ui start`

**Config:** `$GPUCLOUD_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `gpucloud` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between GPUCLOUD Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/NousResearch/gpucloud-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
gpucloud memory setup    # select "holographic"
# Or manually:
gpucloud config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.gpucloud-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$GPUCLOUD_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
gpucloud memory setup    # select "retaindb"
# Or manually:
gpucloud config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.gpucloud/.env
```
...
# Then configure GPUCLOUD
gpucloud memory setup    # select "byterover"
# Or manually:
gpucloud config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$GPUCLOUD_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
gpucloud memory setup    # select "supermemory"
# Or manually:
gpucloud config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.gpucloud/.env
```

**Config:** `$GPUCLOUD_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `gpucloud` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `gpucloud-{identity}` → `gpucloud-coder`) to isolate memories per GPUCLOUD profile
...
| **Requires** | `pip install gpucloud-memori` + `gpucloud-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install gpucloud-memori
gpucloud-memori install
gpucloud config set memory.provider memori
gpucloud memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `gpucloud-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$GPUCLOUD_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$GPUCLOUD_HOME/` so each profile has its own credentials
