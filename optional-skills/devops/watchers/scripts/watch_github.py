...
Usage (via cron with --no-agent):
    ...
    gpucloud cron create gpucloud-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$GPUCLOUD_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name gpucloud-issues --repo NousResearch/gpucloud-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.gpucloud/.env to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "GPUCLOUD-Watcher/1.0",
    }
