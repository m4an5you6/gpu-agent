...
Architecture (two transports):
  ...
  **Local backend (UDS):**
  1. Parent generates a `gpucloud_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `gpucloud_tools.py` with file-based RPC stubs
...
# match a safe prefix, the operational GPUCLOUD_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "GPUCLOUD_" prefix was deliberately removed (#27303) — it leaked
# GPUCLOUD_*-named config that lacks a secret substring (e.g. GPUCLOUD_BASE_URL,
# GPUCLOUD_KANBAN_DB, GPUCLOUD_*_WEBHOOK).  The child only needs the few
# location/profile vars in _GPUCLOUD_CHILD_ALLOWED below; GPUCLOUD_RPC_SOCKET /
# GPUCLOUD_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational GPUCLOUD_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set gpucloud_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_GPUCLOUD_CHILD_ALLOWED = frozenset({
    "GPUCLOUD_HOME",
    "GPUCLOUD_PROFILE",
    "GPUCLOUD_CONFIG",
    "GPUCLOUD_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational GPUCLOUD_* vars (_GPUCLOUD_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret GPUCLOUD_* vars dropped by the tightened allowlist (#27303). The
    # broad "GPUCLOUD_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like GPUCLOUD_KANBAN_DB / GPUCLOUD_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _GPUCLOUD_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("GPUCLOUD_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped GPUCLOUD_* var.
            _dropped_gpucloud.append(k)
    if _dropped_gpucloud:
        logger.debug(
            "execute_code: dropped %d non-allowlisted GPUCLOUD_* var(s) from the "
            ...
            len(_dropped_gpucloud),
            ", ".join(sorted(_dropped_gpucloud)),
        )
    return scrubbed
...
# ---------------------------------------------------------------------------
# gpucloud_tools.py code generator
# ---------------------------------------------------------------------------
...
def generate_gpucloud_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the gpucloud_tools.py stub module.
...
_RPC_DIR = os.environ.get("GPUCLOUD_RPC_DIR") or os.path.join(tempfile.gettempdir(), "gpucloud_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated gpucloud_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/gpucloud_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_gpucloud_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/gpucloud_tools.py", tools_src)
...
def execute_code(
    ...
    # --- Set up temp directory with gpucloud_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="gpucloud_sandbox_")
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"gpucloud_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated gpucloud_tools module.
        ...
        tools_src = generate_gpucloud_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "gpucloud_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        # the staging tmpdir so ``from gpucloud_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _gpucloud_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _gpucloud_root]
        if _existing_pp:
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    description = (
        ...
        f"Available via `from gpucloud_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into gpucloud_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from gpucloud_tools import {import_str}` "
