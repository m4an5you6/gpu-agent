"""
Skills configuration for GPUCLOUD Agent.
`gpucloud skills` enters this module.

Toggle individual skills or categories on/off, globally or per-platform.
Config stored in ~/.gpucloud/config.yaml under:
...
from gpucloud_cli.config import cfg_get, load_config, save_config
from gpucloud_cli.colors import Colors, color
from gpucloud_cli.platforms import PLATFORMS as _PLATFORMS
...
def skills_command(args=None):
    """Entry point for `gpucloud skills`."""
    from gpucloud_cli.curses_ui import curses_checklist
