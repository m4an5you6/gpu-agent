"""MCP picker — interactive `gpucloud mcp picker` (also the default `gpucloud mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``gpucloud mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `gpucloud plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from gpucloud_cli.cli_output import prompt_yes_no
from gpucloud_cli.curses_ui import curses_single_select
from gpucloud_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new GPUCLOUD session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from gpucloud_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `gpucloud mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: gpucloud mcp install <name>    Picker: gpucloud mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their GPUCLOUD is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer GPUCLOUD — run `gpucloud update` "
...
def show_catalog() -> None:
    """`gpucloud mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`gpucloud mcp picker` (and default `gpucloud mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`gpucloud mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `gpucloud mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
