...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_gpucloud_home_override(self, tmp_path):
        """Auth lookup should read from GPUCLOUD_HOME/auth.json, not ~/.gpucloud/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".gpucloud").mkdir(parents=True)
        ...
        with patch.dict(os.environ, {
            "HOME": str(real_home),
            "GPUCLOUD_HOME": str(gpucloud_home),
        }, clear=False):
            import tools.web_tools
...
class TestBackendSelection:
    ...
    ``gpucloud tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "gpucloud_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
