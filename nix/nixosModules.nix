# nix/nixosModules.nix — NixOS module for gpucloud-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: gpucloud runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $GPUCLOUD_HOME/.env
# and read by gpucloud at startup — no container recreation needed for env changes.
#
# Tool resolution: the gpucloud wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.gpucloud-agent;
    effectivePackage =
    ...
    gpucloud-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-gpucloud-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "gpucloud-config-attrs";
      description = "GPUCLOUD YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "gpucloud-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # GPUCLOUD_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "gpucloud-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'GPUCLOUD_DOC_EOF'\n${value}\nGPUCLOUD_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "gpucloud-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/gpucloud";
    ...
    # gpucloud user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "gpucloud-container-entrypoint" ''
      set -eu

      GPUCLOUD_UID="''${GPUCLOUD_UID:?GPUCLOUD_UID must be set}"
      GPUCLOUD_GID="''${GPUCLOUD_GID:?GPUCLOUD_GID must be set}"

      # ── Group: ensure a group with GID=$GPUCLOUD_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$GPUCLOUD_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="gpucloud"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$GPUCLOUD_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$GPUCLOUD_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$GPUCLOUD_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$GPUCLOUD_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="gpucloud"
        TARGET_HOME="/home/gpucloud"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$GPUCLOUD_UID" -g "$GPUCLOUD_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$GPUCLOUD_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$GPUCLOUD_UID:$GPUCLOUD_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure GPUCLOUD_HOME is owned by the target user.
      ...
      if [ -n "''${GPUCLOUD_HOME:-}" ] && [ -d "$GPUCLOUD_HOME" ]; then
        find "$GPUCLOUD_HOME" \! -user "$GPUCLOUD_UID" -exec chown "$GPUCLOUD_UID:$GPUCLOUD_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/gpucloud-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/gpucloud-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/gpucloud ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/gpucloud
        chmod 0440 /etc/sudoers.d/gpucloud
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$GPUCLOUD_UID" --regid="$GPUCLOUD_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $GPUCLOUD_HOME/.env.
    ...
    # Default: /var/lib/gpucloud/workspace → /data/workspace.
  ...
  in {
    options.services.gpucloud-agent = with lib; {
      enable = mkEnableOption "GPUCLOUD Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = gpucloud-agent;
        description = "The gpucloud-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/gpucloud";
        description = "State directory. Contains .gpucloud/ subdir (GPUCLOUD_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative GPUCLOUD config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $GPUCLOUD_HOME/.env at activation time.
          GPUCLOUD reads this file on every startup via load_gpucloud_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $GPUCLOUD_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $GPUCLOUD_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `gpucloud gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the gpucloud user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the gpucloud plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. GPUCLOUD discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          gpucloud_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as gpucloud (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in gpucloud-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the gpucloud CLI to environment.systemPackages and export
          GPUCLOUD_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.gpucloud symlink to the service
            stateDir. These users are automatically added to the gpucloud group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.gpucloud-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the gpucloud CLI to system PATH and export GPUCLOUD_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.gpucloud/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.GPUCLOUD_HOME = "${cfg.stateDir}/.gpucloud";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.gpucloud-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the gpucloud user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.gpucloud-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed gpucloud
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure gpucloud is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.gpucloud        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.gpucloud/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.gpucloud/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.gpucloud/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.gpucloud/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.gpucloud/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."gpucloud-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.gpucloud
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.gpucloud ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.gpucloud ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.gpucloud -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.gpucloud/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.gpucloud/$_subdir"
            chmod 2770 "${cfg.stateDir}/.gpucloud/$_subdir"
            find "${cfg.stateDir}/.gpucloud/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # gpucloud-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.gpucloud/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.gpucloud/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.gpucloud/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.gpucloud/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.gpucloud/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.gpucloud/.managed
          chmod 0644 ${cfg.stateDir}/.gpucloud/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.gpucloud/.container-mode <<'GPUCLOUD_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    gpucloud_bin=${containerDataDir}/current-package/bin/gpucloud
    GPUCLOUD_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.gpucloud/.container-mode
            chmod 0644 ${cfg.stateDir}/.gpucloud/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.gpucloud/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.gpucloud";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.gpucloud" ]; then
                  rm -f "${symlinkPath}"
                  echo "gpucloud-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.gpucloud -> stateDir/.gpucloud for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.gpucloud";
                target = "${cfg.stateDir}/.gpucloud";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "gpucloud-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.gpucloud/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.gpucloud/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.gpucloud/auth.json
              fi
            ''}
          ''}
          ...
          # GPUCLOUD reads $GPUCLOUD_HOME/.env at startup via load_gpucloud_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.gpucloud/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'GPUCLOUD_NIX_ENV_EOF'
    ${envFileContent}
    GPUCLOUD_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
                echo "" >> "$ENV_FILE"
                cat "${f}" >> "$ENV_FILE"
        ...
        find ${cfg.stateDir}/.gpucloud/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.gpucloud/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.gpucloud/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.gpucloud-agent = {
          description = "GPUCLOUD Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            GPUCLOUD_HOME = "${cfg.stateDir}/.gpucloud";
            GPUCLOUD_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $GPUCLOUD_HOME/.env by the activation script. load_gpucloud_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/gpucloud"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the gpucloud group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.gpucloud-agent = {
          description = "GPUCLOUD Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              GPUCLOUD_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              GPUCLOUD_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env GPUCLOUD_UID="$GPUCLOUD_UID" \
                --env GPUCLOUD_GID="$GPUCLOUD_GID" \
                --env GPUCLOUD_HOME=${containerDataDir}/.gpucloud \
                --env GPUCLOUD_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/gpucloud gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
