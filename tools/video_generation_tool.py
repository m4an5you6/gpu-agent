...
provider** — turn on a backend by enabling a plugin (``gpucloud plugins
enable video_gen/<name>``) and selecting it in ``gpucloud tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/gpucloud-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `gpucloud tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `gpucloud plugins list` to see "
            f"installed video gen backends, or `gpucloud tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `gpucloud tools` → "
...
# `gpucloud tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`gpucloud tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `gpucloud tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
