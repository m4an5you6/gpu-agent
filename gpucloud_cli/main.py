...
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("GPUCLOUD_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not (os.environ.get("GPUCLOUD_TUI") == "1" or "--tui" in sys.argv[1:]):
        return
    try:
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to hooks_auto_accept: true)."
        ),
    )
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "packages/gpucloud-ink/package-lock.json",
...
def select_provider_and_model(args=None):
    ...
    from gpucloud_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from gpucloud_constants import OPENROUTER_BASE_URL
    from gpucloud_cli.auth import (
        ProviderConfig,
    ...
    from gpucloud_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.gpucloud/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `gpucloud setup` from scratch. OpenRouter
    ...
    from gpucloud_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from gpucloud_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier:
        try:
            from gpucloud_cli.nous_account import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from gpucloud_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from gpucloud_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `gpucloud auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from gpucloud_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``gpucloud model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``gpucloud model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``gpucloud auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from gpucloud_cli.models import fetch_api_models
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    Flow:
      ...
      5. Save to ~/.gpucloud/config.yaml.
    """
    from gpucloud_cli.auth import (
...
def _model_flow_custom(config):
    ...
    """
    from gpucloud_cli.auth import _save_model_choice, deactivate_provider
    from gpucloud_cli.config import get_env_value, load_config, save_config
    from gpucloud_cli.secret_prompt import masked_secret_prompt
    ...
    from gpucloud_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `gpucloud model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from gpucloud_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from gpucloud_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from gpucloud_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  GPUCLOUD will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  GPUCLOUD will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from gpucloud_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from gpucloud_cli.config import save_env_value, load_config, save_config
    from gpucloud_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from gpucloud_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from gpucloud_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates GPUCLOUD turns to `copilot --acp`.")
    print("  GPUCLOUD currently starts its own ACP subprocess for each request.")
    print("  GPUCLOUD uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set GPUCLOUD_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from gpucloud_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so gpucloud knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   GPUCLOUD typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with GPUCLOUD, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from gpucloud_cli.auth import AuthError
        from gpucloud_cli.models import fetch_lmstudio_models
        ...
        from gpucloud_cli.models import fetch_ollama_cloud_models
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from gpucloud_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'gpucloud model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_portal(args):
    ...
    from gpucloud_cli.portal_cli import portal_command
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``gpucloud_cli/web_dist/`` (per vite.config.ts
    outDir: "../gpucloud_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "gpucloud_cli" / "web_dist"
...
def _update_via_zip(args):
    ...
    # optional — a failure here only affects ``gpucloud dashboard``. Make
...
def _detect_concurrent_hermes_instances(
    ...
    The GPUCLOUD Desktop Electron app spawns ``hermes.EXE`` as a backend child,
    ...
    shims (``hermes.exe`` / ``gpucloud-gateway.exe``) and returns ``(pid,
    ...
    # walking the parent chain, every ``gpucloud update`` reports its own
...
def _quarantine_running_hermes_exe(
    ...
    when ``gpucloud update`` runs, ``hermes.exe`` IS the live process, and uv
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``hermes.exe`` / ``gpucloud-gateway.exe`` shims
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.gpucloud/logs/update.log`` so nothing is lost.
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://raw.githubusercontent.com/NousResearch/gpucloud-agent/main/scripts/install.sh | bash"
            )
            sys.exit(1)
...
def cmd_profile(args):
    ...
    if action is None:
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → gpucloud -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     GPUCLOUD {data['hermes_requires']}")
        if data.get("source"):
...
def cmd_dashboard(args):
    ...
    embedded_chat = args.tui or os.environ.get("GPUCLOUD_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from gpucloud_cli.models import CANONICAL_PROVIDERS as _cp
...
_BUILTIN_SUBCOMMANDS = frozenset(
    ...
    "gpucloud_cli.gpucloud_phase2", fromlist=["DISABLED_CLI_COMMANDS"]
...
def main():
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login",
    )
    model_parser.add_argument(
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in the GPUCLOUD environment file.  "
            "Currently supports Bitwarden Secrets Manager."
        ),
    )
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if dashboard supervision is enabled). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy GPUCLOUD gateway unit files "
            "(e.g. hermes.service) left over from older installs. Profile "
            "units (gpucloud-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `gpucloud proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'gpucloud setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for GPUCLOUD.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to the GPUCLOUD home slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "GPUCLOUD")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for GPUCLOUD CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use"
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of GPUCLOUD Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under the GPUCLOUD scripts directory. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="GPUCLOUD profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under the GPUCLOUD scripts directory. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="GPUCLOUD profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in the user config file, "
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with GPUCLOUD Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `gpucloud "
    ...
    security_parser = subparsers.add_parser(
        ...
        description=(
            "On-demand vulnerability scan against OSV.dev. Covers the GPUCLOUD "
            "venv (installed PyPI dists), Python deps declared by plugins under "
            "plugins directory, and pinned npx/uvx MCP servers in config.yaml. "
    ...
    audit_parser.add_argument(
        "--skip-venv",
        action="store_true",
        help="Skip scanning the GPUCLOUD Python venv",
    )
    audit_parser.add_argument(
        "--skip-plugins",
        action="store_true",
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your GPUCLOUD setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for GPUCLOUD Agent. Use 'gpucloud debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    gpucloud debug share              Upload debug report and print URL
    gpucloud debug share --lines 500  Include more log lines
    gpucloud debug share --expire 30  Keep paste for 30 days
    gpucloud debug share --local      Print report locally (no upload)
    gpucloud debug share --no-redact  Disable upload-time secret redaction
    gpucloud debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'gpucloud debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up GPUCLOUD home directory to a zip file",
        description="Create a zip archive of your entire GPUCLOUD configuration, "
        "skills, sessions, and data (excludes the GPUCLOUD codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/gpucloud-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear filesystem checkpoints",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a GPUCLOUD backup from a zip file",
        description="Extract a previously created GPUCLOUD backup into your "
        "GPUCLOUD home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage GPUCLOUD Agent configuration",
    )
    ...
    # config migrate-home
    config_migrate_home = config_subparsers.add_parser(
        "migrate-home",
        help="Copy legacy ~/.hermes data into ~/.gpucloud without deleting it",
    )
    config_migrate_home.add_argument(
        "--src",
        help="Legacy source directory (default: ~/.hermes)",
    )
    config_migrate_home.add_argument(
        "--dst",
        help="GPUCLOUD target directory (default: ~/.gpucloud)",
    )
    config_migrate_home.add_argument(
        "--dry-run",
        action="store_true",
        help="Preview files that would be copied without writing anything",
    )
    ...
    def cmd_worker(args):
        ...
        if sub == "wait":
            ...
            from gpucloud_cli.gpucloud_worker_connect import run_worker_connect
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "so future 'gpucloud update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/gpucloud-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `gpucloud plugins enable <name>`",
    )
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'gpucloud tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # gpucloud tools list [--platform cli]
    ...
    # gpucloud tools disable <name...> [--platform cli]
    ...
    # gpucloud tools enable <name...> [--platform cli]
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run GPUCLOUD as an MCP server",
        description=(
            "Manage MCP server connections and run GPUCLOUD as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'gpucloud mcp add' to connect to a new server, or\n"
            "'gpucloud mcp serve' to expose GPUCLOUD conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run GPUCLOUD as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `gpucloud mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `gpucloud mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `gpucloud mcp install n8n`)",
    )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to GPUCLOUD",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to GPUCLOUD",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of GPUCLOUD home (by default a "
        "single restore-point archive is written before apply; "
        "restorable with 'gpucloud import').",
    )
    claw_migrate.add_argument(
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update GPUCLOUD Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'gpucloud config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--branch",
        default=None,
        ...
        help=(
            ...
            "If the local checkout is on a different branch, GPUCLOUD will "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another GPUCLOUD process is detected. The concurrent process may leave a reboot-deferred executable replacement.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall GPUCLOUD Agent",
        description="Remove GPUCLOUD Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run GPUCLOUD Agent as an ACP (Agent Client Protocol) server",
        description="Start GPUCLOUD Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print GPUCLOUD ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive GPUCLOUD provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into GPUCLOUD home "
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated GPUCLOUD instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `gpucloud update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `gpucloud profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a GPUCLOUD profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the GPUCLOUD Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `gpucloud --tui` via PTY/WebSocket). "
            "Alternatively enable the dashboard TUI setting."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `gpucloud dashboard` cmdlines and SIGTERM them directly — the same
    # path `gpucloud update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running GPUCLOUD dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running GPUCLOUD dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter GPUCLOUD log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    gpucloud logs                    Show last 50 lines of agent.log
    gpucloud logs -f                 Follow agent.log in real time
    gpucloud logs errors             Show last 50 lines of errors.log
    gpucloud logs gateway -n 100     Show last 100 lines of gateway.log
    gpucloud logs --level WARNING    Only show WARNING and above
    gpucloud logs --session abc123   Filter by session ID
    gpucloud logs --component tools  Only show tool-related lines
    gpucloud logs --since 1h         Lines from the last hour
    gpucloud logs --since 30m -f     Follow, starting from 30 min ago
    gpucloud logs list               List available log files with sizes
""",
    )
    ...
    from gpucloud_cli.gpucloud_phase2 import prune_disabled_cli_subcommands
