...
- Give it a name like "gpucloud-agent"
- Select scopes:
...
- Give it a title like "gpucloud-agent-&lt;machine-name>"
...
elif [ -f ~/.gpucloud/.env ] && grep -q "^GITHUB_TOKEN=" ~/.gpucloud/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.gpucloud/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
