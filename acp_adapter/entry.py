"""CLI entry point for the gpucloud-agent ACP adapter.

Loads environment variables from ``~/.gpucloud/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from GPUCLOUD_HOME (default ``~/.gpucloud``)."""
    from gpucloud_cli.env_loader import load_gpucloud_dotenv

    gpucloud_home = get_gpucloud_home()
    loaded = load_gpucloud_dotenv(gpucloud_home=gpucloud_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", gpucloud_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="gpucloud-acp",
        description="Run GPUCLOUD Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print GPUCLOUD version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.gpucloud/node/ "
...
def _print_version() -> None:
    ...
    print(gpucloud_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import GPUCLOUDACPAgent  # noqa: F401

    print("GPUCLOUD ACP check OK")
...
def _run_setup() -> None:
    ...
    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "gpucloud", "model"]
        gpucloud_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``gpucloud postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting gpucloud-agent ACP adapter")
    ...
    import acp
    from .server import GPUCLOUDACPAgent
