...
def test_get_service_manager_returns_s6_instance(
    ...
    from gpucloud_cli.service_manager import S6ServiceManager
    monkeypatch.setattr(
...
def test_render_run_script_resets_home_before_exec() -> None:
    ...
    assert "export HOME=/opt/data" in run_text
    assert "exec s6-setuidgid gpucloud hermes -p coder gateway run" in run_text
...
def test_lifecycle_raises_s6_command_error_on_subprocess_failure(
    ...
    import subprocess as _sp
    from gpucloud_cli.service_manager import S6CommandError, S6ServiceManager
