...
The `gpucloud honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `gpucloud memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
gpucloud honcho status          # Connection status, config, and key settings
gpucloud honcho setup           # Redirects to `gpucloud memory setup`
