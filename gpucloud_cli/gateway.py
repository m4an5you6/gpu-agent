        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "gpucloud-gateway*",
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"GPUCLOUD_HOME={current_home}" in command
            )
        ...
        if "--profile " in command or " -p " in command:
            return False
        if "GPUCLOUD_HOME=" in command and f"GPUCLOUD_HOME={current_home}" not in command:
            return False
        return True
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}gpucloud gateway restart{scope_flag}")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so GPUCLOUD can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo gpucloud gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo gpucloud gateway install --system --run-as-user <your-user>")
...
def _gpucloud_home_for_target_user(target_home_dir: str) -> str:
    ...
    """
    current_hermes = get_gpucloud_home().resolve()
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    hermes_node = gpucloud_home / "node" / "bin"
    if _is_dir(hermes_node):
        candidates.append(str(hermes_node))
    hermes_nm = gpucloud_home / "node_modules" / ".bin"
...
User={username}
Group={group_name}
ExecStart={python_path} -m gpucloud_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current GPUCLOUD install")
    return True
...
def systemd_install(
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}gpucloud gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}gpucloud gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}gpucloud gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}gpucloud gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}gpucloud gateway start{scope_flag}")
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current GPUCLOUD install")
    return True
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print(
        ...
        "/opt/hermes/docker/entrypoint.sh before the GPUCLOUD command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$GPUCLOUD_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set GPUCLOUD_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
    ...
    },
    {
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address GPUCLOUD will use (e.g., hermes@gmail.com)."},
...
def _setup_weixin():
    ...
    print_info("  3. GPUCLOUD will store the returned account_id/token in ~/.gpucloud/.env.")
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `gpucloud pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `gpucloud pairing approve`.")
    elif access_idx == 1:
        ...
        if service_running:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    ...
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo gpucloud gateway install --system")
                ...
                print_info("  For persistence:   tmux new -s gpucloud 'gpucloud gateway run'")
                print_info("  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'")
            elif is_termux():
                from gpucloud_constants import display_gpucloud_home as _dhh
                print_info("  Termux does not use systemd/launchd services.")
                print_info("  Run in foreground: gpucloud gateway run")
                print_info(f"  Or start it manually in the background (best effort): nohup gpucloud gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
            else:
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    # gateway flaps. `sleep infinity` matches the static main-gpucloud
    # service's pattern (see docker/s6-rc.d/main-gpucloud/run): the CMD
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            if is_wsl():
                ...
                print_info("  Consider running in foreground instead: gpucloud gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s gpucloud 'gpucloud gateway run'")
                print()
            ...
            print()
            print("  gpucloud gateway run                              # direct foreground")
            print("  tmux new -s gpucloud 'gpucloud gateway run'         # persistent via tmux")
            print("  nohup gpucloud gateway run > ~/.gpucloud/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    print("  nohup gpucloud gateway run > ~/.gpucloud/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s gpucloud 'gpucloud gateway run'         # persistent via tmux")
                    print("  nohup gpucloud gateway run > ~/.gpucloud/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  gpucloud gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  gpucloud gateway install  # Install as user service")
                    print("  sudo gpucloud gateway install --system  # Install as boot-time system service")
