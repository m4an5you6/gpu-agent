...
Backend is selected during ``gpucloud tools`` setup (web.backend in config.yaml).
When available, GPUCLOUD can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via GPUCLOUD config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through GPUCLOUD' config/.env layer (``gpucloud config set``, ``gpucloud tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.gpucloud/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``gpucloud tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `gpucloud tools` repaint.
        try:
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `gpucloud tools` to set one up."
                ),
            }
        else:
            logger.info(
