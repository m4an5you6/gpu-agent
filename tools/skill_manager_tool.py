...
~/.gpucloud/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `gpucloud config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.gpucloud/skills/ (single source of truth)
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`gpucloud curator unpin {name}` if they want to delete it. "
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.gpucloud/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER GPUCLOUD profiles.
    ...
    try:
        root = get_default_gpucloud_root()
    except Exception:
        return matches
    ...
    # Default profile (~/.gpucloud/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.gpucloud/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`gpucloud -p "
            ...
            base += (
                ...
                f"Switch profiles (`gpucloud -p <name>`) to edit there, or "
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_gpucloud_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `gpucloud curator unpin <name>`. "
