...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources GPUCLOUD_HOME (and any extra env) via with-contenv —
             so e.g. ``-e GPUCLOUD_HOME=/data/hermes`` is honored at run
          ...
          4. Drops to the gpucloud user and exec's
             ``gpucloud -p <profile> gateway run`` (or just ``gpucloud gateway run`` for the default profile — see below).
        ...
        if profile == "default":
            lines.append("exec s6-setuidgid gpucloud gpucloud gateway run")
        else:
            lines.append(
                f"exec s6-setuidgid gpucloud hermes -p {shlex.quote(profile)} gateway run"
            )
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        — so a container started with ``-e GPUCLOUD_HOME=/data/hermes``
