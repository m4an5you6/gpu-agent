...
import type { AuxiliaryModelsResponse, ModelOptionProvider, StaleAuxAssignment } from '@/gpucloud'
...
// backend now surfaces the full `gpucloud model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in gpucloud_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `gpucloud model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — GPUCLOUD runs the flow for you.`}
          </p>
        )}
