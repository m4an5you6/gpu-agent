...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        gpucloud_home.mkdir()
        (gpucloud_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(gpucloud_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.gpucloud/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        gpucloud_home = tmp_path / ".gpucloud"
        gpucloud_home.mkdir()
        (gpucloud_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        gpucloud_home = tmp_path / ".gpucloud"
        gpucloud_home.mkdir()
        (gpucloud_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        gpucloud_home = tmp_path / ".gpucloud"
        gpucloud_home.mkdir()
        (gpucloud_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.gpucloud/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        gpucloud_home = tmp_path / ".gpucloud"
        (gpucloud_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"GPUCLOUD_HOME": str(gpucloud_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.gpucloud")

        assert mounts[0]["container_path"] == "/home/user/.gpucloud/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.gpucloud/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.gpucloud/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape GPUCLOUD_HOME."""
        monkeypatch.setenv("GPUCLOUD_HOME", str(tmp_path / ".gpucloud"))
        (tmp_path / ".gpucloud").mkdir()

        # Create a sensitive file one level above gpucloud_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside gpucloud_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside GPUCLOUD_HOME must still be registered."""
        gpucloud_home = tmp_path / ".gpucloud"
        gpucloud_home.mkdir()
        monkeypatch.setenv("GPUCLOUD_HOME", str(gpucloud_home))
        (gpucloud_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_gpucloud_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of GPUCLOUD_HOME must be allowed."""
        gpucloud_home = tmp_path / ".gpucloud"
        gpucloud_home.mkdir()
        subdir = gpucloud_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside GPUCLOUD_HOME pointing outside must be rejected."""
        gpucloud_home = tmp_path / ".gpucloud"
        gpucloud_home.mkdir()
        monkeypatch.setenv("GPUCLOUD_HOME", str(gpucloud_home))

        # Create a sensitive file outside gpucloud_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside gpucloud_home pointing outside
        symlink = gpucloud_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes GPUCLOUD_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, gpucloud_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape GPUCLOUD_HOME."""
        gpucloud_home = tmp_path / ".gpucloud"
        gpucloud_home.mkdir()
        monkeypatch.setenv("GPUCLOUD_HOME", str(gpucloud_home))
        ...
        sensitive.write_text("{}")
        self._write_config(gpucloud_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(gpucloud_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside GPUCLOUD_HOME via config must still mount."""
        gpucloud_home = tmp_path / ".gpucloud"
        gpucloud_home.mkdir()
        monkeypatch.setenv("GPUCLOUD_HOME", str(gpucloud_home))

        (gpucloud_home / "oauth.json").write_text("{}")
        self._write_config(gpucloud_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        gpucloud_home = tmp_path / ".gpucloud"
        gpucloud_home.mkdir()
        (gpucloud_home / "cache" / "documents").mkdir(parents=True)
        (gpucloud_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("GPUCLOUD_HOME", str(gpucloud_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.gpucloud/cache/documents" in paths
        assert "/root/.gpucloud/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.gpucloud/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        gpucloud_home = tmp_path / ".gpucloud"
        gpucloud_home.mkdir()
        # Use legacy dir name — get_gpucloud_dir prefers old if it exists
        (gpucloud_home / "document_cache").mkdir()
        (gpucloud_home / "image_cache").mkdir()
        monkeypatch.setenv("GPUCLOUD_HOME", str(gpucloud_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(gpucloud_home / "document_cache") in host_paths
        assert str(gpucloud_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.gpucloud/cache/documents" in container_paths
        assert "/root/.gpucloud/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        gpucloud_home = tmp_path / ".gpucloud"
        img_dir = gpucloud_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.gpucloud/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.gpucloud")
            == "/home/agent/.gpucloud/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        gpucloud_home = tmp_path / ".gpucloud"
        (gpucloud_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("GPUCLOUD_HOME", str(gpucloud_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(gpucloud_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        gpucloud_home = tmp_path / ".gpucloud"
        doc_dir = gpucloud_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        gpucloud_home = tmp_path / ".gpucloud"
        ss_dir = gpucloud_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.gpucloud/cache/screenshots/session_abc/screen1.png"
