...
# ``model_tools`` import time, so ``gpucloud status``, ``gpucloud chat``, etc.
...
# ``gpucloud_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "GPUCLOUD is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a gpucloud MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${GPUCLOUD_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _gpucloud_home = _get_gpucloud_home()
        except (ModuleNotFoundError, ImportError):
            _gpucloud_home = _Path.home() / ".gpucloud"
        self._thread_count_store = _ThreadCountStore(
            _gpucloud_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "GPUCLOUD is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("GPUCLOUD_HOME", str(_Path.home() / ".gpucloud"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${GPUCLOUD_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a gpucloud MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'GPUCLOUD is thinking…' marker message.
        ...
        "GPUCLOUD is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "GPUCLOUD is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "GPUCLOUD is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "GPUCLOUD is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "GPUCLOUD is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``gpucloud setup``.

    The setup wizard at ``gpucloud_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.gpucloud/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. gpucloud-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.gpucloud/.env")
    print_info("Restart the gateway: gpucloud gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``gpucloud cron`` running as a
    separate process from ``gpucloud gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the GPUCLOUD plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'gpucloud-agent[google_chat]'",
        ...
        # the "GPUCLOUD is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'GPUCLOUD is thinking…' marker message "
