...
Every GPUCLOUD request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to GPUCLOUD Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=gpucloud-agent",
        "client=gpucloud-client-v<__version__>",
    ]

The version is sourced live from ``gpucloud_cli.__version__`` so it auto-aligns
...
``gpucloud_cli.__version__`` is the canonical source of truth.
"""
...
def _gpucloud_version() -> str:
    """Return the current GPUCLOUD release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``gpucloud_cli`` cannot be imported (should
...
def gpucloud_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=gpucloud-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=gpucloud-client-v{_gpucloud_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=gpucloud-agent", gpucloud_client_tag()]
