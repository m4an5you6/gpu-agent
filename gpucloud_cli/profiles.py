"""
Profile management for multiple isolated GPUCLOUD instances.

Each profile is a fully independent GPUCLOUD_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.gpucloud/profiles/<name>/`` by default.

The "default" profile is ``~/.gpucloud`` itself — backward compatible,
zero migration needed.

Usage::

    gpucloud profile create coder          # fresh profile + bundled skills
    gpucloud profile create coder --clone  # also copy config, .env, SOUL.md, skills
    gpucloud profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    gpucloud -p coder chat                 # or via flag
    gpucloud profile use coder             # set as sticky default
    gpucloud profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See gpucloud_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.gpucloud``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   gpucloud-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `gpucloud profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `gpucloud
...
# `gpucloud skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         GPUCLOUD infrastructure directories that only the default profile
         (``~/.gpucloud``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_gpucloud_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.gpucloud) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "gpucloud-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "gpucloud", "default", "test", "tmp", "root", "sudo",
})

# GPUCLOUD subcommands that cannot be used as profile names/aliases
_GPUCLOUD_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the gpucloud root, NOT to the current GPUCLOUD_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where GPUCLOUD_HOME points outside
    ``~/.gpucloud``, profiles live under ``GPUCLOUD_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_gpucloud_home() / "profiles"


def _get_default_gpucloud_home() -> Path:
    """Return the default (pre-profile) GPUCLOUD_HOME path.

    In standard deployments this is ``~/.gpucloud``.
    In Docker/custom deployments where GPUCLOUD_HOME is outside ``~/.gpucloud``
    (e.g. ``/opt/data``), returns GPUCLOUD_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_gpucloud_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the GPUCLOUD installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its GPUCLOUD_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_gpucloud_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, gpucloud subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _GPUCLOUD_SUBCOMMANDS:
        return f"'{canon}' conflicts with a gpucloud subcommand"
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``gpucloud -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``gpucloud profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``gpucloud profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing GPUCLOUD config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_gpucloud_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``gpucloud update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.gpucloud)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.gpucloud/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `gpucloud update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`gpucloud profile create --no-skills`).\n"
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `gpucloud profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `gpucloud -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches GPUCLOUD_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``gpucloud profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "GPUCLOUD_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.gpucloud).\n"
            "To remove everything, use: gpucloud uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from gpucloud_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set GPUCLOUD_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("GPUCLOUD_HOME")
    try:
        os.environ["GPUCLOUD_HOME"] = str(profile_dir)
        from gpucloud_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["GPUCLOUD_HOME"] = old_home
        elif "GPUCLOUD_HOME" in os.environ:
            del os.environ["GPUCLOUD_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.gpucloud/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: gpucloud profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from GPUCLOUD_HOME.

    Returns ``"default"`` if GPUCLOUD_HOME is not set or points to ``~/.gpucloud``.
    Returns the profile name if GPUCLOUD_HOME points into ``~/.gpucloud/profiles/<name>``.
    Returns ``"custom"`` if GPUCLOUD_HOME is set to an unrecognized path.
    """
    from gpucloud_constants import get_gpucloud_home
    gpucloud_home = get_gpucloud_home()
    resolved = gpucloud_home.resolve()

    default_resolved = _get_default_gpucloud_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.gpucloud itself — its parent is ~/ and its
        # directory name is ".gpucloud", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: gpucloud profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.gpucloud itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.gpucloud). "
            "Specify a different name: gpucloud profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="gpucloud_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"gpucloud_{old_name}"
    legacy_old_host = f"gpucloud.{old_name}"
    new_host = f"gpucloud_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_gpucloud_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("gpucloud_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a GPUCLOUD_HOME path string.

    Called early in the CLI entry point, before any gpucloud modules
    are imported, to set the GPUCLOUD_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
