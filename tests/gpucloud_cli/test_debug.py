"""Tests for ``gpucloud debug`` CLI command and debug utilities."""
...
@pytest.fixture
def gpucloud_home(tmp_path, monkeypatch):
    """Set up an isolated GPUCLOUD_HOME with minimal logs."""
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from gpucloud_cli.debug import _upload_paste_rs
        ...
        with patch("gpucloud_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "gpucloud_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from gpucloud_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from gpucloud_cli.debug import upload_to_pastebin

        with patch("gpucloud_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("gpucloud_cli.debug._upload_paste_rs",
             ...
             patch("gpucloud_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, gpucloud_home):
        from gpucloud_cli.debug import _capture_log_snapshot
    ...
    def test_empty_primary_reports_file_empty(self, gpucloud_home):
        """Empty primary (no .1 fallback) surfaces as '(file empty)', not missing."""
        (gpucloud_home / "logs" / "agent.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, gpucloud_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = gpucloud_home / "logs" / "agent.log"
        from gpucloud_cli import debug
    ...
    def test_truncates_large_file(self, gpucloud_home):
        """Files larger than max_bytes get tail-truncated."""
        from gpucloud_cli.debug import _capture_log_snapshot

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (gpucloud_home / "logs" / "agent.log").write_text(big_content * 200)

        snap = _capture_log_snapshot("agent", tail_lines=10, max_bytes=1024)
        assert snap.full_text is not None
        assert "truncated" in snap.full_text

    def test_keeps_first_line_when_truncation_on_boundary(self, gpucloud_home):
        ...
        (gpucloud_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, gpucloud_home):
    ...
    def test_unknown_log_returns_none(self, gpucloud_home):
        from gpucloud_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, gpucloud_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from gpucloud_cli.debug import _capture_log_snapshot

        logs_dir = gpucloud_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, gpucloud_home):
    ...
    def test_falls_back_when_primary_empty(self, gpucloud_home):
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of GPUCLOUD_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def gpucloud_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated GPUCLOUD_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".gpucloud"
        home.mkdir()
        monkeypatch.setenv("GPUCLOUD_HOME", str(home))
        ...
        monkeypatch.delenv("GPUCLOUD_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, gpucloud_home_with_secret):
        from gpucloud_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, gpucloud_home_with_secret):
        from gpucloud_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, gpucloud_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when GPUCLOUD_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("GPUCLOUD_REDACT_SECRETS", "false")

        from gpucloud_cli.debug import _capture_log_snapshot

        assert os.environ.get("GPUCLOUD_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, gpucloud_home_with_secret
    ):
        from gpucloud_cli.debug import _capture_log_snapshot

        log_path = gpucloud_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, gpucloud_home_with_secret):
        from gpucloud_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from gpucloud_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, gpucloud_home):
        from gpucloud_cli.debug import collect_debug_report

        with patch("gpucloud_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- gpucloud dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- gpucloud dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, gpucloud_home):
        from gpucloud_cli.debug import collect_debug_report

        with patch("gpucloud_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, gpucloud_home):
        from gpucloud_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, gpucloud_home):
        from gpucloud_cli.debug import collect_debug_report
    ...
    def test_report_includes_desktop_log(self, gpucloud_home):
        from gpucloud_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, gpucloud_home, capsys):
        ...
        with patch("gpucloud_cli.dump.run_dump"), \
             patch("gpucloud_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("gpucloud_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, gpucloud_home, capsys):
        ...
        with patch("gpucloud_cli.dump.run_dump"), \
             patch(
                 "gpucloud_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, gpucloud_home, capsys):
    ...
    def test_share_uploads_four_pastes(self, gpucloud_home, capsys):
        ...
        with patch("gpucloud_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- gpucloud dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- gpucloud dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- gpucloud dump ---" in gateway_paste
        assert "--- full gateway.log ---" in gateway_paste
        desktop_paste = uploaded_content[3]
        assert "--- gpucloud dump ---" in desktop_paste
        assert "--- full desktop.log ---" in desktop_paste

    def test_share_keeps_report_and_full_log_on_same_snapshot(self, gpucloud_home, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from gpucloud_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("gpucloud_cli.dump.run_dump"), \
             patch("gpucloud_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("gpucloud_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, gpucloud_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, gpucloud_home, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, gpucloud_home_with_secret, capsys
    ):
        ...
        with patch("gpucloud_cli.dump.run_dump"), \
             patch("gpucloud_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("gpucloud_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "gpucloud debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, gpucloud_home):
        from gpucloud_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from gpucloud_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from gpucloud_cli.debug import delete_paste
        ...
        with patch("gpucloud_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.gpucloud/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``gpucloud debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, gpucloud_home):
        ...
        import ast
        import inspect
        from gpucloud_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, gpucloud_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from gpucloud_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, gpucloud_home):
    ...
    def test_merges_with_existing_pending(self, gpucloud_home):
        """Subsequent calls merge into existing pending.json."""
        from gpucloud_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, gpucloud_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, gpucloud_home):
    ...
    def test_sweep_deletes_expired_entries(self, gpucloud_home):
        ...
        with patch("gpucloud_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, gpucloud_home):
        from gpucloud_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("gpucloud_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, gpucloud_home):
        ...
        with patch(
            "gpucloud_cli.debug.delete_paste",
    ...
    def test_sweep_drops_entries_past_grace_window(self, gpucloud_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, gpucloud_home):
        from gpucloud_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("gpucloud_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, gpucloud_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: gpucloud debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from gpucloud_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("gpucloud_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("gpucloud_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, gpucloud_home, capsys):
        from gpucloud_cli.debug import run_debug_share
        ...
        with patch("gpucloud_cli.dump.run_dump"), \
             ...
             patch("gpucloud_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, gpucloud_home, capsys):
        from gpucloud_cli.debug import run_debug_share
        ...
        with patch("gpucloud_cli.dump.run_dump"), \
             patch("gpucloud_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("gpucloud_cli.debug._schedule_auto_delete"):
            run_debug_share(args)

        out = capsys.readouterr().out
        assert "public paste service" in out

    def test_local_no_privacy_notice(self, gpucloud_home, capsys):
        from gpucloud_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    Backs both ``gpucloud debug share`` (CLI) and ``POST /api/ops/debug-share``
    (dashboard). The dashboard renders ``urls`` as real, copyable links, so the
    contract here is the return value, not stdout.
    """

    def test_returns_structured_urls(self, gpucloud_home):
        from gpucloud_cli.debug import build_debug_share, DebugShareResult
        ...
        with patch("gpucloud_cli.dump.run_dump"), patch(
            "gpucloud_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("gpucloud_cli.debug._schedule_auto_delete"):
    ...
    def test_skips_missing_logs_without_failure(self, gpucloud_home):
        from gpucloud_cli.debug import build_debug_share

        # Remove desktop.log so it should be neither uploaded nor reported failed.
        (gpucloud_home / "logs" / "desktop.log").unlink()

        with patch("gpucloud_cli.dump.run_dump"), patch(
            "gpucloud_cli.debug.upload_to_pastebin",
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, gpucloud_home):
        from gpucloud_cli.debug import build_debug_share
    ...
    def test_optional_log_failure_is_collected_not_raised(self, gpucloud_home):
        from gpucloud_cli.debug import build_debug_share
    ...
    def test_required_report_failure_raises(self, gpucloud_home):
        from gpucloud_cli.debug import build_debug_share
