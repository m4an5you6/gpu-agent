...
description: "Plugins shipped with GPUCLOUD Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

GPUCLOUD ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.gpucloud/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a GPUCLOUD Plugin](/guides/build-a-gpucloud-plugin) to write your own.
...
2. **User** — `~/.gpucloud/plugins/<name>/`
3. **Project** — `./.gpucloud/plugins/<name>/` (requires `GPUCLOUD_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `gpucloud_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `gpucloud memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `gpucloud plugins list` and the interactive `gpucloud plugins` UI), but none load until you explicitly enable them:

```bash
gpucloud plugins enable disk-cleanup
```

Or via `~/.gpucloud/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer GPUCLOUD. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
gpucloud plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `gpucloud plugins enable <name>`.
...
| `gpucloud-achievements` | dashboard tab | Steam-style collectible badges generated from your real GPUCLOUD session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `gpucloud memory` and `gpucloud plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `GPUCLOUD_HOME` or `/tmp/gpucloud-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under GPUCLOUD_HOME | always | Never |
...
**State** — everything lives at `$GPUCLOUD_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `GPUCLOUD_HOME` or `/tmp/gpucloud-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `gpucloud plugins enable disk-cleanup` (or check the box in `gpucloud plugins`).

**Disabling again:** `gpucloud plugins disable disk-cleanup`.
...
**Enabling:** `gpucloud plugins enable security-guidance` (or check the box in `gpucloud plugins`).

**Disabling again:** `gpucloud plugins disable security-guidance`.
...
Traces GPUCLOUD turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of GPUCLOUD' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `gpucloud logs`.
...
```bash
gpucloud tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart GPUCLOUD and the next turn ships a trace.
...
Then put the credentials in `~/.gpucloud/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "GPUCLOUD turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `GPUCLOUD_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the GPUCLOUD session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `gpucloud chat` session lives under one Langfuse session.

**Verify:**

```bash
gpucloud plugins list                 # observability/langfuse should show "enabled"
gpucloud chat -q "hello"              # check the Langfuse UI for a "GPUCLOUD turn" trace
```
...
| `GPUCLOUD_LANGFUSE_RELEASE` | — | Release/version tag |
| `GPUCLOUD_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `GPUCLOUD_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `GPUCLOUD_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

GPUCLOUD-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — GPUCLOUD-prefixed wins when both are set.
...
**Disabling:** `gpucloud plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.gpucloud/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
gpucloud plugins enable google_meet
...
**Disabling:** `gpucloud plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.gpucloud/cache/google_meet/` until you remove them.

### gpucloud-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real GPUCLOUD session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with GPUCLOUD feature changes.

**How it works:**

- Scans your entire `~/.gpucloud/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$GPUCLOUD_HOME/plugins/gpucloud-achievements/state.json`
...
| Secret | Hidden until GPUCLOUD detects the first related signal in your history |

**API** — routes mount under `/api/plugins/gpucloud-achievements/`:
...
**State files** — live under `$GPUCLOUD_HOME/plugins/gpucloud-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across GPUCLOUD updates. |
...
**Enabling:** Nothing to enable — `gpucloud-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `gpucloud dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/gpucloud-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.gpucloud/plugins/gpucloud-achievements/` that ships no dashboard. The plugin's state files under `$GPUCLOUD_HOME/plugins/gpucloud-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other GPUCLOUD plugin — see [Build a GPUCLOUD Plugin](/guides/build-a-gpucloud-plugin). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.gpucloud/plugins/<name>/`
- Manifest source is reported as `bundled` in `gpucloud plugins list`
