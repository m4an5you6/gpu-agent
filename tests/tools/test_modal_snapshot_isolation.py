...
def _restore_tool_modules():
    original_gpucloud_home = os.environ.get("GPUCLOUD_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "gpucloud_cli"
        or name.startswith("gpucloud_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_gpucloud_home is None:
            os.environ.pop("GPUCLOUD_HOME", None)
        else:
            os.environ["GPUCLOUD_HOME"] = original_gpucloud_home
        _reset_modules(("tools", "gpucloud_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    gpucloud_cli = types.ModuleType("gpucloud_cli")
    gpucloud_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["gpucloud_cli"] = gpucloud_cli
    gpucloud_home = tmp_path / "gpucloud-home"
    os.environ["GPUCLOUD_HOME"] = str(gpucloud_home)
    sys.modules["gpucloud_cli.config"] = types.SimpleNamespace(
        get_gpucloud_home=lambda: gpucloud_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="gpucloud-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": gpucloud_home / "modal_snapshots.json",
