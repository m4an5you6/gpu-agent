# GPUCLOUD Agent Has Had "Routines" Since March
...
GPUCLOUD equivalent — works today:
```bash
...
GPUCLOUD equivalent — works today:
```bash
gpucloud webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working GPUCLOUD implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | GPUCLOUD Agent |
|---|---|---|
...
## Things GPUCLOUD Does That Routines Can't
...
GPUCLOUD has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
GPUCLOUD Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
gpucloud gateway setup    # enable webhooks
gpucloud webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [gpucloud-agent.nousresearch.com/docs/guides/automation-templates](https://gpucloud-agent.nousresearch.com/docs/guides/automation-templates)

Documentation: [gpucloud-agent.nousresearch.com](https://gpucloud-agent.nousresearch.com)

GitHub: [github.com/NousResearch/gpucloud-agent](https://github.com/NousResearch/gpucloud-agent)

---

*GPUCLOUD Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
