...
class TestIRCProtocolHelpers:
    ...
    def test_parse_numeric_reply(self):
        ...
        assert msg["params"] == ["gpucloud-bot", "Welcome to IRC"]
...
class TestIRCAdapterInit:
    ...
    def test_init_from_config_extra(self, monkeypatch):
        ...
        assert adapter.channel == "#gpucloud-dev"
...
class TestIRCAdapterMessageParsing:
    ...
    async def test_handle_nick_collision(self, adapter):
        ...
        assert adapter._current_nick == "gpucloud_"
        sent = writer.write.call_args[0][0]
        assert b"NICK gpucloud_" in sent
    ...
    async def test_handle_addressed_channel_message(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG #test :gpucloud: hello there")
    ...
    async def test_ignores_own_messages(self, adapter):
        ...
        await adapter._handle_line(":gpucloud!bot@host PRIVMSG #test :my own msg")
        assert len(dispatched) == 0
    ...
    async def test_allowed_users_case_insensitive(self, monkeypatch):
        ...
        await adapter._handle_line(":admin!u@host PRIVMSG #test :gpucloud: hello")
        assert len(dispatched) == 1
    ...
    async def test_unauthorized_user_blocked(self, monkeypatch):
        ...
        await adapter._handle_line(":eve!u@host PRIVMSG #test :gpucloud: hello")
        assert len(dispatched) == 0
    ...
    async def test_nick_collision_retry(self, adapter):
        ...
        await adapter._handle_line(":server 433 * gpucloud :Nickname in use")
        assert adapter._current_nick == "gpucloud_"
        await adapter._handle_line(":server 433 * gpucloud_ :Nickname in use")
        assert adapter._current_nick == "gpucloud_1"
        await adapter._handle_line(":server 433 * gpucloud_1 :Nickname in use")
        assert adapter._current_nick == "gpucloud_2"
...
class TestIRCStandaloneSend:
    ...
    async def test_standalone_send_completes_handshake_and_sends_privmsg(self, monkeypatch):
        ...
        monkeypatch.setenv("IRC_NICKNAME", "gpucloudbot")
        monkeypatch.setenv("IRC_USE_TLS", "false")

        # Server greets us with 001 RPL_WELCOME, then nothing for QUIT drain.
        conn = _FakeIRCConnection([b":server 001 gpucloudbot-cron :Welcome"])
        ...
        assert any(line.startswith("NICK gpucloudbot-cron") for line in sent_lines)
        assert any(line.startswith("USER gpucloudbot-cron 0 * :GPUCLOUD Agent (cron)")
    ...
    async def test_standalone_send_strips_crlf_from_message_body(self, monkeypatch):
        ...
        assert all(line.startswith("NICK gpucloudbot-cron") for line in nick_lines)
    ...
    async def test_standalone_send_joins_channel_before_privmsg(self, monkeypatch):
        ...
        conn = _FakeIRCConnection([
            b":server 001 gpucloudbot-cron :Welcome",
            b":server 366 gpucloudbot-cron #cron :End of /NAMES list.",
        ])
