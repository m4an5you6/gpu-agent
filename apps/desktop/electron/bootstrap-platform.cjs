 ...
 * null to keep it enabled. `GPUCLOUD_DESKTOP_DISABLE_GPU` overrides detection
...
function detectRemoteDisplay(options = {}) {
  ...
  const override = String(env.GPUCLOUD_DESKTOP_DISABLE_GPU || '')
    .trim()
    .toLowerCase()
  if (GPU_OVERRIDE_ON.has(override)) return 'override (GPUCLOUD_DESKTOP_DISABLE_GPU)'
