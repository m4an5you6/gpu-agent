...
//!
//! Driven when the installer is launched as `GPUCLOUD-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old GPUCLOUD desktop process to fully exit (so the venv
//!      shim is free; otherwise `gpucloud update` aborts with exit code 2),
//!   2. run `gpucloud update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in gpucloud_cli/main.py),
//!   3. run `gpucloud desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `gpucloud update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_gpucloud) and
...
/// `gpucloud update` exit code meaning "another gpucloud process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// gpucloud_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release the venv shim
/// before giving up and letting `gpucloud update`'s own guard decide.
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
        ...
        .is_err()
    {
        ...
        let mut stages = vec![
            stage_info("update", "Updating GPUCLOUD"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    let gpucloud_home = crate::paths::gpucloud_home();
    let install_root = gpucloud_home.join("gpucloud-agent");
    ...
    let gpucloud = resolve_gpucloud(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the gpucloud CLI under {}. Is GPUCLOUD installed? \
    ...
    // async on Windows. If it still holds the venv shim, `gpucloud update`
    // aborts with exit 2. Give it a bounded window to clear.
    wait_for_venv_free(&install_root, &app).await;

    // ---- stage 1: gpucloud update -----------------------------------------
    // Pass --branch so `gpucloud update` targets the branch this installer was
    ...
    // without --branch, `gpucloud update` switches the checkout to `main` (a
    ...
    // --force skips `gpucloud update`'s Windows running-exe guard (which would
    ...
    // time `gpucloud update` runs there is no legitimate gpucloud.exe to protect,
    // and the guard would only produce a false "GPUCLOUD is still running" stop.
    ...
    // Retry-once for the update-boundary crash. `gpucloud update` lazily imports
    ...
    // second `gpucloud update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close GPUCLOUD" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "GPUCLOUD is still running. Close all GPUCLOUD windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "gpucloud update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::gpucloud_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 2: gpucloud desktop --build-only ----------------------------
    // `gpucloud update` deliberately does NOT build apps/desktop (it installs
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `gpucloud desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch GPUCLOUD manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_gpucloud_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
async fn wait_for_venv_free(install_root: &Path, app: &AppHandle) {
    let shim = venv_gpucloud(install_root);
    let deadline = Instant::now() + DESKTOP_EXIT_WAIT;

    emit_log(app, Some("update"), LogStream::Stdout, "[update] waiting for GPUCLOUD to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend gpucloud.exe (or a grandchild it spawned)
            ...
            // every gpucloud.exe except ourselves, then give the OS a beat to
            // unload the image.
            emit_log(
                app,
                Some("update"),
                LogStream::Stdout,
                "[update] GPUCLOUD still holding the venv shim; force-killing stragglers…",
            );
            force_kill_other_gpucloud();
...
/// Force-kill any `gpucloud.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `gpucloud.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside `wait_for_venv_free`,
/// which completes BEFORE we spawn `venv\Scripts\gpucloud.exe update`. At this
/// point no update-driven gpucloud.exe exists yet, so the only gpucloud.exe images
...
/// Spawn `gpucloud <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv gpucloud shim under an install root, regardless of existence.
fn venv_gpucloud(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("gpucloud.exe")
    } else {
        install_root.join("venv").join("bin").join("gpucloud")
    }
}

/// Resolve the gpucloud CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `gpucloud` on PATH.
fn resolve_gpucloud(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_gpucloud(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "gpucloud.exe" } else { "gpucloud" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let gpucloud_home = crate::paths::gpucloud_home();
    let mut envs = vec![(
        "GPUCLOUD_HOME".to_string(),
        gpucloud_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        gpucloud_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_gpucloud_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no GPUCLOUD.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.gpucloud-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.gpucloud-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::gpucloud_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_gpucloud_is_under_install_root() {
        let root = Path::new("/x/gpucloud-agent");
        let shim = venv_gpucloud(root);
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/GPUCLOUD.app"]),
            Some(PathBuf::from("/Applications/GPUCLOUD.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("GPUCLOUD.app");
        let tmp = base.join("GPUCLOUD.app.gpucloud-update-new");
        let old = base.join("GPUCLOUD.app.gpucloud-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("GPUCLOUD.app");
        let tmp = base.join("GPUCLOUD.app.gpucloud-update-new"); // intentionally absent
        let old = base.join("GPUCLOUD.app.gpucloud-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("GPUCLOUD.app");
        let tmp = base.join("GPUCLOUD.app.gpucloud-update-new"); // absent
        let old = base.join("GPUCLOUD.app.gpucloud-update-old");
        write_marker(&target, "OLD");
