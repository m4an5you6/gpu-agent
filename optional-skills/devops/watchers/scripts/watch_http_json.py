...
Usage (via cron with --no-agent):
    ...
    gpucloud cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$GPUCLOUD_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "GPUCLOUD-Watcher/1.0"})
