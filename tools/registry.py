"""Central registry for all gpucloud-agent tools.
...
def discover_builtin_tools(tools_dir: Optional[Path] = None) -> List[str]:
    ...
    from gpucloud_cli.gpucloud_phase2 import tool_module_enabled
...
# timescales. Cache results for ~30 s so env-var flips via ``gpucloud tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``gpucloud tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``gpucloud tools enable foo``)
