...
See: https://github.com/NousResearch/gpucloud-agent/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for gpucloud_constants.get_subprocess_home()."""

    def test_returns_none_when_gpucloud_home_unset(self, monkeypatch):
    ...
    def test_returns_path_when_home_dir_exists(self, tmp_path, monkeypatch):
        gpucloud_home = tmp_path / ".gpucloud"
        gpucloud_home.mkdir()
        profile_home = gpucloud_home / "home"
        profile_home.mkdir()
        monkeypatch.setenv("GPUCLOUD_HOME", str(gpucloud_home))
        from gpucloud_constants import get_subprocess_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        base = tmp_path / ".gpucloud" / "profiles"
        ...
        monkeypatch.setenv("GPUCLOUD_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("GPUCLOUD_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_gpucloud_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_gpucloud_home_override(profile)
        try:
            assert get_gpucloud_home() == profile
            release.set()
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(gpucloud_home / "home")
    ...
    def test_no_injection_when_gpucloud_home_unset(self, monkeypatch):
        monkeypatch.delenv("GPUCLOUD_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["GPUCLOUD_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from gpucloud_cli.profiles import _PROFILE_DIRS
