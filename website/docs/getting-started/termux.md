...
description: "Run GPUCLOUD Agent directly on an Android phone with Termux"
---

# GPUCLOUD on Android with Termux

This is the tested path for running GPUCLOUD Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop GPUCLOUD from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

GPUCLOUD now ships a Termux-aware installer path:
...
- links `gpucloud` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `gpucloud` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/gpucloud" "$PREFIX/bin/gpucloud"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `gpucloud` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.gpucloud/.env`.
...
### `gpucloud doctor` says ripgrep or Node is missing
