...
Pure logic: walks a GPUCLOUD config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `gpucloud doctor` and a future `gpucloud migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a GPUCLOUD config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a GPUCLOUD config and return retirement issues.
