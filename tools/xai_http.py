...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``gpucloud tools`` repaint, tool-registration scans,
    ...
    2. ``~/.gpucloud/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_gpucloud_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.gpucloud/.env`` first, then ``os.environ``.

    Wraps :func:`gpucloud_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from gpucloud_cli.config import get_env_value as _gpucloud_get_env_value

        value = _gpucloud_get_env_value(name)
...
def gpucloud_xai_user_agent() -> str:
    """Return a stable GPUCLOUD-specific User-Agent for xAI HTTP calls."""
    try:
        from gpucloud_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"GPUCLOUD-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers GPUCLOUD-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``gpucloud_cli.config.get_env_value`` so keys
    stored in ``~/.gpucloud/.env`` (the standard GPUCLOUD location) are honored —
