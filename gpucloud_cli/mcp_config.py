"""
MCP Server Management CLI — ``gpucloud mcp`` subcommand.

Implements ``gpucloud mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.gpucloud/config.yaml under the ``mcp_servers`` key.
"""
...
from gpucloud_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_gpucloud_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from gpucloud_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.gpucloud/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``gpucloud mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from tools.mcp_oauth import GPUCLOUDTokenStorage
        return GPUCLOUDTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # gpucloud_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  gpucloud mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  gpucloud mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  gpucloud mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_gpucloud_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: gpucloud mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_gpucloud_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `gpucloud mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  gpucloud mcp add <name> --url <endpoint>')
        _info('  gpucloud mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `gpucloud mcp remove` + `gpucloud mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `gpucloud mcp login " + name + "`.")
            return
        if tools:
...
# ─── gpucloud mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'gpucloud mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``gpucloud mcp`` subcommands."""
    ...
    if action == "picker":
        from gpucloud_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from gpucloud_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from gpucloud_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `gpucloud plugin`.
        from gpucloud_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("gpucloud mcp                                    Open the catalog picker (default)")
        _info("gpucloud mcp catalog                            List Nous-approved MCPs")
        _info("gpucloud mcp install <name>                     Install a catalog MCP")
        _info("gpucloud mcp serve                              Run as MCP server")
        _info("gpucloud mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("gpucloud mcp add <name> --command <cmd>         Add a stdio server")
        _info("gpucloud mcp add <name> --preset <preset>       Add from a known preset")
        _info("gpucloud mcp remove <name>                      Remove a server")
        _info("gpucloud mcp list                               List configured servers")
        _info("gpucloud mcp test <name>                        Test connection")
        _info("gpucloud mcp configure <name>                   Toggle tools")
        _info("gpucloud mcp login <name>                       Re-authenticate OAuth")
        print()
