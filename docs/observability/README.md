# GPUCLOUD Observer Hooks

GPUCLOUD observer hooks are the read-only telemetry contract for plugins that
...
They do not replace GPUCLOUD' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. GPUCLOUD catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so GPUCLOUD only builds
