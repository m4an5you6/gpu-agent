"""Tests for gpucloud_cli configuration management."""
...
class TestGetGPUCLOUDHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            assert home == Path.home() / ".gpucloud"

    def test_env_override(self):
        with patch.dict(os.environ, {"GPUCLOUD_HOME": "/custom/path"}):
            home = get_gpucloud_home()
            assert home == Path("/custom/path")


class TestEnsureGPUCLOUDHome:
    def test_creates_subdirs(self, tmp_path):
        with patch.dict(os.environ, {"GPUCLOUD_HOME": str(tmp_path)}):
            ensure_gpucloud_home()
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``gpucloud logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from gpucloud_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"GPUCLOUD_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="gpucloud_cli.config"):
                config = load_config()
            ...
            # stands out at gpucloud startup before logging is configured)
            captured = capsys.readouterr()
            assert "gpucloud config:" in captured.err
    ...
    def test_dedup_on_repeated_load_same_file(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"GPUCLOUD_HOME": str(tmp_path)}):
            ...
            assert "gpucloud config:" in first
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"GPUCLOUD_HOME": str(tmp_path)}):
            ...
            assert "gpucloud config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"GPUCLOUD_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"GPUCLOUD_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"GPUCLOUD_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"GPUCLOUD_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"GPUCLOUD_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from gpucloud_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """GPUCLOUD_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `gpucloud setup`) lets a
        ...
        via config.yaml; GPUCLOUD_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from gpucloud_cli.config import OPTIONAL_ENV_VARS
        assert "GPUCLOUD_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``GPUCLOUD_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next GPUCLOUD
    process on next startup via the dotenv → ``os.environ`` chain in
    ``gpucloud_cli/env_loader.py``.
    ...
    def _gpucloud_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "GPUCLOUD_SPOTIFY_CLIENT_ID",
            "GPUCLOUD_QWEN_BASE_URL",
            "GPUCLOUD_MAX_ITERATIONS",
        ],
    )
    def test_gpucloud_integration_keys_still_writable(self, allowed_key):
        """``GPUCLOUD_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``GPUCLOUD_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``GPUCLOUD_*``) keep working. The denylist
