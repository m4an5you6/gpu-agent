...
# (e.g. nix-built gpucloud — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/gpucloud-agent.git"
...
def _fetch_pypi_latest(package: str = "gpucloud-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a GPUCLOUD update is available.

    Two paths: if ``GPUCLOUD_REVISION`` is set (nix builds embed it), compare
    ...
    if embedded_rev:
        ...
        # $GPUCLOUD_HOME/gpucloud-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = gpucloud_home / "gpucloud-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active GPUCLOUD git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$GPUCLOUD_HOME/gpucloud-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``gpucloud_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from gpucloud_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/gpucloud-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    GPUCLOUD checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/gpucloud-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"GPUCLOUD Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else GPUCLOUD_CADUCEUS
    except Exception:
        _bskin = None
        _hero = GPUCLOUD_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        from gpucloud_cli.codex_runtime_switch import get_current_runtime
        from gpucloud_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from gpucloud_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else GPUCLOUD_AGENT_LOGO
        console.print(_logo)
        console.print()
