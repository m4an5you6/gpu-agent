"""``gpucloud logs`` — view and filter GPUCLOUD log files.
...
Usage examples::
    ...
    gpucloud logs                    # last 50 lines of agent.log
    gpucloud logs -f                 # follow agent.log in real time
    gpucloud logs errors             # last 50 lines of errors.log
    gpucloud logs gateway -n 100    # last 100 lines of gateway.log
    gpucloud logs gui -f            # follow gui.log (dashboard/pty/ws)
    gpucloud logs desktop -f        # follow desktop.log (Electron app boot/backend)
    gpucloud logs --level WARNING    # only WARNING+ lines
    gpucloud logs --session abc123   # filter by session ID substring
    gpucloud logs --component tools  # only tool-related lines
    gpucloud logs --since 1h         # lines from the last hour
    gpucloud logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_gpucloud_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when GPUCLOUD runs — try 'gpucloud chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_gpucloud_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_gpucloud_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_gpucloud_home()}/logs/")
        return

    print(f"Log files in {display_gpucloud_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'gpucloud chat' to generate logs)")
