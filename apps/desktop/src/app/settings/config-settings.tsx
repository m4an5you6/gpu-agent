...
import {
  getElevenLabsVoices,
  getGPUCLOUDConfigDefaults,
  getGPUCLOUDConfigRecord,
  getGPUCLOUDConfigSchema,
  saveGPUCLOUDConfig
} from '@/gpucloud'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, GPUCLOUDConfigRecord } from '@/types/gpucloud'
...
export function ConfigSettings({
  ...
  const { t } = useI18n()
  const c = t.settings.config
  const [config, setConfig] = useState<GPUCLOUDConfigRecord | null>(null)
  const [_defaults, setDefaults] = useState<GPUCLOUDConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getGPUCLOUDConfigRecord(), getGPUCLOUDConfigDefaults(), getGPUCLOUDConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          await saveGPUCLOUDConfig(config)
  ...
  const updateConfig = (next: GPUCLOUDConfigRecord) => {
