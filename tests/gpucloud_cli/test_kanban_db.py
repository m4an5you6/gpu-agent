...
class TestSharedBoardPaths:
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: GPUCLOUD_HOME=/opt/hermes/profiles/coder;
        # `get_default_gpucloud_root()` walks up to /opt/hermes because
...
def test_resolve_hermes_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("GPUCLOUD_BIN", ".\\hermes.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)
...
def test_resolve_hermes_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    assert kb._resolve_hermes_argv() == [sys.executable, "-m", "gpucloud_cli.main"]
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from gpucloud_cli.kanban_db import connect, write_txn, create_task
    db = tmp_path / "test.db"
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from gpucloud_cli.kanban_db import connect, write_txn, _check_file_length_invariant
