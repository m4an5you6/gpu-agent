...
# 5. Symlinks the 'gpucloud' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ GPUCLOUD Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/gpucloud-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink gpucloud into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up gpucloud command..."

GPUCLOUD_BIN="$SCRIPT_DIR/venv/bin/gpucloud"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$GPUCLOUD_BIN" "$COMMAND_LINK_DIR/gpucloud"
echo -e "${GREEN}✓${NC} Symlinked gpucloud → $COMMAND_LINK_DISPLAY_DIR/gpucloud"
...
# ============================================================================
# Seed bundled skills into ~/.gpucloud/skills/
# ============================================================================

GPUCLOUD_SKILLS_DIR="${GPUCLOUD_HOME:-$HOME/.gpucloud}/skills"
mkdir -p "$GPUCLOUD_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.gpucloud/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$GPUCLOUD_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  gpucloud status        # Check configuration"
if is_termux; then
    echo "  gpucloud gateway       # Run gateway in foreground"
else
    echo "  gpucloud gateway install # Install gateway service (messaging + cron)"
fi
echo "  gpucloud cron list     # View scheduled jobs"
echo "  gpucloud doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m gpucloud_cli.main setup
fi
