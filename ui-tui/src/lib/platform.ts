 ...
 * gpucloud-ink sets ``key.meta`` for plain Alt/Option on every platform
 ...
 * (``gpucloud_cli/voice.py::normalize_voice_record_key_for_prompt_toolkit``)
 ...
 * modifier — but gpucloud-ink reports Alt as ``key.meta`` on many
...
export const parseVoiceRecordKey = (raw: unknown): ParsedVoiceRecordKey => {
  ...
  // On macOS gpucloud-ink reports Alt as ``key.meta``, which ``isActionMod``
...
export const isVoiceToggleKey = (
  ...
  switch (configured.mod) {
    case 'alt':
      ...
      // alt binding.
      //
      // Bare Escape on gpucloud-ink can arrive as ``key.meta=true`` on some
    ...
    case 'ctrl':
      ...
      // ``key.meta`` is gpucloud-ink's Alt signal and accepting it would
