...
Config file: `$GPUCLOUD_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `gpucloud` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `gpucloud-{identity}` → `gpucloud-coder`). |
...
All Supermemory API calls send `x-sm-source: gpucloud`, and document writes stamp
`metadata.sm_source: gpucloud`. This is a **functional routing key, not telemetry**:
it groups GPUCLOUD-written memories into a dedicated "GPUCLOUD" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per GPUCLOUD profile:

```json
{
  "container_tag": "gpucloud-{identity}"
}
```

For a profile named `coder`, this resolves to `gpucloud-coder`. The default profile resolves to `gpucloud-default`. Without `{identity}`, all profiles share the same container.
