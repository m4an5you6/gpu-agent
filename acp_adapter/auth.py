"""ACP auth helpers — detect and advertise GPUCLOUD authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "gpucloud-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active GPUCLOUD runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if GPUCLOUD can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for GPUCLOUD.
    ...
    not have GPUCLOUD provider credentials configured yet, so GPUCLOUD always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate GPUCLOUD using the currently configured "
