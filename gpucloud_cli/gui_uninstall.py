"""
GPUCLOUD Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``gpucloud desktop`` / ``gpucloud gui``)
     Built inside the agent checkout under ``$GPUCLOUD_HOME/gpucloud-agent/``:
       ...
       - ``$GPUCLOUD_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/GPUCLOUD.app`` or ``~/Applications/GPUCLOUD.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\GPUCLOUD`` (NSIS per-user)
...
the app name ("GPUCLOUD"), separate from ``$GPUCLOUD_HOME``:
  - macOS:   ``~/Library/Application Support/GPUCLOUD``
  - Windows: ``%APPDATA%\\GPUCLOUD``
  - Linux:   ``$XDG_CONFIG_HOME/GPUCLOUD`` (default ``~/.config/GPUCLOUD``)
...
``gpucloud uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(gpucloud_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return gpucloud_home / "gpucloud-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "GPUCLOUD"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "GPUCLOUD"
    if sys.platform == "win32":
        appdata = os.environ.get("APPDATA")
        base = Path(appdata) if appdata else (home / "AppData" / "Roaming")
        return base / "GPUCLOUD"
...
def source_built_gui_artifacts(gpucloud_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``gpucloud desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``gpucloud-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(gpucloud_home)
    ...
    return [
        ...
        gpucloud_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "GPUCLOUD" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/GPUCLOUD.app"),
            home / "Applications" / "GPUCLOUD.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\GPUCLOUD).
            local_base / "Programs" / "GPUCLOUD",
            # Older / alternate layout some builds used.
            local_base / "gpucloud-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "GPUCLOUD")
    else:
        ...
        paths += [
            data_base / "applications" / "gpucloud.desktop",
            data_base / "applications" / "GPUCLOUD.desktop",
        ]
    return paths


def agent_is_installed(gpucloud_home: Path) -> bool:
    """Return True when a usable Python agent install exists under GPUCLOUD_HOME.
    ...
    if (agent_root / "gpucloud_cli").is_dir():
        return True
...
def gui_is_installed(gpucloud_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(gpucloud_home):
        if p.exists():
            return True
...
def gui_install_summary(gpucloud_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = gpucloud_home if gpucloud_home is not None else get_gpucloud_home()
...
def uninstall_gui(gpucloud_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``gpucloud-agent/gpucloud_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$GPUCLOUD_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove gpucloud' or "
            "'sudo dnf remove gpucloud'). AppImage builds are a single file you "
