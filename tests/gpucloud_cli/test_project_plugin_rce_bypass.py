...
Execution via the ``GPUCLOUD_ENABLE_PROJECT_PLUGINS`` bypass in the web
server's dashboard plugin loader.

Two primitives combined into the original advisory chain:

1. ``gpucloud_cli.web_server._discover_dashboard_plugins`` opted into
   the untrusted ``./.gpucloud/plugins/`` source via
   ``os.environ.get("GPUCLOUD_ENABLE_PROJECT_PLUGINS")`` — truthy for
...
2. ``gpucloud_cli.web_server._mount_plugin_api_routes`` then imported
...
# ---------------------------------------------------------------------------
# Layer 1 — GPUCLOUD_ENABLE_PROJECT_PLUGINS env gate uses truthy semantics.
# ---------------------------------------------------------------------------
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        """Plant a project-source plugin under CWD's ``.gpucloud/plugins``
        and isolate the user-plugins dir to an empty tmp tree."""
        monkeypatch.setenv("GPUCLOUD_HOME", str(tmp_path / "home"))
        (tmp_path / "home").mkdir()
        cwd = tmp_path / "evil-repo"
        cwd.mkdir()
        monkeypatch.chdir(cwd)
        _write_plugin_manifest(
            cwd / ".gpucloud" / "plugins",
            "evil",
            {
                "name": "evil",
                "label": "Evil",
    ...
    def test_falsy_values_keep_project_plugins_disabled(
        ...
        if value == "":
            monkeypatch.delenv("GPUCLOUD_ENABLE_PROJECT_PLUGINS", raising=False)
        else:
            monkeypatch.setenv("GPUCLOUD_ENABLE_PROJECT_PLUGINS", value)

        plugins = web_server._get_dashboard_plugins(force_rescan=True)
        names = {p["name"] for p in plugins}
        assert "evil" not in names, (
            f"GPUCLOUD_ENABLE_PROJECT_PLUGINS={value!r} must NOT enable the "
...
class TestEndToEndPocBlocked:
    ...
    ``GPUCLOUD_ENABLE_PROJECT_PLUGINS=0`` (so the operator believed the
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("GPUCLOUD_ENABLE_PROJECT_PLUGINS", "0")
        ...
        for call in spec.call_args_list:
            ...
            assert module_name != "gpucloud_dashboard_plugin_evil"
            assert target != payload_py
            assert "evil-repo" not in target.parts
        assert "gpucloud_dashboard_plugin_evil" not in sys.modules
