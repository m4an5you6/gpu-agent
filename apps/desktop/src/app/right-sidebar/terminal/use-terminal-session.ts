...
const GPUCLOUD_PATHS_MIME = 'application/x-gpucloud-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(GPUCLOUD_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(GPUCLOUD_PATHS_MIME)
...
export function useTerminalSession({ cwd, onAddSelectionToChat }: UseTerminalSessionOptions) {
  ...
  useEffect(() => {
    ...
    const terminalApi = window.gpucloudDesktop?.terminal
    ...
    try {
      ...
      console.warn('[gpucloud-terminal] WebGL unavailable; falling back to DOM', err)
    }
