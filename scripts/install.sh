...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --hermes-home PATH   Deprecated alias for --gpucloud-home"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/NousResearch/gpucloud-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$GPUCLOUD_HOME/node/bin/node" ]; then
        export PATH="$GPUCLOUD_HOME/node/bin:$PATH"
        local found_ver=$("$GPUCLOUD_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (GPUCLOUD-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    # Place into ~/.gpucloud/node/ and symlink binaries to ~/.local/bin/
    ...
    ln -sf "$GPUCLOUD_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$GPUCLOUD_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$GPUCLOUD_HOME/node/bin/npx"  "$HOME/.local/bin/npx"
...
#!/usr/bin/env bash
echo "Warning: 'hermes' is deprecated; use 'gpucloud' instead." >&2
unset PYTHONPATH
unset PYTHONHOME
exec "$GPUCLOUD_BIN" "\$@"
EOF
    ...
    log_info "Installed deprecated hermes shim → $command_link_display_dir/hermes"
