        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.gpucloudDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask GPUCLOUD to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      target.kind !== 'file' ||
      !window.gpucloudDesktop?.watchPreviewFile ||
      !window.gpucloudDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.gpucloudDesktop.onPreviewFileChanged(payload => {
    ...
    void window.gpucloudDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.gpucloudDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.gpucloudDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:gpucloud-preview')
