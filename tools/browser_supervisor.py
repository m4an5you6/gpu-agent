...
One ``CDPSupervisor`` runs per GPUCLOUD ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "gpucloud-dialog-bridge.invalid"
...
(() => {
  if (window.__gpucloudDialogBridgeInstalled) return;
  window.__gpucloudDialogBridgeInstalled = true;
  const ENDPOINT = "http://gpucloud-dialog-bridge.invalid/";
