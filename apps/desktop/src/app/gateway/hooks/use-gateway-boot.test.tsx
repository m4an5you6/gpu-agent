...
// bug that drives the REAL useGatewayBoot hook + REAL GPUCLOUDGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshGPUCLOUDConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { gpucloudDesktop?: unknown }).gpucloudDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { gpucloudDesktop?: unknown }).gpucloudDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForGPUCLOUD) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startGPUCLOUD()'s remote branch awaits waitForGPUCLOUD() for 45s before it
    ...
    ;(window as { gpucloudDesktop?: unknown }).gpucloudDesktop = desktop
    ...
    // After ~45s waitForGPUCLOUD gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('GPUCLOUD backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
