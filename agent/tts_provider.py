...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into GPUCLOUD with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.gpucloud/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of GPUCLOUD
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``gpucloud tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``gpucloud setup`` for
