...
from gpucloud_cli.gpucloud_jobs import get_job, init_jobs_db
from gpucloud_cli.gpucloud_ssh import SSHResult
...
def test_start_health_and_stop_vllm_service(tmp_path, monkeypatch):
    ...
    path.write_text(MINIMAL, encoding="utf-8")
    db = tmp_path / "jobs.db"
    monkeypatch.setattr("gpucloud_cli.gpucloud_jobs.jobs_db_path", lambda: db)
    init_jobs_db()
    ...
    monkeypatch.setattr("gpucloud_cli.gpucloud_inference.run_ssh_command", fake_ssh)
