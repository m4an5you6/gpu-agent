...
export const $gpucloudHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, gpucloudHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_gpucloud_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $gpucloudHome.set(gpucloudHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch GPUCLOUD" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (GPUCLOUD-Setup.exe --update);
...
export async function launchGPUCLOUDDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_gpucloud_desktop', { installRoot })
}
