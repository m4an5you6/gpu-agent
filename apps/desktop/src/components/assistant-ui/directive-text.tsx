...
const GPUCLOUD_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type GPUCLOUDRefType = (typeof GPUCLOUD_REF_TYPES)[number]
...
const ICON_PATHS: Record<GPUCLOUDRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as GPUCLOUDRefType] ?? ICON_FALLBACK
...
const GPUCLOUD_DIRECTIVE_RE = new RegExp(
...
export const gpucloudDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(GPUCLOUD_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as GPUCLOUDRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: GPUCLOUDRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing GPUCLOUD directives (`@file:...`, `@image:...`) as
...
export function DirectiveContent({ text }: { text: string }) {
  ...
  const segments = useMemo(() => gpucloudDirectiveFormatter.parse(cleanedText), [cleanedText])
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.gpucloudDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.gpucloudDesktop?.readFileDataUrl(id)
