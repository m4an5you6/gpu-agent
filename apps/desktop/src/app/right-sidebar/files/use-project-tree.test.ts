...
import type { GPUCLOUDReadDirResult } from '@/global'

import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<GPUCLOUDReadDirResult>>()

beforeEach(() => {
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { gpucloudDesktop: { readDir: typeof readDir } }).gpucloudDesktop = { readDir }
})

afterEach(() => {
  resetProjectTreeState()
  delete (window as unknown as { gpucloudDesktop?: unknown }).gpucloudDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): GPUCLOUDReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: GPUCLOUDReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<GPUCLOUDReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.gpucloudDesktop is missing', async () => {
    delete (window as unknown as { gpucloudDesktop?: unknown }).gpucloudDesktop
