...
GPUCLOUD_HOME_DIR="${GPUCLOUD_HOME:-$HOME/.gpucloud}"
GPUCLOUD_CFG="${GPUCLOUD_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure GPUCLOUD config has twozero_td MCP entry ──
if [[ ! -f "$GPUCLOUD_CFG" ]]; then
    echo -e " ${FAIL} GPUCLOUD config not found at ${GPUCLOUD_CFG}"
    manual_steps+=("Create ${GPUCLOUD_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$GPUCLOUD_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in GPUCLOUD config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to GPUCLOUD config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$GPUCLOUD_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${GPUCLOUD_CFG} manually"); }
    manual_steps+=("Restart GPUCLOUD session to pick up config change")
fi
