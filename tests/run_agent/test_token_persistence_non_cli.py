...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    gpucloud_state = ModuleType("gpucloud_state")
    gpucloud_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "gpucloud_state", gpucloud_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "GPUCLOUD"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "GPUCLOUD"
