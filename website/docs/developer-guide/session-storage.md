...
GPUCLOUD Agent uses a SQLite database (`~/.gpucloud/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `gpucloud_state.py`


## Architecture Overview

```
~/.gpucloud/state.db (SQLite, WAL mode)
...
Multiple gpucloud processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.gpucloud/state.db
...
Default path: `~/.gpucloud/state.db`

This is derived from `gpucloud_constants.get_gpucloud_home()` which resolves to
`~/.gpucloud/` by default, or the value of `GPUCLOUD_HOME` environment variable.
