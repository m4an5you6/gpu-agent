...
Optional GPUCLOUD observability plugin that maps GPUCLOUD observer hooks to
...
not replace GPUCLOUD Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets GPUCLOUD emit NeMo Relay lifecycle events for
the work GPUCLOUD already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, GPUCLOUD Agent can:

- Preserve GPUCLOUD execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
gpucloud plugins enable observability/nemo_relay
```

The `GPUCLOUD_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `GPUCLOUD_HOME` that the
agent run will use:

```bash
env GPUCLOUD_HOME=/tmp/gpucloud-nemo-relay-test \
  gpucloud plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`GPUCLOUD_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`GPUCLOUD_HOME` is the GPUCLOUD profile/config home used by both
`gpucloud plugins enable ...` and the later `gpucloud chat ...` run. If unset,
GPUCLOUD uses the user's default home, usually `~/.gpucloud`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export GPUCLOUD_HOME=/tmp/gpucloud-nemo-relay-test
gpucloud plugins enable observability/nemo_relay
gpucloud chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `gpucloud` command you run is built from the
...
uv run gpucloud plugins enable observability/nemo_relay
uv run gpucloud chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/gpucloud_agent-*.whl
...
The plugin can configure exporters directly from `GPUCLOUD_NEMO_RELAY_*`
...
```bash
export GPUCLOUD_NEMO_RELAY_ATOF_ENABLED=1
export GPUCLOUD_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export GPUCLOUD_NEMO_RELAY_ATIF_ENABLED=1
export GPUCLOUD_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `GPUCLOUD_NEMO_RELAY_ATOF_FILENAME`
- `GPUCLOUD_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `GPUCLOUD_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `GPUCLOUD_NEMO_RELAY_ATIF_AGENT_NAME`
- `GPUCLOUD_NEMO_RELAY_ATIF_AGENT_VERSION`
- `GPUCLOUD_NEMO_RELAY_ATIF_MODEL_NAME`
- `GPUCLOUD_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point GPUCLOUD at it:

```bash
export GPUCLOUD_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `GPUCLOUD_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`GPUCLOUD_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`GPUCLOUD_NEMO_RELAY_ATIF_*` fallback setup is also skipped so GPUCLOUD does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
GPUCLOUD keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, GPUCLOUD routes LLM and tool
...
For the full generic GPUCLOUD middleware contract, see
...
export GPUCLOUD_HOME=/tmp/gpucloud-nemo-relay-docs/gpucloud-home
mkdir -p "$GPUCLOUD_HOME"

cat > "$GPUCLOUD_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent GPUCLOUD session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export GPUCLOUD_NEMO_RELAY_ATOF_ENABLED=1
export GPUCLOUD_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/gpucloud-nemo-relay-docs/subagent/atof
export GPUCLOUD_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export GPUCLOUD_NEMO_RELAY_ATOF_MODE=overwrite
export GPUCLOUD_NEMO_RELAY_ATIF_ENABLED=1
export GPUCLOUD_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/gpucloud-nemo-relay-docs/subagent/atif
export GPUCLOUD_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export GPUCLOUD_NEMO_RELAY_ATIF_AGENT_NAME='GPUCLOUD Agent E2E'
export GPUCLOUD_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export GPUCLOUD_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"gpucloud.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "GPUCLOUD Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. GPUCLOUD dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/gpucloud-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/gpucloud-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/gpucloud-nemo-relay-docs/workdir/beta.txt
cd /tmp/gpucloud-nemo-relay-docs/workdir

export GPUCLOUD_NEMO_RELAY_ATOF_ENABLED=1
export GPUCLOUD_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/gpucloud-nemo-relay-docs/parallel/atof
export GPUCLOUD_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export GPUCLOUD_NEMO_RELAY_ATOF_MODE=overwrite
export GPUCLOUD_NEMO_RELAY_ATIF_ENABLED=1
export GPUCLOUD_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/gpucloud-nemo-relay-docs/parallel/atif
export GPUCLOUD_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export GPUCLOUD_NEMO_RELAY_ATIF_AGENT_NAME='GPUCLOUD Agent E2E'
export GPUCLOUD_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- GPUCLOUD sessions map to `agent` scopes.
- GPUCLOUD API request hooks map to `llm` scope start/end events.
- GPUCLOUD tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses GPUCLOUD execution middleware to hand
...
Enable it for GPUCLOUD:

```bash
export GPUCLOUD_NEMO_RELAY_PLUGINS_TOML=/tmp/gpucloud-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, GPUCLOUD routes execution
through these boundaries:

```text
GPUCLOUD provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> GPUCLOUD provider adapter next_call(...)

GPUCLOUD tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> GPUCLOUD tool dispatcher next_call(...)
```
...
middleware for a local GPUCLOUD run. This path requires a NeMo Relay runtime that
...
```bash
export GPUCLOUD_HOME=/tmp/gpucloud-middleware-test/gpucloud-home
mkdir -p "$GPUCLOUD_HOME" /tmp/gpucloud-middleware-test/nemo-relay
...
cat > /tmp/gpucloud-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/gpucloud-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/gpucloud-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "GPUCLOUD Middleware E2E"
agent_version = "local"
...
export GPUCLOUD_NEMO_RELAY_PLUGINS_TOML=/tmp/gpucloud-middleware-test/nemo-relay/plugins.toml
