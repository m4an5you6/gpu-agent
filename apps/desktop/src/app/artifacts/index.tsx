...
import { getSessionMessages, listSessions } from '@/gpucloud'
...
import type { SessionInfo, SessionMessage } from '@/types/gpucloud'
  ...
  const openArtifact = useCallback(async (href: string) => {
    try {
      if (window.gpucloudDesktop?.openExternal) {
        await window.gpucloudDesktop.openExternal(href)
      } else {
