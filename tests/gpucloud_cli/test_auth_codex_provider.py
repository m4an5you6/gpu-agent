"""Tests for Codex auth — tokens stored in GPUCLOUD auth store (~/.gpucloud/auth.json)."""
...
def _setup_gpucloud_auth(gpucloud_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the GPUCLOUD auth store."""
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_gpucloud_auth(gpucloud_home)
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_gpucloud_auth(gpucloud_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("GPUCLOUD_HOME", str(gpucloud_home))
    ...
    monkeypatch.setattr("gpucloud_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    gpucloud_home = tmp_path / "gpucloud"
    _setup_gpucloud_auth(gpucloud_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("GPUCLOUD_HOME", str(gpucloud_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((gpucloud_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``gpucloud auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``gpucloud auth add openai-codex``.
    ...
    (gpucloud_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `gpucloud auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``gpucloud auth add openai-codex`` for accounts B and C
    ...
    (gpucloud_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``gpucloud auth add openai-codex`` (account B and account C).
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``gpucloud auth add openai-codex`` as a workaround when there was no
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to GPUCLOUD auth store, not ~/.codex/."""
    gpucloud_home = tmp_path / "gpucloud"
    codex_home = tmp_path / "codex-cli"
    gpucloud_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "gpucloud-at", "refresh_token": "gpucloud-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches GPUCLOUD store
    assert not (codex_home / "auth.json").exists()

    # GPUCLOUD auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "gpucloud-at"


def test_resolve_returns_gpucloud_auth_store_source(tmp_path, monkeypatch):
    gpucloud_home = tmp_path / "gpucloud"
    _setup_gpucloud_auth(gpucloud_home)
    monkeypatch.setenv("GPUCLOUD_HOME", str(gpucloud_home))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run gpucloud auth".
    """
    from gpucloud_cli.auth import (
    ...
    assert "gpucloud auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from gpucloud_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from gpucloud_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "gpucloud_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("gpucloud_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("gpucloud_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
