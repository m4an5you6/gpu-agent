...
GPUCLOUD Agent connects to external systems for AI inference, tool servers, IDE workflows, programmatic access, and more. These integrations extend what GPUCLOUD can do and where it can run.
...
GPUCLOUD supports multiple AI inference providers out of the box. Use `gpucloud model` to configure interactively, or set them in `config.yaml`.

- **[AI Providers](/user-guide/features/provider-routing)** — OpenRouter, Anthropic, OpenAI, Google, and any OpenAI-compatible endpoint. GPUCLOUD auto-detects capabilities like vision, streaming, and tool use per provider.
...
GPUCLOUD includes full browser automation with multiple backend options for navigating websites, filling forms, and extracting information:
...
- **[IDE Integration (ACP)](/user-guide/features/acp)** — Use GPUCLOUD Agent inside ACP-compatible editors such as VS Code, Zed, and JetBrains. GPUCLOUD runs as an ACP server, rendering chat messages, tool activity, file diffs, and terminal commands inside your editor.

## Programmatic Access

- **[API Server](/user-guide/features/api-server)** — Expose GPUCLOUD as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox — can connect and use GPUCLOUD as a backend with its full toolset.
...
GPUCLOUD runs as a gateway bot on 27+ messaging platforms, all configured through the same `gateway` subsystem:
...
- **[Plugin System](/user-guide/features/plugins)** — Extend GPUCLOUD with custom tools, lifecycle hooks, and CLI commands without modifying core code. Plugins are discovered from `~/.gpucloud/plugins/`, project-local `.gpucloud/plugins/`, and pip-installed entry points.
- **[Build a Plugin](/guides/build-a-gpucloud-plugin)** — Step-by-step guide for creating GPUCLOUD plugins with tools, hooks, and CLI commands.
