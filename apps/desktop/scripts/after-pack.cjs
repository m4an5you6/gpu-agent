 ...
 *
 * Stamps the GPUCLOUD icon + identity onto the packed Windows GPUCLOUD.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `gpucloud desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'GPUCLOUD')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'GPUCLOUD'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); GPUCLOUD.exe keeps the stock Electron icon`)
  }
}
