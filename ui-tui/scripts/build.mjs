...
await build({
  ...
  // Skip the prebuilt @gpucloud/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@gpucloud/ink': resolve(root, 'packages/gpucloud-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The gpucloud_cli launcher
