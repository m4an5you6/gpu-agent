"""ntfy platform adapter (GPUCLOUD plugin).
...
a GPUCLOUD dependency.

This adapter ships as a GPUCLOUD platform plugin under
``plugins/platforms/ntfy/``. The GPUCLOUD plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "gpucloud-in"              # subscribe topic (incoming)
          publish_topic: "gpucloud-out"     # optional — defaults to topic
...
_ECHO_TAG = "gpucloud-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``gpucloud cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a GPUCLOUD dependency",
