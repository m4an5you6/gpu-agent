"""Tests for gpucloud_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestSaveDisabledSkills:
    @patch("gpucloud_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from gpucloud_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("gpucloud_cli.config.load_config")
    ...
    @patch("gpucloud_cli.config.load_config")
    @patch.dict("os.environ", {"GPUCLOUD_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """GPUCLOUD_SESSION_PLATFORM should be used when GPUCLOUD_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_gpucloud_platform_takes_precedence(self, tmp_path, monkeypatch):
        """GPUCLOUD_PLATFORM should win over GPUCLOUD_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("GPUCLOUD_HOME", str(tmp_path))
        monkeypatch.setenv("GPUCLOUD_PLATFORM", "telegram")
        monkeypatch.setenv("GPUCLOUD_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from gpucloud_cli.skills_config import _get_categories
        skills = [
