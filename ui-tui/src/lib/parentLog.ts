...
const logDir = join(process.env.GPUCLOUD_HOME?.trim() || join(homedir(), '.gpucloud'), 'logs')
...
// a real ~/.gpucloud (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('gpucloud-tui: parent lifecycle log unavailable\n')
    }
  }
}
