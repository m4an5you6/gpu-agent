...
``gpucloud_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``gpucloud --help``, ``gpucloud version``,
``gpucloud logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``gpucloud --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from gpucloud_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["gpucloud", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["gpucloud", "-w", "--tui"], None),
        (["gpucloud", "version"], "version"),
        (["gpucloud", "--tui", "chat"], "chat"),
        (["gpucloud", "-w", "logs"], "logs"),
        (["gpucloud", "chat", "hello world"], "chat"),
        (["gpucloud", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["gpucloud", "-m", "gpt5", "chat"], "chat"),
        (["gpucloud", "--model", "gpt5", "chat", "hi"], "chat"),
        (["gpucloud", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["gpucloud", "-z", "hello world"], None),
        (["gpucloud", "-z", "hello", "chat"], "chat"),
        (["gpucloud", "--model=gpt5", "chat"], "chat"),     # inline form
        (["gpucloud", "--", "chat"], "chat"),               # -- terminator
        (["gpucloud", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["gpucloud", "some-plugin-cmd"], "some-plugin-cmd"),
        (["gpucloud", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["gpucloud"],                          # bare → chat
        ["gpucloud", "--help"],                # top-level help
        ["gpucloud", "-h"],
        ["gpucloud", "version"],               # known built-in
        ["gpucloud", "logs"],
        ["gpucloud", "gateway", "run"],
        ["gpucloud", "--tui"],
        ["gpucloud", "-w", "--tui"],
        ["gpucloud", "chat", "hi"],
        ["gpucloud", "help"],                  # accepted built-in-ish
        ["gpucloud", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["gpucloud", "meet", "join"],          # potential google_meet plugin
        ["gpucloud", "honcho", "status"],      # potential memory plugin
        ["gpucloud", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``gpucloud help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"gpucloud_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
