...
Plugins built on GPUCLOUD Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing GPUCLOUD
...
:class:`~gpucloud_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors GPUCLOUD' main config
...
handles every provider, fallback chain, and per-task override GPUCLOUD
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`gpucloud_cli.plugins.PluginContext`
