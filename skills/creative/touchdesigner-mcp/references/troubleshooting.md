...
## 5. GPUCLOUD Configuration

### Config location

`$GPUCLOUD_HOME/config.yaml` (defaults to `~/.gpucloud/config.yaml` when `GPUCLOUD_HOME` is unset)
...
Restart the GPUCLOUD session for changes to take effect. The MCP connection is
