...
-----------------------------------------------------------------------------
    ...
    ~/.gpucloud/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/gpucloud/<hash16>        — per-project branch tip
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("GPUCLOUD_CHECKPOINT_TIMEOUT", 30)))
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "gpucloud@local"], store, cfg_wd)
