...
# argparse runs (it sets ``GPUCLOUD_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``gpucloud_cli.relaunch`` should carry over when
...
Examples:
    ...
    gpucloud --tui                  Launch the modern TUI (or set display.interface: tui)
    gpucloud --cli                  Force the classic REPL (overrides display.interface: tui)
    ...
    gpucloud gateway                Run messaging gateway
    gpucloud -s gpucloud-agent-dev,github-auth
    gpucloud -w                     Start in isolated git worktree
    gpucloud gateway install        Install gateway background service
    ...
    gpucloud dashboard              Start web UI dashboard (port 9119)
    gpucloud dashboard --stop       Stop running dashboard processes
    gpucloud dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="gpucloud",
        description="GPUCLOUD Agent - AI assistant with tool-calling capabilities",
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via GPUCLOUD_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to GPUCLOUD_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.gpucloud/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also GPUCLOUD_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.gpucloud/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
