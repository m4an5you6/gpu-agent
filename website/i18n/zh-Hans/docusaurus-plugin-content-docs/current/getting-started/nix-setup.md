...
执行 `nix profile install` 后，`gpucloud`、`gpucloud-agent` 和 `hermes-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`gpucloud setup` 引导你完成提供商选择，`gpucloud gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.gpucloud/`。
...
~/.gpucloud -> /var/lib/hermes/.gpucloud       （符号链接桥接，按 hostUsers）
...
对于只包含 `plugin.yaml` + `__init__.py` 的源码树插件（例如 [hermes-lcm](https://github.com/stephenschoettler/gpucloud-lcm)）：
...
| `stateDir` | `str` | `"/var/lib/hermes"` | 状态目录（`GPUCLOUD_HOME` 的父目录） |
