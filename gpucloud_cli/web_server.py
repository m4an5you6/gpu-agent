"""
GPUCLOUD Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m gpucloud_cli.main web          # Start on http://127.0.0.1:9119
    python -m gpucloud_cli.main web --port 8080
"""
...
try:
    ...
    # running `gpucloud dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["GPUCLOUD_WEB_DIST"]) if "GPUCLOUD_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``gpucloud gateway run`` — but the
    desktop app spawns a ``gpucloud dashboard`` backend, not a gateway, so a cron
    ...
    GPUCLOUD_HOME — whichever process grabs the lock first wins the tick.
    """
...
async def _lifespan(app: "FastAPI"):
    ...
    # Desktop-spawned backends (GPUCLOUD_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `gpucloud
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("GPUCLOUD_DESKTOP") == "1":
...
app = FastAPI(title="GPUCLOUD Agent", version=__version__, lifespan=_lifespan)

# ---------------------------------------------------------------------------
# Session token for protecting sensitive endpoints (reveal).
# The desktop shell mints the token and injects it via
# GPUCLOUD_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("GPUCLOUD_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-GPUCLOUD-Session-Token"
...
#
# This list is defined in ``gpucloud_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from gpucloud_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from gpucloud_cli.dashboard_auth.middleware import gated_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates GPUCLOUD (no terminal prompt), "
...
async def get_status():
    ...
    # and which providers are registered so ``gpucloud status`` and the
    # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from gpucloud_cli.dashboard_auth import list_providers as _list_providers
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the gpucloud home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_gpucloud_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_gpucloud_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from gpucloud_cli.auth import get_nous_auth_status
    ...
    features = []
    try:
        from gpucloud_cli.nous_subscription import get_nous_subscription_features
...
async def run_prompt_size():
    try:
        proc = _spawn_gpucloud_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_gpucloud_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_gpucloud_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from gpucloud_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.gpucloud/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_gpucloud_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "gpucloud-update": "gpucloud-update.log",
...
def _spawn_gpucloud_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``gpucloud <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``gpucloud_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "gpucloud_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "GPUCLOUD_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``gpucloud gateway restart`` in the background."""
    try:
        proc = _spawn_gpucloud_action(["gateway", "restart"], "gateway-restart")
    except Exception as exc:
...
@app.post("/api/gpucloud/update")
async def update_gpucloud():
    """Kick off ``gpucloud update`` in the background."""
    install_method = detect_install_method(PROJECT_ROOT)
    if install_method == "docker":
        message = format_docker_update_message()
        _record_completed_action("gpucloud-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "gpucloud-update",
    ...
    try:
        proc = _spawn_gpucloud_action(["update"], "gpucloud-update")
    except Exception as exc:
        _log.exception("Failed to spawn gpucloud update")
...
@app.get("/api/gpucloud/update/check")
async def check_gpucloud_update(force: bool = False):
    """Report whether a GPUCLOUD update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/gpucloud/update`` actually runs ``gpucloud update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed GPUCLOUD version string
    ...
    try:
        ...
        if force:
            try:
                (get_gpucloud_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.gpucloud/config.yaml`` under ``tts.``.
    """
...
async def get_profiles_sessions(
    ...
    from gpucloud_state import SessionDB
    from gpucloud_cli import profiles as profiles_mod
...
async def search_sessions(q: str = "", limit: int = 20):
    ...
    try:
        ...
        try:
            ...
            # logs, or another GPUCLOUD surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    GPUCLOUD supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in gpucloud_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options():
    ...
    """
    try:
        from gpucloud_cli.inventory import build_models_payload, load_picker_context
        ...
        # onboarding picker) sees the SAME full provider universe `gpucloud model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `gpucloud model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            from gpucloud_cli.auth import get_provider_auth_state
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.gpucloud/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if scope == "main":
            ...
            # post-model-selection behaviour (gpucloud_cli/main.py
            ...
            if provider.strip().lower() == "nous":
                try:
                    from gpucloud_cli.nous_subscription import apply_nous_managed_defaults
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run GPUCLOUD from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect GPUCLOUD to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use GPUCLOUD from Slack via Socket Mode.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect GPUCLOUD to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use GPUCLOUD in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use GPUCLOUD through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from GPUCLOUD via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to GPUCLOUD through an IMAP/SMTP mailbox.",
        "docs_url": "https://gpucloud-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect GPUCLOUD to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use GPUCLOUD inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use GPUCLOUD through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect GPUCLOUD to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect GPUCLOUD to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose GPUCLOUD as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://gpucloud-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://gpucloud-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.gpucloud-agent.nousresearch.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"GPUCLOUDDashboard/{__version__}"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "GPUCLOUD Agent").strip() or "GPUCLOUD Agent"
...
# the canonical ``gpucloud auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    GPUCLOUD resolves Anthropic creds in this order at runtime:
    1. ``~/.gpucloud/.anthropic_oauth.json`` — GPUCLOUD-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_gpucloud_oauth_credentials,
            read_claude_code_credentials,
            _GPUCLOUD_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_gpucloud_oauth_credentials = None  # type: ignore
        _GPUCLOUD_OAUTH_FILE = None  # type: ignore

    gpucloud_creds = None
    if read_gpucloud_oauth_credentials:
        try:
            gpucloud_creds = read_gpucloud_oauth_credentials()
        except Exception:
            gpucloud_creds = None
    if gpucloud_creds and gpucloud_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "gpucloud_pkce",
            "source_label": f"GPUCLOUD PKCE ({_GPUCLOUD_OAUTH_FILE})",
            "token_preview": _truncate_token(gpucloud_creds.get("accessToken")),
            "expires_at": gpucloud_creds.get("expiresAt"),
            "has_refresh_token": bool(gpucloud_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into GPUCLOUD even
    when they also have a separate GPUCLOUD-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "gpucloud auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "gpucloud auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "gpucloud auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "gpucloud auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "gpucloud auth add xai-oauth",
        "docs_url": "https://gpucloud-agent.nousresearch.com/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "gpucloud auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from gpucloud_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("gpucloud_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same GPUCLOUD-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _GPUCLOUD_OAUTH_FILE
            if _GPUCLOUD_OAUTH_FILE.exists():
                _GPUCLOUD_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from gpucloud_cli.auth import clear_provider_auth
...
#          → persists to ~/.gpucloud/.anthropic_oauth.json AND credential pool
...
# Guarded so gpucloud web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both GPUCLOUD file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``gpucloud auth add anthropic``.
    """
    from agent.anthropic_adapter import _GPUCLOUD_OAUTH_FILE
    payload = {
    ...
    _GPUCLOUD_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _GPUCLOUD_OAUTH_FILE.with_name(
        f"{_GPUCLOUD_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _GPUCLOUD_OAUTH_FILE)
        try:
            _GPUCLOUD_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "gpucloud-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("GPUCLOUD_PORTAL_BASE_URL")
...
# the tokens exactly like `gpucloud auth add xai-oauth`.
...
def _xai_loopback_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = hauth._xai_validate_inference_base_url(
            os.getenv("GPUCLOUD_XAI_BASE_URL", "").strip().rstrip("/")
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `gpucloud auth add xai-oauth`'s credential-pool insert.
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from gpucloud_cli.auth import persist_nous_credentials
...
def _minimax_poller(session_id: str) -> None:
    ...
    ``gpucloud auth add minimax-oauth``.
    """
    from gpucloud_cli.auth import (
        _minimax_poll_token,
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from gpucloud_cli.auth import _save_codex_tokens
...
async def get_session_stats():
    """Session-store statistics for the Sessions page (mirrors `gpucloud sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `gpucloud sessions prune`)."""
...
async def get_logs(
    ...
    from gpucloud_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_gpucloud_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from gpucloud_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, GPUCLOUD_HOME)."""
    from gpucloud_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["gpucloud_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process GPUCLOUD_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (gpucloud_cli.mcp_config), so
# servers managed here show up under `gpucloud mcp list` and vice versa.  Secrets
...
async def list_mcp_servers():
    from gpucloud_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate):
    from gpucloud_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str):
    from gpucloud_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str):
    ...
    from gpucloud_cli.mcp_config import _get_mcp_servers, _probe_single_server
...
async def list_mcp_catalog():
    ...
    `gpucloud mcp catalog` / `gpucloud mcp install` read.
    """
    try:
        from gpucloud_cli import mcp_catalog
    except Exception as exc:
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall):
    ...
    if entry.install is not None:
        try:
            proc = _spawn_gpucloud_action(["mcp", "install", name], "mcp-install")
        except Exception as exc:
...
#
# Wraps the same JSON store the CLI uses (gpucloud_cli.webhook); the webhook
...
async def list_webhooks():
    import gpucloud_cli.webhook as wh
...
# spawn the real `gpucloud gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway():
    try:
        proc = _spawn_gpucloud_action(["gateway", "start"], "gateway-start")
    except Exception as exc:
...
async def stop_gateway():
    try:
        proc = _spawn_gpucloud_action(["gateway", "stop"], "gateway-stop")
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from gpucloud_cli.auth import read_credential_pool
...
# `gpucloud memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `gpucloud memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_gpucloud_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_gpucloud_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_gpucloud_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    try:
        proc = _spawn_gpucloud_action(["import", archive], "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    try:
        from gpucloud_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <gpucloud_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_gpucloud_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_gpucloud_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
async def install_skill_hub(body: SkillInstallRequest):
    ...
    try:
        proc = _spawn_gpucloud_action(["skills", "install", identifier], "skills-install")
    except Exception as exc:
...
async def uninstall_skill_hub(body: SkillUninstallRequest):
    ...
    try:
        proc = _spawn_gpucloud_action(["skills", "uninstall", name, "--yes"], "skills-uninstall")
    except Exception as exc:
...
async def update_skills_hub():
    try:
        proc = _spawn_gpucloud_action(["skills", "update"], "skills-update")
    except Exception as exc:
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from gpucloud_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_gpucloud_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "gpucloud setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local GPUCLOUD_HOME override so the write lands in the target
    ...
    """
    from gpucloud_constants import set_gpucloud_home_override, reset_gpucloud_home_override

    token = set_gpucloud_home_override(str(profile_dir))
    try:
        cfg = load_config()
        cfg["model"] = _apply_main_model_assignment(cfg.get("model", {}), provider, model)
        save_config(cfg)
    finally:
        reset_gpucloud_home_override(token)
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``gpucloud profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from GPUCLOUD_HOME).
    """
    from gpucloud_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``gpucloud profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the GPUCLOUD_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``gpucloud profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from gpucloud_cli import profile_describer
...
async def get_skills():
    ...
    from gpucloud_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from gpucloud_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from gpucloud_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle):
    ...
    helper the CLI ``gpucloud tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str):
    ...
    Surfaces the same provider rows the CLI ``gpucloud tools`` picker shows
...
async def select_toolset_provider(name: str, body: ToolsetProviderSelect):
    ...
    core extracted from the CLI configurator — so the GUI and ``gpucloud tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate):
    ...
    Writes each ``key: value`` to ``~/.gpucloud/.env`` via ``save_env_value`` —
    the same store ``gpucloud tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(name: str, body: ToolsetPostSetup):
    ...
    ``gpucloud tools post-setup <key>`` and the frontend tails the log via
...
#
# The endpoint spawns the same ``gpucloud --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from gpucloud_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from gpucloud_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from gpucloud_cli.dashboard_auth.audit import AuditEvent, audit_log
        from gpucloud_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``gpucloud --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``GPUCLOUD_TUI_RESUME`` env var —
    matching what ``gpucloud_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``GPUCLOUD_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``GPUCLOUD_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from gpucloud_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("GPUCLOUD_TUI_DISABLE_MOUSE", "1")
    env.setdefault("GPUCLOUD_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["GPUCLOUD_TUI_RESUME"] = resume

    if sidecar_url:
        env["GPUCLOUD_TUI_SIDECAR_URL"] = sidecar_url

    if gateway_ws_url := _build_gateway_ws_url():
        env["GPUCLOUD_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from gpucloud_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall GPUCLOUD inside WSL2 to use the dashboard's /chat "
...
# GPUCLOUD_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`gpucloud_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from gpucloud_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/gpucloud/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /gpucloud`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__GPUCLOUD_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/gpucloud``)
        ...
        ``__GPUCLOUD_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__GPUCLOUD_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__GPUCLOUD_BASE_PATH__="{prefix}";'
                f"window.__GPUCLOUD_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__GPUCLOUD_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__GPUCLOUD_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__GPUCLOUD_BASE_PATH__="{prefix}";'
                f"window.__GPUCLOUD_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/gpucloud`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the GPUCLOUD backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "GPUCLOUD Teal",         "description": "Classic dark teal — the canonical GPUCLOUD look"},
    {"name": "default-large", "label": "GPUCLOUD Teal (Large)", "description": "GPUCLOUD Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.gpucloud/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.gpucloud/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_gpucloud_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.gpucloud/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as gpucloud_cli.plugins):
    1. User plugins:    ~/.gpucloud/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.gpucloud/plugins/  (only if GPUCLOUD_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from gpucloud_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_gpucloud_home() / "plugins", "user"),
    ...
    # ``gpucloud_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("GPUCLOUD_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".gpucloud" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from gpucloud_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_gpucloud_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"gpucloud auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from gpucloud_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from gpucloud_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from gpucloud_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from gpucloud_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.gpucloud/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.gpucloud/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"gpucloud_dashboard_plugin_{plugin['name']}"
...
from gpucloud_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        from gpucloud_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing GPUCLOUD_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with GPUCLOUD Agent exposes a
    ...
    print(f"  GPUCLOUD Web UI → http://{host}:{port}")
