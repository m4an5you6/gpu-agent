"""Tests for the GPUCLOUD plugin system (gpucloud_cli.plugins)."""
...
from gpucloud_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<gpucloud_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<gpucloud_home>/plugins/``; we derive
    ``<gpucloud_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <GPUCLOUD_HOME>/config.yaml.
        # Config is always read from GPUCLOUD_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        gpucloud_home_str = os.environ.get("GPUCLOUD_HOME")
        if gpucloud_home_str:
            gpucloud_home = Path(gpucloud_home_str)
        else:
            gpucloud_home = base.parent
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.gpucloud/plugins/ are discovered."""
        plugins_dir = tmp_path / "gpucloud_test" / "plugins"
    ...
    def test_execution_middleware_does_not_retry_downstream_failure(self, monkeypatch):
        ...
        monkeypatch.setattr("gpucloud_cli.plugins.get_plugin_manager", lambda: manager)
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("GPUCLOUD_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".gpucloud" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        gpucloud_home = tmp_path / "gpucloud_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under gpucloud_plugins.<name>."""
        plugins_dir = tmp_path / "gpucloud_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("GPUCLOUD_HOME", str(tmp_path / "gpucloud_test"))

        # Clean up any prior namespace module
        sys.modules.pop("gpucloud_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "gpucloud_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="gpucloud_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "gpucloud_cli.plugins.invoke_hook",
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import gpucloud_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("gpucloud_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        gpucloud_home = tmp_path / "gpucloud_test"
        plugins_dir = gpucloud_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "gpucloud_test" / "plugins"
        # Set GPUCLOUD_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("gpucloud_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("gpucloud_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("gpucloud_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("gpucloud_cli.plugins.PluginContext.dispatch_tool.__module__", "gpucloud_cli.plugins"):
...
class TestPluginDebugLogging:
    """GPUCLOUD_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("GPUCLOUD_PLUGINS_DEBUG", raising=False)
        from gpucloud_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With GPUCLOUD_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("GPUCLOUD_PLUGINS_DEBUG", "1")
        from gpucloud_cli import plugins as plugins_mod
