...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_gpucloud_env(self):
        """GPUCLOUD' own .env inside GPUCLOUD_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: gpucloud internal .env is in ~/.gpucloud/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.gpucloud/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.gpucloud/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal GPUCLOUD cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        gpucloud_home = tmp_path / ".gpucloud"
        cache = gpucloud_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._gpucloud_home_path", return_value=gpucloud_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal GPUCLOUD cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        gpucloud_home = tmp_path / ".gpucloud"
        hub = gpucloud_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_gpucloud_home(self, tmp_path):
        """The env basename guard does not depend on GPUCLOUD_HOME resolution."""
        gpucloud_home = tmp_path / ".gpucloud"
        gpucloud_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        gpucloud_home = tmp_path / ".gpucloud"
        cache = gpucloud_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
