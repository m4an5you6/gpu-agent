...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("gpucloud_cli.config.get_config_path", return_value=config_path), \
             patch("gpucloud_cli.config.ensure_gpucloud_home"):
    ...
    def test_ensure_gpucloud_home_sets_0700(self):
        home = Path(self.tmpdir) / ".gpucloud"
        with patch("gpucloud_cli.config.get_gpucloud_home", return_value=home):
