...
def _isolate(tmp_path, monkeypatch):
    """Isolate GPUCLOUD_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal GPUCLOUDCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = GPUCLOUDCLI.__new__(GPUCLOUDCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["gpucloud-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below GPUCLOUD' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly GPUCLOUD' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above GPUCLOUD' minimum context length."""
