...
``/opt/hermes``), ``<root>`` is ``GPUCLOUD_HOME``. Legacy env-var
...
def _resolve_hermes_argv() -> list[str]:
    ...
    hermes_bin = _safe_which_no_cwd("gpucloud") if _IS_WINDOWS else shutil.which("gpucloud")
    if hermes_bin:
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled GPUCLOUD_PROFILE values like "hermes-system"
