 ...
 * product name (`GPUCLOUD`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/GPUCLOUD'
 *
 * This is a hard failure with no obvious cause for the user — `gpucloud desktop`
 ...
 * (the mac-unpacked GPUCLOUD.app bundle) and Windows (win-unpacked), so we
